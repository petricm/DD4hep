...
/// Default constructor
ConditionsDependencyHandler::ConditionsDependencyHandler( ConditionsManager::Object* mgr, UserPool& pool,
                                                          const Dependencies& dependencies, void* user_param )
...
Condition::Object* ConditionsDependencyHandler::do_callback( const ConditionDependency& dep ) const {
  try {
    ...
    if ( obj ) {
      if ( !obj->hash )
        obj->hash = ConditionKey::hashCode( obj->name );
    ...
    printout( ERROR, "ConditionDependency", "+++ Exception while creating dependent Condition %s:",
              dep.target.name.c_str() );
    printout( ERROR, "ConditionDependency", "\t\t%s", e.what() );
  } catch ( ... ) {
    printout( ERROR, "ConditionDependency", "+++ UNKNOWN exception while creating dependent Condition %s.",
              dep.target.name.c_str() );
  }
  m_pool.print( "*" );
  except( "ConditionDependency", "++ Exception while creating dependent Condition %s.", dep.target.name.c_str() );
  return 0;
}
