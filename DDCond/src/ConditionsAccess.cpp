...
Condition ConditionsAccess::get( Condition::key_type key, const Condition::iov_type& req_validity ) {
  return access()->get( key, req_validity );
}

/// Retrieve a condition given the conditions path = <Detector Element path>.<conditions name>
Condition ConditionsAccess::get( const string& path, const Condition::iov_type& req_validity ) {
  Condition::key_type key = ConditionKey::hashCode( path );
  return access()->get( key, req_validity );
}

/// Retrieve a condition given a Detector Element and the conditions name
RangeConditions ConditionsAccess::getRange( Condition::key_type key, const Condition::iov_type& req_validity ) {
  return access()->getRange( key, req_validity );
}

/// Retrieve a condition given a Detector Element path and the conditions name
RangeConditions ConditionsAccess::getRange( const string& path, const Condition::iov_type& req_validity ) {
  Condition::key_type key = ConditionKey::hashCode( path );
  return access()->getRange( key, req_validity );
}
