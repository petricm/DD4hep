...
template <typename T>
const IOVType* check_iov_type( const ConditionsManagerObject* o, const IOV* iov );

/// Helper: Specialized IOV check
template <>
const IOVType* check_iov_type<void>( const ConditionsManagerObject* o, const IOV* iov ) {
  if ( iov ) {
...
template <>
const IOVType* check_iov_type<Discrete>( const ConditionsManagerObject* o, const IOV* iov ) {
  const IOVType* typ = check_iov_type<void>( o, iov );
  if ( typ && !iov->has_range() )
    return typ;
  return 0;
}
/// Helper: Specialized IOV check for range IOV values
template <>
const IOVType* check_iov_type<Range>( const ConditionsManagerObject* o, const IOV* iov ) {
  const IOVType* typ = check_iov_type<void>( o, iov );
  if ( typ && iov->has_range() )
    return typ;
  return 0;
...
template <typename T>
void __check_values__( const ConditionsManagerObject* o, Condition::key_type key, const IOV* iov ) {
  if ( !iov ) {
  ...
  if ( !typ ) {
    ...
    except( "ConditionsManager", "+++ Invalid IOV type [%d] to access condition: %08X.", iov->type, key );
  }
}
  ...
  if ( !conditions.empty() ) {
    ...
    for ( size_t j = 0; j < conditions.size(); ++j ) {
      for ( RC::const_iterator i = conditions.begin(); i != conditions.end(); ++i ) {
        ...
        if ( k.first <= test.first + 1 && k.second >= test.first )
          test.first = k.second;
        if ( k.first + 1 <= test.second && k.second >= test.second )
          test.second = k.first;
        // printout(INFO,"Test","IOV: %ld,%ld --> %ld,%ld",k.first,k.second, test.first, test.second);
        if ( test.first >= test.second )
          return true;
      }
      if ( test.first <= iov.keyData.first && test.second >= iov.keyData.second )
        return false;
    }
  }
  return false;
...
ConditionsManagerObject::ConditionsManagerObject( LCDD& lcdd_instance )
    : NamedObject(),
      ObjectExtensions( typeid( ConditionsManagerObject ) ),
      m_lcdd( lcdd_instance ),
      m_iovTypes(),
      m_rawPool(),
      m_updateLock(),
      m_poolLock(),
...
void ConditionsManagerObject::initialize() {
  if ( !m_updatePool.get() ) {
    ...
    if ( !m_updatePool.get() ) {
      except( "ConditionsManager", "+++ The update pool of type %s cannot be created. [%s]", m_updateType.c_str(),
              Errors::noSys().c_str() );
    }
...
pair<bool, const IOVType*> ConditionsManagerObject::registerIOVType( size_t iov_type, const string& iov_name ) {
  if ( iov_type < m_iovTypes.size() ) {
    ...
    if ( eq_type && eq_name ) {
      ...
      except( "ConditionsManager", "Cannot register IOV %s. Type %d already in use!", iov_name.c_str(), iov_type );
    }
    t.name              = iov_name;
    t.type              = iov_type;
...
void ConditionsManagerObject::fromString( const std::string& data, IOV& iov ) {
  ...
  if ( nents == 0 ) {
    except( "ConditionsManager", "+++ Failed to read keys from IOV string representation: %s", data.c_str() );
  }
  ...
  if ( !typ ) {
    // Severe: We have an unknown IOV type. This is not allowed,
    // because we do not known hot to handle it.....
    except( "ConditionsManager", "+++ Unknown IOV type requested from data: %s. [%s]", data.c_str(),
...
bool ConditionsManagerObject::registerUnlocked( ConditionsPool* pool, Condition cond ) {
  if ( pool && cond.isValid() ) {
    ...
    except( "ConditionsManager", "+++ Unknown Conditions pool to register entry. [%s]", Errors::invalidArg().c_str() );
...
Condition ConditionsManagerObject::__queue_update( Conditions::Entry* e ) {
  if ( e ) {
    ...
    p->insert( c );
    if ( s_debug > INFO ) {
      printout( INFO, "Conditions", "+++ Loaded condition: %s.%s to %s [%s] V: %s", e->detector.path().c_str(),
                c->name.c_str(), c->value.c_str(), c->type.c_str(), c->validity.c_str() );
    }
    return c;
  }
  return Condition();
...
void ConditionsManagerObject::__get_checked_pool( const IOV& req_iov, dd4hep_ptr<UserPool>& up ) {
...
long ConditionsManagerObject::prepare( const IOV& req_iov, const ConditionKeys& keys, dd4hep_ptr<UserPool>& up ) {
  RC valid, expired;
...
/// Prepare all updates for the given keys to the clients with the defined IOV
long ConditionsManagerObject::prepare( const IOV& req_iov, const ConditionKeys& keys, dd4hep_ptr<UserPool>& up,
                                       const Dependencies& dependencies, bool verify_dependencies ) {
...
bool ConditionsManagerObject::select( Condition::key_type key, const Condition::iov_type& req_validity,
...
/// Retrieve  a condition set given a Detector Element and the conditions name according to their validity
bool ConditionsManagerObject::select_range( Condition::key_type key, const Condition::iov_type& req_validity,
                                            RangeConditions& conditions ) {
  {
    ConditionsIOVPool* p = 0;
    dd4hep_lock_t      locked_action( m_poolLock );
...
Condition ConditionsManagerObject::get( Condition::key_type key, const Condition::iov_type& iov ) {
  RC conditions;
  ...
  if ( conditions.size() == 1 ) {
    ...
    except( "ConditionsManager", "+++ Condition %08X for the requested IOV %s do not exist.", key, iov.str().c_str() );
  } else if ( conditions.size() > 1 ) {
    RC::const_iterator start = conditions.begin();
    Condition          first = *start;
    printout( ERROR, "ConditionsManager", "+++ Condition %s [%08X] is ambiguous for IOV %s:", first.name().c_str(), key,
              iov.str().c_str() );
    for ( RC::const_iterator i = start; i != conditions.end(); ++i ) {
      Condition c = *i;
      printout( ERROR, "ConditionsManager", "+++ %s [%s] = %s", c.name().c_str(), c->iov->str().c_str(),
                c->value.c_str() );
    }
    except( "ConditionsManager", "+++ Condition %s [%08X] is ambiguous for IOV %s:", first.name().c_str(), key,
            iov.str().c_str() );
  }
  return Condition();
}
...
/// Retrieve a condition given a Detector Element and the conditions name
RangeConditions ConditionsManagerObject::getRange( Condition::key_type key, const Condition::iov_type& iov ) {
  RC conditions;
  ...
  if ( rc ) {
    ...
    if ( conditions.empty() ) {
      except( "ConditionsManager", "+++ Conditions %08X for IOV %s do not exist.", key, iov.str().c_str() );
    }
    conditions.clear();
  }
