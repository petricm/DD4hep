...
template <>
ConditionsManager ConditionsManager::from<LCDD>( LCDD& host ) {
  Object* obj = host.extension<Object>();
  if ( obj )
    return ConditionsManager( obj );
...
pair<bool, const IOVType*> ConditionsManager::registerIOVType( size_t iov_type, const string& iov_name ) {
...
const vector<const IOVType*> ConditionsManager::iovTypesUsed() const {
  ...
  for ( IOVTypes::const_iterator i = types.begin(); i != types.end(); ++i )
    if ( int( ( *i ).type ) != IOVType::UNKNOWN_IOV )
      result.push_back( &( *i ) );
  return result;
}
...
long ConditionsManager::prepare( const IOV& required_validity, const ConditionKeys& keys,
...
/// Prepare all updates for the given keys to the clients with the defined IOV
long ConditionsManager::prepare( const IOV& required_validity, const ConditionKeys& keys,
                                 dd4hep_ptr<UserPool>& user_pool, const Dependencies& dependencies,
