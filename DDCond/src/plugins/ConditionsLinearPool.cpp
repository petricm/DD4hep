...
class ConditionsLinearPool : public BASE {
 ...
 public:
  ...
  virtual Condition exists( Condition::key_type key ) const {
    typename Mapping::const_iterator i = find_if( m_entries.begin(), m_entries.end(), HashConditionFind( key ) );
 ...
 */
template <typename MAPPING, typename BASE>
class ConditionsLinearUpdatePool : public ConditionsLinearPool<MAPPING, BASE> {
  typedef typename ConditionsLinearPool<MAPPING, BASE>::key_type key_type;

 public:
  /// Default constructor
  ConditionsLinearUpdatePool( ConditionsManager mgr ) : ConditionsLinearPool<MAPPING, BASE>( mgr ) {
  }
  ...
  /// Select the conditions matching the DetElement and the conditions name
  virtual void select_range( Condition::key_type key, const Condition::iov_type& req, RangeConditions& result ) {
    MAPPING& m = this->ConditionsLinearPool<MAPPING, BASE>::m_entries;
    if ( !m.empty() ) {
...
/// Namespace for the geometry part of the AIDA detector description toolkit
namespace Conditions {} /* End namespace Conditions             */
} /* End namespace DD4hep                   */
...
#define _CR( fun, x, b, y )                                       \
  void* fun( DD4hep::Geometry::LCDD&, int argc, char** argv ) {   \
