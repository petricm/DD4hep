  ...
  for ( _T::const_iterator i = types.begin(); i != types.end(); ++i ) {
    ...
    if ( type ) {
      ...
      if ( pool ) {
        ...
        printout( INFO, "CondPoolDump", "+++ ConditionsIOVPool for type %s  [%d IOV elements]", type->str().c_str(),
                  int( e.size() ) );
...
static long ddcond_synchronize_conditions( lcdd_t& lcdd, int argc, char** argv ) {
  if ( argc > 0 ) {
    ...
    if ( iov_type == "epoch" ) {
      ...
      printout( INFO, "Conditions", "+++ ConditionsUpdate: Updated %ld conditions... event time: %s", num_updated,
                c_evt );
    } else {
      printout( INFO, "Conditions", "+++ ConditionsUpdate: Updated %ld conditions... key[%s]: %ld", num_updated,
                iov_type.c_str(), iov_key );
    }
...
static long ddcond_clean_conditions( lcdd_t& lcdd, int argc, char** argv ) {
  if ( argc > 0 ) {
    ...
    printout( INFO, "Conditions", "+++ ConditionsUpdate: Cleaning conditions... type:%s max age:%d", iov_type.c_str(),
              max_age );
    ConditionsManager manager = ConditionsManager::from( lcdd );
    const IOVType*    epoch   = manager.iovType( iov_type );
...
static long ddcond_create_repository( lcdd_t& lcdd, int argc, char** argv ) {
  if ( argc > 0 ) {
    ...
    printout( INFO, "Conditions", "+++ ConditionsRepository: Creating %s", output.c_str() );
    ConditionsManager manager = ConditionsManager::from( lcdd );
