...
class ConditionsMultiLoader : public ConditionsDataLoader {
 ...
 public:
  ...
  virtual size_t load( key_type key, const iov_type& req_validity, RangeConditions& conditions );
  /// Load  a condition set given a Detector Element and the conditions name according to their validity
  virtual size_t load_range( key_type key, const iov_type& req_validity, RangeConditions& conditions );
  /// Update a range of conditions according to the required IOV
  virtual size_t update( const iov_type& req_validity, RangeConditions& conditions, iov_type& conditions_validity );
};
...
ConditionsDataLoader* ConditionsMultiLoader::load_source( const std::string& nam, const iov_type& req_validity ) {
  ...
  if ( iop == m_openSources.end() ) {
    ...
    if ( ild == m_loaders.end() ) {
      ...
      if ( !loader ) {
        except( "ConditionsMultiLoader", "Failed to create conditions loader of type: " + typ + " to read:" + nam );
      }
    } else {
      loader = ( *ild ).second;
    }
...
/// Load  a condition set given a Detector Element and the conditions name according to their validity
size_t ConditionsMultiLoader::load_range( key_type key, const iov_type& req_validity, RangeConditions& conditions ) {
...
size_t ConditionsMultiLoader::load( key_type key, const iov_type& req_validity, RangeConditions& conditions ) {
  size_t len = conditions.size();
  // No better idea: Must chack all sources to find the required condition
  for ( Sources::const_iterator i = m_sources.begin(); i != m_sources.end(); ++i ) {
    const IOV& iov = ( *i ).second;
...
/// Update a range of conditions according to the required IOV
size_t ConditionsMultiLoader::update( const iov_type& req_validity, RangeConditions& conditions,
  ...
  for ( RangeConditions::const_iterator i = conditions.begin(); i != conditions.end(); ++i ) {
    ...
    if ( items < 1 ) {
      ...
      except( "ConditionsManager", "+++ update_expired: Cannot update condition %s [%s] to iov:%s.", cond->name.c_str(),
              cond->iov->str().c_str(), req_validity.str().c_str() );
    }
  }
  conditions = upda;
  for ( RangeConditions::const_iterator i = conditions.begin(); i != conditions.end(); ++i )
