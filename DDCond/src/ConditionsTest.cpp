...
template <typename T>
void print_bound_condition( Condition c, const char* norm ) {
}
...
void __print_bound_val<string>( Condition c, const char* ) {
  ...
  printout( INFO, "Cond_Value", "  Bound value  %s : string value:%s  Type: %s Ptr:%016X", c.name().c_str(),
            c.get<string>().c_str(), typeName( c.typeInfo() ).c_str(), (void*)&v );
}
template <typename T>
void __print_bound_container( Condition c, const char* ) {
  const T& v = access_val<T>( c );
  printout( INFO, "Cond_Value", "  Bound value  %s : size:%d = %s Type: %s Ptr:%016X", c.name().c_str(),
            int( v.size() ), c.data().str().c_str(), typeName( c.typeInfo() ).c_str(), (void*)&v );
}

#define TEMPLATE_SIMPLE_TYPE( x )                              \
  template <>                                                  \
  void print_bound_value<x>( Condition c, const char* norm ) { \
...
#define TEMPLATE_CONTAINER_TYPE( container, x )                                      \
  ...
  template <>                                                                        \
  void print_bound_value<container>( Condition c, const char* norm ) {               \
...
void print_condition<void>( Condition c ) {
  string type = c.type();
  printout( INFO, "Cond_Value", "%-32s  [%16s] :  %s [%s] ", c.name().c_str(), c.type().c_str(), c.value().c_str(),
            c->validity.c_str() );
  if ( type == "alignment" )
...
void check_discrete_condition( Condition c, const IOV& iov ) {
  ...
  if ( i->is_discrete() ) {
    ...
    except( "Example", "+++ The condition %s [%s] is not fully contained in iov:%s", c->name.c_str(), i->str().c_str(),
            iov.str().c_str() );
  }
  except( "Example", "+++ The condition %s [%s] has no discrete type matching iov:%s", c->name.c_str(),
          i->str().c_str(), iov.str().c_str() );
}
}
}
}

Test::TestEnv::TestEnv( LCDD& _lcdd, const string& detector_name ) : lcdd( _lcdd ), detector(), manager() {
  ...
  for ( _P::const_iterator i = p.begin(); i != p.end(); ++i, ++cnt ) {
    ...
    if ( pool ) {
      ...
      for ( _E::const_iterator j = e.begin(); j != e.end(); ++j ) {
        ...
        cp->select_all( rc );
        printout( INFO, "Example", "+++ Conditions for pool with IOV: %s age:%d", cp->iov->str().c_str(),
                  cp->age_value );
