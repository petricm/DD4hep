...
int createText( const string& output, const AllConditions& all, char sep ) {
  ...
  ofstream out( output );
  if ( !out.good() ) {
    except( "ConditionsTextRepository", "++ Failed to open output file:%s [errno:%d %s]", output.c_str(), errno,
            ::strerror( errno ) );
  } else if ( sep ) {
    ...
    for ( AllConditions::const_iterator i = all.begin(); i != all.end(); ++i ) {
      ...
      if ( siz_add < siz_a )
        siz_add = siz_a;
      if ( siz_tot < ( siz_n + siz_a ) )
        siz_tot = siz_n + siz_a;
    }
    siz_tot += 8 + 2 + 1;
    ::snprintf( fmt, sizeof( fmt ), "%%08X %%-%lds %%-%lds", long( siz_nam ), long( siz_add ) );
  }
  out << "dd4hep." << char( sep ? sep : '-' ) << "." << long( siz_nam ) << "." << long( siz_add ) << "."
      << long( siz_tot ) << endl;
  for ( AllConditions::const_iterator i = all.begin(); i != all.end(); ++i ) {
    Condition c = ( *i ).second;
...
int readText( const string& input, ConditionsTextRepository::Data& data ) {
  ...
  ifstream                        in( input );
  in >> c >> c >> c >> c >> c >> c >> c >> sep >> c >> siz_nam >> c >> siz_add >> c >> siz_tot;
  text[ 0 ] = 0;
