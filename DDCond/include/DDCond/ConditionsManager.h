...
class ConditionsManager : public Handle<ConditionsManagerObject> {
 ...
 public:
  ...
  template <typename Q>
  ConditionsManager( const Handle<Q>& e ) : Handle<Object>( e ) {
  }
  ...
  /// Prepare all updates for the given keys to the clients with the defined IOV
  long prepare( const IOV& required_validity, const ConditionKeys& keys, dd4hep_ptr<UserPool>& user_pool,
                const Dependencies& dependencies, bool verify_dependencies = true );

  /// Prepare all updates to the clients with the defined IOV
  long prepare( const IOV& required_validity, dd4hep_ptr<UserPool>& user_pool );

  /// Prepare all updates to the clients with the defined IOV
  long prepare( const IOV& required_validity, dd4hep_ptr<UserPool>& user_pool, const Dependencies& dependencies,
