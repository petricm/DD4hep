...
namespace DD4hep {
  ...
  namespace Conditions {
    ...
    public:
      ...
      iterator find(Condition::key_type key) { return dependencies.find(key); }
      /// Mini-container interface: find element by key (const)
      const_iterator find(Condition::key_type key) const { return dependencies.find(key); }
     ...
     *  \version 1.0
     */
    template <typename Data, typename Oper> class DependencyCollector : public ConditionsDependencyCollection::Functor {
    public:
      ...
      void operator()(const Dependencies::value_type& e) const { user_oper(user_data, e); }
    };
