...
namespace DD4hep {
  ...
  public:
     ...
     */
    template <class V1, class V2> void operator()(const V1& v1, const V2& v2, const std::string& name) {
      if (!(v1 == v2)) {
        std::stringstream sstr;
        sstr << "  " << name << " : [" << v1 << "] != [" << v2 << "]";
     ...
     */
    template <class Cond> void operator()(const Cond& c, const std::string& name) {
      if (!(c)) {
        std::stringstream sstr;
        sstr << "  " << name << " : [" << c << "] ";
    ...
    /** Simple log message */
    void log(const std::string& msg) { _out << "[" << _testname << "] " << msg << std::endl; }
    ...
    const char* last_test_status() { return (_last_test_status ? "PASSED" : "FAILED"); }
