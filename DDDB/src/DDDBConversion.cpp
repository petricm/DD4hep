...
#define CHECK_OBJECT( obj )                                                                 \
  if ( !obj ) {                                                                             \
    printout( ERROR, "dddb_print", "++ Attempt to print invalid (null) object of type %s.", \
...
/// Default constructor
Named::Named() : name(), id(), document( 0 ), refCount( 0 ) {
}

/// Initializing constructor
Named::Named( const std::string& c ) : name( c ), id(), document( 0 ), refCount( 0 ) {
}

/// Copy constructor
Named::Named( const Named& c ) : name( c.name ), id( c.id ), document( c.document ), refCount( 0 ) {
  if ( document )
    document->addRef();
}
...
/// Default destructor
Shape::~Shape() {
  if ( type == BooleanUnion::type() || type == BooleanSubtraction::type() || type == BooleanIntersection::type() ) {
...
template <>
void dddb_print( const Isotope* i ) {
  CHECK_OBJECT( i );
  printout( INFO, "Isotope", "++ %-20s A=%6.0f Z=%3.0f Density=%8.2g  id=%s", i->c_name(), i->A, i->Z, i->density,
            i->c_id() );
}

template <>
void dddb_print( const TGeoIsotope* i ) {
  CHECK_OBJECT( i );
  printout( INFO, "TGeoIsotope", "++ %-20s N=%4d Z=%3d A=%8.2g", i->GetName(), i->GetN(), i->GetZ(), i->GetA() );
}

template <>
void dddb_print( const Element* e ) {
  CHECK_OBJECT( e );
  printout( INFO, "Element", "++ %-20s A=%6.0f Z=%3.0f Density=%8g ioni:%8.2g (%d isotopes) id=%s",
            ( e->name + "/" + e->symbol ).c_str(), e->atom.A, e->atom.Zeff, e->density, e->ionization,
            int( e->isotopes.size() ), e->c_id() );
}

template <>
void dddb_print( const TGeoElement* e ) {
  CHECK_OBJECT( e );
  printout( INFO, "TGeoElement", "++ %-20s N=%4d Z=%3d A=%8.2g", e->GetName(), e->N(), e->Z(), e->A() );
}

template <>
void dddb_print( const Material* m ) {
  CHECK_OBJECT( m );
  printout( INFO, "Material", "++ %-20s Density=%8g P=%f T=%f %d components id:%s", ( "'" + m->path + "'" ).c_str(),
            m->density, m->pressure, m->temperature, int( m->components.size() ), m->c_id() );
}

template <>
void dddb_print( const TGeoMedium* m ) {
  CHECK_OBJECT( m );
  TGeoMaterial* mat = m->GetMaterial();
  printout( INFO, "TGeoMedium",
            "++ %-20s Material:%-20s A=%4.0f Z=%3.0f "
            "Radlen:%8.2g IntLen:%8.2g components:%d",
            m->GetName(), mat->GetName(), mat->GetA(), mat->GetZ(), mat->GetRadLen(), mat->GetIntLen(),
            mat->GetNelements() );
}

template <>
void dddb_print( const Shape* s ) {
  CHECK_OBJECT( s );
  if ( s->type == Box::type() ) {
    printout( INFO, "Box", "++ %3d/%-12s: x=%f y=%f z=%f", s->type, s->c_name(), s->s.box.x, s->s.box.y, s->s.box.z );
  } else if ( s->type == Cons::type() ) {
    printout( INFO, "Cons", "++ %3d/%-12s: z=%f innerMZ=%f innerPZ=%f outerMZ=%f outerPZ=%f", s->type, s->c_name(),
              s->s.cons.sizeZ, s->s.cons.innerRadiusMZ, s->s.cons.innerRadiusPZ, s->s.cons.outerRadiusMZ,
              s->s.cons.outerRadiusPZ );
  } else if ( s->type == Tubs::type() ) {
    printout( INFO, "Tubs", "++ %3d/%-12s: z=%f innerR=%f outerR=%f", s->type, s->c_name(), s->s.tubs.sizeZ,
              s->s.tubs.innerRadius, s->s.tubs.outerRadius );
  } else if ( s->type == Polycone::type() ) {
    printout( INFO, "Polycone", "++ %3d/%-12s: %d zplanes", s->type, s->c_name(), int( s->zplanes.size() ) );
  } else if ( s->type == BooleanUnion::type() ) {
    printout( INFO, "union", "++ %3d/%-12s: %d modifications", s->type, s->c_name(), int( s->boolean_ops.size() ) );
  } else if ( s->type == BooleanSubtraction::type() ) {
    printout( INFO, "subtraction", "++ %3d/%-12s: %d modifications", s->type, s->c_name(),
              int( s->boolean_ops.size() ) );
  } else if ( s->type == BooleanIntersection::type() ) {
    printout( INFO, "intersection", "++ %3d%-12s: %d modifications", s->type, s->path.c_str(),
              int( s->boolean_ops.size() ) );
  }
  printout( INFO, "Shape", "++ %-15s  name:%s id:%s", "", s->path.c_str(), s->c_id() );
}

template <>
void dddb_print( const PhysVol* obj ) {
  CHECK_OBJECT( obj );
  printout( INFO, "PhysVol", "++ %-12s: logvol:%s", obj->path.c_str(), obj->logvol.c_str() );
  printout( INFO, "PhysVol", "++ %-12s  name:%s id:%s", "", obj->c_name(), obj->c_id() );
}

template <>
void dddb_print( const LogVol* obj ) {
  CHECK_OBJECT( obj );
  printout( INFO, "LogVol", "++ %-12s: Material:%s Shape:%s", obj->path.c_str(), obj->material.c_str(),
            obj->shape.empty() ? "<INVALID>" : obj->shape.c_str() );
  printout( INFO, "LogVol", "++ %-12s  name:%s id:%s", "", obj->c_name(), obj->c_id() );
}

template <>
void dddb_print( const TabProperty* obj ) {
  CHECK_OBJECT( obj );
  printout( INFO, "Detector", "++ %-12s: [%s] xunit:%s xaxis:%s yunit:%s yaxis:%s siz:%d", obj->path.c_str(),
            obj->type.c_str(), obj->xunit.c_str(), obj->xaxis.c_str(), obj->yunit.c_str(), obj->yaxis.c_str(),
            int( obj->data.size() ) );
}

template <>
void dddb_print( const Catalog* obj ) {
  CHECK_OBJECT( obj );
  printout( INFO, "Detector", "++ %-12s: [%s] %d children support:%s geo:%s", obj->path.c_str(), obj->type.c_str(),
            int( obj->catalogrefs.size() ), obj->support.c_str(), obj->logvol.c_str() );
  printout( INFO, "Detector", "++ %-12s  name:%s id:%s", "", obj->c_name(), obj->c_id() );
}
template <>
void dddb_print( const Document* obj ) {
  CHECK_OBJECT( obj );
