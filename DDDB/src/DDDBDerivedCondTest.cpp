 ...
 public:
  ...
  virtual Condition operator()( const ConditionKey& key, const Context& context ) {
    ...
    if ( par0.typeInfo() == typeid( Delta ) ) {
      ...
      printout( INFO, "ConditionUpdate1", "++ Failed to access Delta from %s", cond0->value.c_str() );
      _Printer()( cond0 );
    }
 ...
 public:
  ...
  virtual Condition operator()( const ConditionKey& key, const Context& context ) {
    ...
    if ( par0.typeInfo() == typeid( Delta ) ) {
      ...
      printout( INFO, "ConditionUpdate2", "++ Failed to access Delta from %s", cond0->value.c_str() );
      _Printer()( cond0 );
    }
    // data.condition   = target;
    return target;
...
class ConditionsSelector {
 public:
  ...
  int computeDependencies( long time ) {
    ...
    printout( INFO, "Conditions", "+++ ConditionsUpdate: Updated %ld conditions... IOV:%s", num_expired,
              iov.str().c_str() );
    user_pool->clear();
    return 1;
  }
