...
class Context {
 private:
  ...
  void collect_id( Q& container, const string& id, T* object ) const {
    ...
    if ( i != container.end() ) {
      if ( object == ( *i ).second )
        return;
      printout( ERROR, "collect", "++ Duplicate ID: %s  %p <-> %p", id.c_str(), object, ( *i ).second );
    }
    object->id      = id;
  ...
  void collect_p( Q& container, const string& path, T* object ) const {
    ...
    if ( i != container.end() ) {
      if ( object == ( *i ).second )
        return;
      printout( ERROR, "collectPath", "++ Duplicate ID: %s  %p <-> %p", path.c_str(), object, ( *i ).second );
    }
 ...
 public:
  ...
  /** Printout helpers                                                                             */
  void print( const Isotope* obj ) const {
    if ( print_materials )
      dddb_print( obj );
  }
  void print( const Element* obj ) const {
    if ( print_materials )
      dddb_print( obj );
  }
  void print( const Material* obj ) const {
    if ( print_materials )
      dddb_print( obj );
  }
  void print( const Shape* obj ) const {
    if ( print_shapes )
      dddb_print( obj );
  }
  void print( const PhysVol* obj ) const {
    if ( print_physvol )
      dddb_print( obj );
  }
  void print( const LogVol* obj ) const {
    if ( print_logvol )
      dddb_print( obj );
  }
  void print( const Catalog* obj ) const {
    if ( print_catalog )
      dddb_print( obj );
  }
  void print( const TabProperty* obj ) const {
    if ( print_tabprop )
      dddb_print( obj );
  }
  void print( const Document* obj ) const {
    if ( print_docs )
      dddb_print( obj );
  }

  /** Data collection helpers for indexing by object identifier                                    */
  void collect( const string& id, Catalog* obj ) {
    collect_id( geo->catalogs, id, obj );
  }
  void collect( const string& id, Shape* obj ) {
    collect_id( geo->shapes, id, obj );
  }
  void collect( const string& id, PhysVol* obj ) {
    collect_id( geo->placements, id, obj );
  }
  void collect( const string& id, LogVol* obj ) {
    collect_id( geo->volumes, id, obj );
  }
  void collect( const string& id, Isotope* obj ) {
    collect_id( geo->isotopes, id, obj );
  }
  void collect( const string& id, Element* obj ) {
    collect_id( geo->elements, id, obj );
  }
  void collect( const string& id, Material* obj ) {
    collect_id( geo->materials, id, obj );
  }
  void collect( const string& id, TabProperty* obj ) {
    collect_id( geo->tabproperties, id, obj );
  }
  void collect( const string& id, Condition& object ) {
    ...
    if ( i != geo->conditions.end() ) {
      if ( object.ptr() == ( *i ).second )
        return;
      printout( ERROR, "collect", "++ Duplicate ID: %s  %p <-> %p", id.c_str(), object.ptr(), ( *i ).second );
    }
    geo->conditions[ id ] = object.ptr();
  }

  /** Data collection helpers for indexing by path                                                 */
  void collectPath( const string& path, Element* obj ) {
    collect_p( geo->elementPaths, path, obj );
  }
  void collectPath( const string& path, Material* obj ) {
    collect_p( geo->materialPaths, path, obj );
  }
  void collectPath( const string& path, PhysVol* obj ) {
    collect_p( geo->placementPaths, path, obj );
  }
  void collectPath( const string& path, LogVol* obj ) {
    collect_p( geo->volumePaths, path, obj );
  }
  void collectPath( const string& path, TabProperty* obj ) {
    collect_p( geo->tabpropertyPaths, path, obj );
  }
  void collectPath( const string& path, Catalog* obj ) {
    collect_p( geo->catalogPaths, path, obj );
  }
  void collectPath( const string& path, Condition& object ) {
    ...
    if ( i != geo->conditionPaths.end() ) {
      if ( object.ptr() == ( *i ).second )
        return;
      printout( ERROR, "collectPath", "++ Duplicate ID: %s  %p <-> %p", path.c_str(), object.ptr(), ( *i ).second );
    }
 ...
 */
template <typename T>
struct Conv : protected Converter<T> {
 public:
 ...
 */
template <typename T>
struct ShapeConv : private Converter<T> {
 public:
  /// Initializing constructor of the functor with initialization of the user parameter
...
template <>
void ShapeConv<Shape>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Assembly>::convert( xml_h, Shape*& s ) const;
template <>
void ShapeConv<Box>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Cons>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<ConeSegment>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Tubs>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Trap>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Polycone>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Polygon>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<EllipticalTube>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<TRD>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<Sphere>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<BooleanShape>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<BooleanUnion>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<BooleanIntersection>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<BooleanSubtraction>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<BooleanSubtraction>::convert( xml_h element, Shape*& s ) const;
template <>
void ShapeConv<BooleanOperation>::convert( xml_h element, Shape*& ptr_shape ) const;

template <>
void Conv<ZPlane>::convert( xml_h element ) const;
template <>
void Conv<dddb>::convert( xml_h element ) const;
template <>
void Conv<LogVol>::convert( xml_h element ) const;
template <>
void Conv<LogVolRef>::convert( xml_h element ) const;
template <>
void Conv<PhysVol>::convert( xml_h element ) const;
template <>
void Conv<ParamPhysVol>::convert( xml_h element ) const;
template <>
void Conv<ParamPhysVol2D>::convert( xml_h element ) const;
template <>
void Conv<ParamPhysVol3D>::convert( xml_h element ) const;
template <>
void Conv<ConditionParam>::convert( xml_h element ) const;
template <>
void Conv<AlignmentDelta>::convert( xml_h element ) const;
...
void print_ref( const char* desc, Context* context, xml_h element, const string& ref, const string& opt = "" ) {
  ...
  printout( INFO, desc, "** %s --> %s  path: %s # %s  %s", id1.c_str(), id2.c_str(), path.c_str(), obj.c_str(),
            opt.c_str() );
}

template <typename ACTION = dddb>
void load_dddb_entity( Context* context, Catalog* catalog, xml_h element, const string& ref, bool print = false );

/// Helper to locate objects in a map using string identifiers
template <typename Q>
bool _find( const string& id, const Q& container ) {
...
bool checkParents( Context* context, Catalog* det ) {
  ...
  if ( det == geo->top ) {
    ...
    printout( ERROR, "checkParents", "++ %s: NO PARENT Detector: '%s'!", det->path.c_str(), parent_id.c_str() );
  }
  return false;
}
...
static void insert_map_key( const string& key_val, const string& val_type, const string& val, OpaqueDataBlock& block ) {
  KEY key;
  ...
  if ( val_type.substr( 0, 4 ) == "char" )
    ...
    printout( INFO, "Param", "++ Unknown conditions parameter type:%s data:%s", val_type.c_str(), val.c_str() );
    insert_map_item<KEY, string>( key, val, block );
  }
}
...
/// Specialized conversion of <param/> and <paramVector> entities
template <>
void Conv<ConditionParamVector>::convert( xml_h element ) const {
  string nam  = element.attr<string>( _U( name ) );
  string typ  = element.hasAttr( _U( type ) ) ? element.attr<string>( _U( type ) ) : string( "int" );
  string data = element.text();
  pair<string, OpaqueDataBlock> block;
...
template <>
void Conv<ConditionParamMap>::convert( xml_h element ) const {
  string nam = element.attr<string>( _U( name ) );
  pair<string, OpaqueDataBlock> block;
  block.first = nam;
...
template <>
void Conv<ConditionParamSpecific>::convert( xml_h element ) const {
...
void Conv<AlignmentDelta>::convert( xml_h element ) const {
  ...
  if ( !g.fromString( &pos, d ) )
    g.invalidConversion( d, g.type() );
  if ( nam == "dPosXYZ" ) {
    a->translation = pos;
...
template <>
void Conv<Condition>::convert( xml_h element ) const {
  ...
  if ( !_find( id, context->geo->conditions ) ) {
    ...
    for ( xml_coll_t iter( element, _U( star ) ); iter; ++iter ) {
      ...
      printout( INFO, "Condition", "++ Unknown conditions tag:%s obj:%s id:%s", tag.c_str(), path.c_str(), id.c_str() );
    }
    ...
    if ( ( context->geo->conditions.size() % 500 ) == 0 ) {
      printout( INFO, "Condition",
                "++ Processed %d conditions....last:%s Number of Params: %d Vec:%d Map:%d Spec:%d Align:%d",
...
template <>
void Conv<ConditionRef>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  Catalog* catalog = _option<Catalog>();
  ...
  if ( i == context->geo->conditions.end() ) {
    printout( ERROR, "ConditionRef", "++  MISSING ID: %s Failed to convert ref:%s cat:%s", refid.c_str(), path.c_str(),
              catalog ? catalog->path.c_str() : "???" );
...
template <>
void Conv<Author>::convert( xml_h element ) const {
  string* context = _option<string>();
  if ( element.hasAttr( _U( author ) ) )
    *context = element.attr<string>( _U( author ) );
}

/// Specialized conversion of <version/> entities
template <>
void Conv<Version>::convert( xml_h element ) const {
  string* context = _option<string>();
  if ( element.hasAttr( _U( version ) ) )
    *context = element.attr<string>( _U( version ) );
}

/// Specialized conversion of <param/> entities
template <>
void Conv<Param>::convert( xml_h element ) const {
...
template <>
void Conv<ConditionInfo>::convert( xml_h element ) const {
  Catalog* det               = _option<Catalog>();
  string   name              = element.attr<string>( _U( name ) );
...
template <>
void Conv<Isotope>::convert( xml_h element ) const {
  Context*   context = _param<Context>();
...
template <>
void Conv<ElementRef>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  Catalog* catalog = _option<Catalog>();
  string   href    = element.attr<string>( _LBU( href ) );
  string   refid   = reference_href( element, href );
...
template <>
void Conv<Element>::convert( xml_h element ) const {
  Context*   context = _param<Context>();
  ...
  if ( !_find( id, context->geo->elements ) ) {
    ...
    if ( st == "solid" )
      e->state = Element::SOLID;
    else if ( st == "liquid" )
      e->state = Element::LIQUID;
    else if ( st == "gas" )
      e->state = Element::GAS;
    else
      e->state = Element::UNKNOWN;
...
template <>
void Conv<MaterialComponent>::convert( xml_h element ) const {
...
template <>
void Conv<MaterialRef>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  Catalog* catalog = _option<Catalog>();
  string   href    = element.attr<string>( _LBU( href ) );
  string   refid   = reference_href( element, href );
...
template <>
void Conv<Material>::convert( xml_h element ) const {
  Context*   context = _param<Context>();
  Catalog*   catalog = _option<Catalog>();
  dddb_dim_t x_mat   = element;
  ...
  if ( !_find( id, context->geo->materials ) ) {
    ...
    if ( catalog )
      context->collectPath( m->path, m );
  }
}

/// Specialized conversion of <geometryinfo/> entities
template <>
void Conv<GeometryInfo>::convert( xml_h element ) const {
  Catalog* context = _option<Catalog>();
  context->logvol  = element.attr<string>( _LBU( lvname ) );
  if ( element.hasAttr( _LBU( npath ) ) )
    context->npath = element.attr<string>( _LBU( npath ) );
  if ( element.hasAttr( _LBU( support ) ) )
    context->support = element.attr<string>( _LBU( support ) );
  if ( element.hasAttr( _LBU( condition ) ) )
    context->condition = element.attr<string>( _LBU( condition ) );
}

/// Specialized conversion of <posXYZ/> entities
template <>
void Conv<Position>::convert( xml_h element ) const {
...
template <>
void Conv<RotationZYX>::convert( xml_h element ) const {
  dddb_dim_t   dim = element;
...
template <>
void Conv<Transform3D>::convert( xml_h element ) const {
...
template <>
void ShapeConv<BooleanOperation>::convert( xml_h element, Shape*& s ) const {
  xml_h    elt     = element;
    ...
    if ( ( elt = element.child( _U( box ), false ) ) )
      ...
      for ( xml_coll_t p( element, _U( star ) ); p; ++p ) {
        if ( p.parent() == element ) {
          ...
          if ( tag == "paramphysvol" )
            continue;
          if ( tag == "paramphysvol2D" )
            continue;
          if ( tag == "paramphysvol3D" )
            continue;
...
template <>
void Conv<PhysVol>::fill( xml_h element, PhysVol* pv ) const {
  xml_h      elt;
...
/// Specialized conversion of EllipticalTube shapes
template <>
void Conv<ParamPhysVol>::fill( xml_h element, ParamPhysVol* pv ) const {
...
/// Specialized conversion of EllipticalTube shapes
template <>
void Conv<ParamPhysVol2D>::fill( xml_h element, ParamPhysVol2D* pv ) const {
  dddb_dim_t elt = element;
...
/// Specialized conversion of EllipticalTube shapes
template <>
void Conv<ParamPhysVol3D>::fill( xml_h element, ParamPhysVol3D* pv ) const {
  dddb_dim_t elt = element;
...
/// Specialized conversion of <geometryinfo/> entities
template <>
void Conv<TabProperty>::convert( xml_h element ) const {
  Context*   context = _param<Context>();
  ...
  if ( !_find( id, context->geo->tabproperties ) ) {
    ...
    if ( element.hasAttr( _U( type ) ) )
      p->type = element.attr<string>( _U( type ) );
    if ( element.hasAttr( _LBU( xunit ) ) )
      p->xunit = element.attr<string>( _LBU( xunit ) );
    if ( element.hasAttr( _LBU( yunit ) ) )
      p->yunit = element.attr<string>( _LBU( yunit ) );
    if ( element.hasAttr( _LBU( xaxis ) ) )
      p->xaxis = element.attr<string>( _LBU( xaxis ) );
    if ( element.hasAttr( _LBU( yaxis ) ) )
      p->yaxis = element.attr<string>( _LBU( yaxis ) );
...
template <>
void Conv<TabPropertyRef>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  Catalog* catalog = _option<Catalog>();
  string   href    = element.attr<string>( _LBU( href ) );
  string   refid   = reference_href( element, href );
...
template <>
void Conv<Parameter>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  string   name    = element.attr<string>( _U( name ) );
...
template <>
void Conv<DetElemRef>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  Catalog* catalog = _option<Catalog>();
  string   href    = element.attr<string>( _LBU( href ) );
  string   refid   = reference_href( element, href );
...
template <>
void Conv<DetElem>::convert( xml_h element ) const {
  Context*   context = _param<Context>();
  dddb*      geo     = context->geo;
  ...
  if ( idet == geo->catalogs.end() ) {
    ...
    if ( context->print_detelem ) {
      printout( INFO, "DetElem", "  xml:%s id=%s  [%s/%s] doc:%s obj:%s / %s", element.parent().tag().c_str(),
                det->id.c_str(), det->name.c_str(), det->type.c_str(), context->locals.xml_doc->id.c_str(),
                context->locals.obj_path.c_str(), det->name.c_str() );
    }
...
template <>
void Conv<CatalogRef>::convert( xml_h element ) const {
  Context* context = _param<Context>();
  Catalog* catalog = _option<Catalog>();
  string   href    = element.attr<string>( _LBU( href ) );
  string   refid   = reference_href( element, href );
...
template <>
void Conv<Catalog>::convert( xml_h e ) const {
  Context*                       context = _param<Context>();
...
template <>
void Conv<dddb>::convert( xml_h e ) const {
  Catalog* catalog = 0;
  Context* context = _param<Context>();
  xml_coll_t( e, _U( parameter ) ).for_each( Conv<Parameter>( lcdd, context, catalog ) );
  xml_coll_t( e, _U( isotope ) ).for_each( Conv<Isotope>( lcdd, context, catalog ) );
...
/// Specialized conversion of <DDDB/> entities
template <>
void Conv<dddb_conditions>::convert( xml_h e ) const {
  Catalog* catalog = 0;
  Context* context = _param<Context>();
  context->check   = false;
  xml_coll_t( e, _LBU( conditionref ) ).for_each( Conv<ConditionRef>( lcdd, context, catalog ) );
...
void load_dddb_entity( Context* context, Catalog* catalog, xml_h element, const string& ref, bool prt ) {
  size_t hash = ref.find( "#" );
  if ( hash != 0 ) {
    try {
      ...
      hash       = doc_path.find( '#' );
      if ( hash != string::npos )
        doc_path = doc_path.substr( 0, hash );
#if 0
      ...
      if ( e ) {
        if ( !key.empty() ) {
          ...
          doc.assign( xml_handler_t().parse( buffer.c_str(), buffer.length(), doc_path.c_str(), &reader ) );
          e = doc.root();
        }
        Context::PreservedLocals locals( context );
...
void config_context( Context& context, XML::UriReader* rdr, const std::string& doc_path, const std::string& obj_path ) {
  DDDBReaderContext* ctx   = (DDDBReaderContext*)rdr->context();
