...
long dump_detelement_tree( LCDD& lcdd, int argc, char** argv ) {
  ...
  struct Actor {
      ...
      if ( flag >= 3 ) {
        ...
        for ( RangeConditions::const_iterator ic = rc.begin(); ic != rc.end(); ++ic ) {
          ...
          if ( !m_iov.iovType )
            ...
            m_iov.iov_intersection( cond.iov() );
        }
    ...
    /// __________________________________________________________________________________
    void printDetElement( int level, DetElement de, ConditionPrinter& prt, bool with_placement = false,
                          bool with_keys = false, bool with_values = false ) {
    ...
    void printConditionInfo( int level, Catalog* cat, ConditionPrinter& prt, bool with_elements = false ) {
      if ( cat && !cat->conditioninfo.empty() ) {
        ...
        for ( DDDB::Named::StringMap::const_iterator i = cat->conditioninfo.begin(); i != cat->conditioninfo.end();
              ++i ) {
          const string& cond_name = ( *i ).second;
