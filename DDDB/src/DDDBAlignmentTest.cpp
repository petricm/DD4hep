...
class AlignmentSelector {
 public:
  ...
  int computeDependencies( dd4hep_ptr<UserPool>& user_pool, ConditionsManager conds, AlignmentsManager align,
                           long time ) {
        ...
        if ( d->detector.hasAlignments() ) {
          ...
          Alignments::Container c = det.alignments();
          {
            ...
            printout( INFO, "Alignment", "++ [%08X] (%11s-%8s-%5s) Cond:%p '%s'", k.hash,
                      ...
                      D.hasPivot() ? "Pivot" : "", a.data().hasCondition() ? a.data().condition.ptr() : 0,
                      k.name.c_str() );
            ++count;
          }
          {
