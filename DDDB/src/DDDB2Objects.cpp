...
struct Context {
  ...
  template <typename T>
  void collect( const string& id, T* s );
  template <typename T, typename Q>
  void collect( const string& id, T* s, Q* c );
  LCDD&             lcdd;
  DDDB::dddb*       geo;
...
struct CNV : Converter<T, T*> {
 public:
  ...
  template <typename Q>
  CNV<Q> cnv() const {
    return CNV<Q>( this->lcdd, this->param, this->optional );
  }
  void* convert( T* obj ) const;
  template <typename Q>
  Q get( const string& obj ) const {
...
namespace {
template <>
void* CNV<GeoCondition>::convert( GeoCondition* object ) const;
template <>
void* CNV<Isotope>::convert( Isotope* object ) const;
template <>
void* CNV<Element>::convert( Element* object ) const;
template <>
void* CNV<Material>::convert( Material* object ) const;
template <>
void* CNV<BooleanOperation>::convert( BooleanOperation* object ) const;
template <>
void* CNV<Shape>::convert( Shape* object ) const;
template <>
void* CNV<DetElem>::convert( DetElem* object ) const;
template <>
void* CNV<LogVol>::convert( LogVol* object ) const;
template <>
void* CNV<Catalog>::convert( Catalog* object ) const;
template <>
void* CNV<dddb>::convert( dddb* obj ) const;
...
template <>
template <>
LogVol* CNV<LogVol>::get<LogVol*>( const string& obj ) const;
template <>
template <>
GeoVolume CNV<LogVol>::get<GeoVolume>( const string& obj ) const;

/// Convert single condition objects
template <>
void* CNV<GeoCondition>::convert( GeoCondition* obj ) const {
...
TGeoElement* createElement( const char* nam, Context* context, Element* o ) {
  ...
  if ( !e ) {
    ...
    for ( auto i = o->isotopes.begin(); i != o->isotopes.end(); ++i ) {
      ...
      TGeoIsotope* geo_iso = (TGeoIsotope*)CNV<Isotope>( context->lcdd, context ).convert( isotope );
      if ( !geo_iso ) {
        /// Error!
      }
...
void* CNV<Shape>::convert( Shape* object ) const {
  ...
  if ( !shape ) {
    if ( object->id == "/dd/Geometry/LHCb/lvLHCb" ) {
      ...
      shape = Geometry::Cone( c.sizeZ, c.innerRadiusMZ, c.outerRadiusMZ, c.innerRadiusPZ, c.outerRadiusPZ ).ptr();
    } else if ( object->type == ConeSegment::type() ) {
      const ConeSegment& o = object->s.coneSegment;
      shape = Geometry::ConeSegment( o.sizeZ, o.innerRadiusMZ, o.outerRadiusMZ, o.innerRadiusPZ, o.outerRadiusPZ,
                                     o.start, o.delta )
                  .ptr();
    ...
    //}
      ...
      shape = Geometry::Trap( o.dz, o.theta, o.phi, o.h1, o.bl1, o.tl1, o.alpha1, o.h2, o.bl2, o.tl2, o.alpha2 );
    } else if ( object->type == BooleanUnion::type() || object->type == BooleanSubtraction::type() ||
      ...
      if ( !left_solid.isValid() ) {  // Error ....
        except( "Cnv<Shape>", "++ %s: Unknown left boolean shape creation:%s -> %d", object->c_name(),
                left_shape->c_name(), left_shape->type );
      }
      shape = left_solid;
      for ( i = object->boolean_ops.begin(); i != object->boolean_ops.end(); ++i ) {
        ...
        if ( !right_solid.isValid() ) {  // Error ....
          except( "Cnv<Shape>", "++ %s: Unknown right boolean shape creation:%s -> %d", object->c_name(),
                  right_shape->c_name(), right_shape->type );
        }
        shape = GeoSolid();
        ...
        if ( !shape.isValid() ) {
          except( "Cnv<Shape>", "++ %s: Unknown boolean shape creation:%d", object->c_name(), object->type );
        }
        left_solid = shape;
      }
    }
    if ( !shape ) {
      except( "Cnv<Shape>", "++ Undefined shape conversion %s [id:%d]", object->c_id(), object->type );
    }
...
void* CNV<LogVol>::convert( LogVol* object ) const {
  ...
  if ( !mother.isValid() ) {
    if ( depth.counter() >= context->max_volume_depth ) {
      ...
      printout( WARNING, "Cnv<LogVol>", "++ Ignore placements below level:%d -> %s", depth.counter(),
                object->path.c_str() );
      return mother;
    }
    ...
    for ( auto i = object->physvols.begin(); i != object->physvols.end(); ++i ) {
      ...
      if ( !place.isValid() ) {
        ...
        if ( !daughter.isValid() ) {
          printout( WARNING, "Cnv<PhysVol>",
                    "++ Failed to convert placement: %s."
                    " Unknown daughter vol:%s.",
                    pv->c_id(), pv->logvol.c_str() );
          continue;
        }
        int         num_places = 0;
        ...
        if ( context->print_physvol ) {
          ...
          printout( INFO, "Cnv<PhysVol>", "++ Converted physVol: depth:%d typ:%d places:%d [%p lv:%p] %s",
                    ...
                    mother->GetTitle() );
          printout( INFO, "Cnv<PhysVol>", "++ \tPosition: x=%f y=%f z=%f %p -> %s", pos.X(), pos.Y(), pos.Z(),
                    (void*)pv, pv->c_name() );
        }
      }
    }
    if ( context->print_logvol ) {
      printout( INFO, "Cnv<LogVol>", "++ Converted   logVol: [%p -> %p] %s NDau:%d", (void*)object, (void*)mother.ptr(),
                object->path.c_str(), mother->GetNdaughters() );
    }
  }
  return mother.ptr();
}
...
LogVol* CNV<LogVol>::get<LogVol*>( const string& nam ) const {
  ...
  if ( cat.first ) {
    ...
    for ( iv = c->logvols.begin(); iv != c->logvols.end(); ++iv )
      printout( WARNING, "Cnv<LogVol>", "++ %s --volume--> %s", c->id.c_str(), ( *iv ).second->name.c_str() );
  }
...
void* CNV<Catalog>::convert( Catalog* object ) const {
  ...
  if ( context->print_detelem ) {
    ...
    printout( INFO, "CNV<Catalog>", "++ Starting catalog %p %s [cref:%d/%d lref:%d/%d lv:%s sup:%s np:%s] Cond:%s ",
              (void*)object, object->path.c_str(), int( object->catalogrefs.size() ), int( object->catalogs.size() ),
              int( object->logvolrefs.size() ), int( object->logvols.size() ),
              object->logvol.empty() ? "--" : object->logvol.c_str(),
              object->support.empty() ? "--" : object->support.c_str(),
              object->npath.empty() ? "--" : object->npath.c_str(), object->condition.c_str() );
  }
    ...
    if ( !object->logvol.empty() ) {
      ...
      if ( !object->npath.empty() ) {
        ...
        if ( context->volumePlaced( par_vol, vol ) ) {
          printout( ERROR, "CNV<DetElem>", "++ %s : Volume already placed %s -> %s", det.path().c_str(),
                    par_vol->GetTitle(), vol->GetTitle() );
        }
  ...
  if ( !object->conditioninfo.empty() ) {
    Conditions::Container::Object* conditions = Conditions::DetConditions( det ).conditions().ptr();
    for ( Catalog::StringMap::const_iterator i = object->conditioninfo.begin(); i != object->conditioninfo.end();
          ++i ) {
  ...
  if ( context->print_detelem ) {
    printout( INFO, "CNV<Catalog>",
              "++ Converting catalog %p -> %p [cref:%d/%d lref:%d/%d lv:%s [%p] sup:%s np:%s] %s ", (void*)object,
              det.ptr(), int( object->catalogrefs.size() ), int( object->catalogs.size() ),
              int( object->logvolrefs.size() ), int( object->logvols.size() ),
              object->logvol.empty() ? "--" : object->logvol.c_str(), vol.ptr(),
              object->support.empty() ? "--" : object->support.c_str(),
              object->npath.empty() ? "--" : object->npath.c_str(), object->path.c_str() );
    int cnt;
    cnt = 0;
    for ( Catalog::CatRefs::const_iterator i = object->catalogrefs.begin(); i != object->catalogrefs.end();
          ++i, ++cnt ) {
      const Catalog::CatRefs::value_type& v = *i;
      if ( v.second )
        printout( INFO, "CNV<DE>:cref", "++ DE:%s ref[%2d]: %p -> %s", object->path.c_str(), cnt, v.second,
                  v.second->c_name() );
      else
    ...
    for ( Catalog::LvRefs::const_iterator i = object->logvolrefs.begin(); i != object->logvolrefs.end(); ++i, ++cnt ) {
      ...
      if ( lv )
        printout( INFO, "CNV<DE>:lref", "++ DE:%s ref[%2d]: %p / %s  -> %p [%s]", object->path.c_str(), cnt, (void*)lv,
                  lv->c_name(), (void*)geoVol.ptr(), geoVol.isValid() ? geoVol->GetName() : "????" );
      else
    ...
    for ( Catalog::LvRefs::const_iterator i = object->logvols.begin(); i != object->logvols.end(); ++i, ++cnt ) {
      ...
      if ( lv ) {
        GeoVolume geoVol = Context::find( context->volumes, lv );
        printout( INFO, "CNV<DE>:lvol", "++ DE:%s ref[%2d]: %p / %s  -> %p [%s]", object->path.c_str(), cnt, (void*)lv,
                  lv->c_id(), (void*)geoVol.ptr(), geoVol.isValid() ? geoVol->GetName() : "????" );
      } else
    ...
    if ( vol && !object->npath.empty() ) {
      for ( int i = 0; i < vol->GetNdaughters(); ++i ) {
        TGeoNode* dau = vol->GetNode( i );
        printout( INFO, "CNV<DE>:npath", "++ DE:%s npath:%s Dau[%2d]: %s", object->path.c_str(), object->npath.c_str(),
                  i, dau->GetName() );
      }
    }
  }
...
long dddb_2_dd4hep( LCDD& lcdd, int, char** ) {
  ...
  if ( helper ) {
    ...
    printout( INFO, "DDDB", "++ Converted %8d conditions.", context.geo ? int( context.geo->conditions.size() ) : 0 );
