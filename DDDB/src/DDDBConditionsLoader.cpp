...
namespace {
enum CMD { INSERT = 1, REPLACE = 2, NONE };
struct CallArgs {
  CMD                  cmd;
  Condition::key_type  key;
  Condition::iov_type& iov;
  RC&                  rc;
  CallArgs( CMD c, Condition::key_type k, Condition::iov_type& i, RC& r ) : cmd( c ), key( k ), iov( i ), rc( r ) {
  }
};
...
/// Load single conditions document
void DDDBConditionsLoader::loadDocument( XML::UriContextReader& rdr, const string& sys_id, const string& obj_id ) {
...
size_t DDDBConditionsLoader::load_range( key_type key, const iov_type& req_validity, RangeConditions& conditions ) {
...
size_t DDDBConditionsLoader::load( key_type key, const iov_type& req_validity, RangeConditions& conditions ) {
  KeyMap::const_iterator k = m_keys.keys.find( key );
  if ( k != m_keys.keys.end() ) {
    size_t              len = conditions.size();
    DDDBReaderContext   local;
...
size_t DDDBConditionsLoader::update( const iov_type& req_validity, RangeConditions& conditions,
...
void DDDBConditionsLoader::onRegisterCondition( Condition cond, void* param ) {
  ...
  if ( param ) {
    ...
    if ( arg->cmd == REPLACE ) {
      ...
      if ( i != r.end() ) {
        ( *i ) = cond;
        printout( DEBUG, "DDDB", "++ Got  MATCH: %-40s [%08X] --> %s.", c->name.c_str(), c->hash, c->address.c_str() );
        arg->iov.iov_intersection( cond.iov() );
        return;
      }
      printout( INFO, "DDDB", "++ Got update: %-40s [%08X] --> %s.", c->name.c_str(), c->hash, c->address.c_str() );
