...
DDDBReader::DDDBReader( const std::string& dir ) : m_directory( dir ), m_match( "conddb:" ) {
...
/// Resolve a given URI to a string containing the data
bool DDDBReader::load( const string& system_id, UserContext* ctxt, string& buffer ) {
  if ( system_id.substr( 0, m_match.length() ) == m_match ) {
    ...
    if ( ret == 1 )
      return true;
    printout( ERROR, "DDDBReader", "++ Failed to resolve system id: %s [%s]", id.c_str(), ::strerror( errno ) );
  }
  return false;
}
...
int DDDBReader::getObject( const string& sys_id, UserContext* /* ctxt */, string& /* out */ ) {
          ...
          "DDDBReader::getObject is a virtual method, "
          "which must be overloaded by the concrete instance!! sys_id:%s",
          sys_id.c_str() );
  return 0;
}
