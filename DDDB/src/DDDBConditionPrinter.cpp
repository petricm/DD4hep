...
void ConditionPrinter::ParamPrinter::operator()( const AbstractMap::Params::value_type& obj ) const {
  ...
  if ( type == typeid( string ) ) {
    ...
    printout( INFO, "Condition", "++ %s\t-> Param: %-16s %-8s -> %s", m_prefix.c_str(), obj.first.c_str(),
              obj.second.dataType().c_str(), value.c_str() );
  } else if ( type == typeid( AbstractMap ) ) {
    const AbstractMap& d = obj.second.get<AbstractMap>();
    printout( INFO, "Condition", "++ %s\t-> [%s] CL:%d %-8s -> %s", m_prefix.c_str(), obj.first.c_str(), d.classID,
    ...
    printout( INFO, "Condition", "++ %s\t-> [%s] %-8s -> %s", m_prefix.c_str(), obj.first.c_str(),
              obj.second.dataType().c_str(), value.c_str() );
  }
}
...
int ConditionPrinter::operator()( Condition cond ) {
  if ( cond.isValid() ) {
    ...
    printout( INFO, "Condition", "++ %s Path:%s Class:%d [%s]", m_prefix.c_str(), cond.name().c_str(), data.classID,
