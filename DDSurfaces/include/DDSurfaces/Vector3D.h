...
namespace DDSurfaces {
  ...
  public:
    ...
    template <class T> Vector3D(double x, double y, double z, T (&)());
    ...
    inline double phi() const { return _x == 0.0 && _y == 0.0 ? 0.0 : atan2(_y, _x); }

    /** Transversal component - cylindrical 'r' */
    inline double rho() const { return trans(); }

    /** Transversal component */
    inline double trans() const { return sqrt(_x * _x + _y * _y); }

    /** Transversal component squared */
    inline double trans2() const { return _x * _x + _y * _y; }

    /** Spherical r/magnitude */
    inline double r() const { return sqrt(_x * _x + _y * _y + _z * _z); }

    /** Spherical r/magnitude, squared */
    inline double r2() const { return _x * _x + _y * _y + _z * _z; }

    /** Polar angle - spherical */
    inline double theta() const { return _x == 0.0 && _y == 0.0 && _z == 0.0 ? 0.0 : atan2(rho(), _z); }

    /** Scalar product */
    inline double dot(const Vector3D& v) const { return _x * v.x() + _y * v.y() + _z * v.z(); }
     ...
     *  @see operator T()
     */
    template <class T> inline T to() const { return T(_x, _y, _z); }
   ...
   */
  template <> inline Vector3D::Vector3D(double x, double y, double z, Vector3D::Cartesian (&)()) : _x(x), _y(y), _z(z) {}

  /** Cylindrical c'tor  - example: <br> 
   *  Vector3D  v( rho, phi, z , Vector3D::cylindrical ) ;
   */
  template <> inline Vector3D::Vector3D(double rho, double phi, double z, Vector3D::Cylindrical (&)()) : _z(z) {
    _x = rho * cos(phi);
    _y = rho * sin(phi);
  }
   ...
   */
  template <> inline Vector3D::Vector3D(double r, double phi, double theta, Vector3D::Spherical (&)()) {
