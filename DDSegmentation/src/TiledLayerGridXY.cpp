...
TiledLayerGridXY::TiledLayerGridXY( const std::string& cellEncoding ) : CartesianGrid( cellEncoding ) {
...
Vector3D TiledLayerGridXY::position( const CellID& cID ) const {
  ...
  if ( _layerOffsetX.size() != 0 && _layerIndex <= _layerOffsetX.size() ) {
    cellPosition.X =
        binToPosition( ( *_decoder )[ _xId ].value(), _gridSizeX, _layerOffsetX[ _layerIndex - 1 ] * _gridSizeX / 2. );
    ...
    if ( ( _layerDimX.size() != 0 && _layerIndex <= _layerDimX.size() ) &&
         ...
         ( std::fabs( cellPosition.X ) >
           ( _layerDimX.at( _layerIndex - 1 ) - _fractCellSizeXPerLayer.at( _layerIndex - 1 ) ) ) ) {
...
CellID TiledLayerGridXY::cellID( const Vector3D& localPosition, const Vector3D& /* globalPosition */,
  ...
  if ( _layerOffsetX.size() != 0 && _layerIndex <= _layerOffsetX.size() ) {
    ( *_decoder )[ _xId ] =
        positionToBin( localPosition.X, _gridSizeX, _layerOffsetX[ _layerIndex - 1 ] * _gridSizeX / 2. );
  } else {
