...
MegatileLayerGridXY::MegatileLayerGridXY( const std::string& cellEncoding ) : CartesianGrid( cellEncoding ) {
  // define type and description
  _type = "MegatileLayerGridXY";
  // _description = "Cartesian segmentation in the local XY-plane for both Normal wafer and Magic wafer(depending on the
  // layer dimensions)";
  _description =
      "Cartesian segmentation in the local XY-plane: megatiles with dead areas; integer number of megatiles and cells";

  //////// setup in the compact-steering file (i.e., ILD_o3_v05.xml)////
  // for example,
  // <segmentation type="MegatileLayerGridXY" yer_nCells="36" layer_nStripsX="2" layer_nStripsY="36" deadWidth="0"
  // layer_configuration="TL"/>
  ////
  //
  ...
  registerParameter( "layer_nCells", "division of megatile into square tiles", _nCells, 36,
                     SegmentationParameter::NoUnit, true );
  registerParameter( "layer_nStripsX", "division of megatile into strips (1)", _nStripsX, 4,
                     SegmentationParameter::NoUnit, true );
  registerParameter( "layer_nStripsY", "division of megatile into strips (2)", _nStripsY, 36,
                     SegmentationParameter::NoUnit, true );

  registerParameter( "deadWidth", "width of dead region at edge of megatile", _deadWidth, 0.,
  ...
  registerParameter( "identifier_module", "Cell encoding identifier for module", _identifierModule,
                     std::string( "module" ), SegmentationParameter::NoUnit, true );

  registerParameter( "layer_configuration", "layer configuration (S, T, L)", _layerConfig, std::string( "TLS" ),
                     SegmentationParameter::NoUnit, true );
...
/// default constructor using an encoding string
MegatileLayerGridXY::MegatileLayerGridXY( BitField64* decode ) : CartesianGrid( decode ) {
  // define type and description
  _type = "MegatileLayerGridXY";
  // _description = "Cartesian segmentation in the local XY-plane for both Normal wafer and Magic wafer(depending on the
  // layer dimensions)";
...
void MegatileLayerGridXY::calculateDerivedQuantities() const {
  ...
  std::cout << "calculateDerivedQuantities: total X width: " << _totalSizeX << " / nMega: " << _nMegaY << " = "
            << _megaSize << " ; dead width = " << _deadWidth << " ; nstripsX,Y, nCells : " << _nStripsX << " "
            << _nStripsY << " " << _nCells << " ; stripX,Y/cell size: " << _gridSizeL << " " << _gridSizeT << " "
            << _gridSizeS << std::endl;
...
Vector3D MegatileLayerGridXY::position( const CellID& cID ) const {
...
#if VERBOSEcoterra
  if ( cellPosition.X > 100000. )
    std::cout << "HERE too large cellPosition.X place 1" << std::endl;
...
#if VERBOSEcoterra
    std::cout << "This hit is out of regulation EBU=> merged into neighbour strip; XfromEdge = " << XfromEdge
              << std::endl;
#endif
  } else {
  ...
  if ( cellPosition.X > 100000. )
    std::cout << "HERE too large cellPosition.X place 2" << std::endl;
#endif

#if VERBOSEcoterra
  if ( cellPosition.X > 50000 || cellPosition.Y > 50000 || cellPosition.Z > 50000 || cellPosition.X < -50000 ||
       cellPosition.Y < -50000 || cellPosition.Z < -50000 ) {
    ...
    std::cout << "CellIndexX * celldims[0] = " << CellIndexX << " x " << celldims[ 0 ]
              << ", CellIndexY * celldims[1] = " << CellIndexY << " x " << celldims[ 1 ] << ", _isRegulatingEBU["
              << LayerIndex << "][" << WaferIndex << "] = " << _isRegulatingEBU[ LayerIndex ][ WaferIndex ]
              << std::endl;
...
CellID MegatileLayerGridXY::cellID( const Vector3D& localPosition, const Vector3D& /* globalPosition */,
  ...
  std::cout << "_x = localPosition.X: " << _x << " = " << localPosition.X << ", _waferOffsetX[" << layerIndex << "]["
            << waferIndex << "] = " << _waferOffsetX[ layerIndex ][ waferIndex ] << std::endl;
  std::cout << "_y = localPosition.Y: " << _y << " = " << localPosition.Y << ", _waferOffsetY[" << layerIndex << "]["
            << waferIndex << "] = " << _waferOffsetY[ layerIndex ][ waferIndex ] << std::endl;
...
#if VERBOSEcoterra
  std::cout << "WaferOffsetX = " << _waferOffsetX[ layerIndex ][ waferIndex ] << " layer, wafer = " << layerIndex
            << ", " << waferIndex << std::endl;
#endif
  // _waferOffsetX has negative sign. <<<---??? not true!!
  if ( _x < -WaferOffsetX || _x > WaferOffsetX || _y < -WaferOffsetY || _y > WaferOffsetY ) {
#if VERBOSEcoterra
    ...
    std::cout << "layerIndex = " << layerIndex << ", waferIndex = " << waferIndex << ", _isRegulatingEBU[" << layerIndex
              << "][" << waferIndex << "] = " << _isRegulatingEBU[ layerIndex ][ waferIndex ] << std::endl;
#endif
...
#if VERBOSEcoterra
    if ( _isRegulatingEBU[ layerIndex ][ waferIndex ] ) {
      if ( localPosition.X < -10000 || localPosition.X > 10000 || localPosition.Y < -10000 ||
           localPosition.Y > 10000 ) {
        std::cout << "in cellID on hit " << std::endl;
        std::cout << "_x = localPosition.X + _waferOffsetX; => " << _x << " = " << localPosition.X << " + "
                  << WaferOffsetX << std::endl;
        std::cout << "_y = localPosition.Y + _waferOffsetY; => " << _y << " = " << localPosition.Y << " + "
                  << WaferOffsetY << std::endl;
        std::cout << "_megaSize = " << _megaSize << std::endl;
      }
    }
#endif
    ...
    if ( _isRegulatingEBU[ layerIndex ][ waferIndex ] && celldims[ 0 ] < celldims[ 1 ] * 1.5 ) {
      ...
      std::cout << "EXESS hit in transeverse layer WaferOffsetX = " << WaferOffsetX
                << ", celldims[0] = " << celldims[ 0 ] << ", _cellIndexX = " << _cellIndexX << ", _x = " << _x
                << ", _megaSize = " << _megaSize << ", temp_n_cellX = " << temp_n_cellX << std::endl;
      //		}
    }
...
//}
    ...
    if ( _isRegulatingEBU[ layerIndex ][ waferIndex ] && celldims[ 0 ] > celldims[ 1 ] * 1.5 ) {
      ...
      if ( _cellIndexX > n_cellX - 1 ) {               // hit position is out of this EBU.
        _cellIndexX -= 1;
#if VERBOSEcoterra
        std::cout << "This hit is out of regulation EBU=> merged into neighbour strip; _cellIndexX = " << _cellIndexX
                  << std::endl;
#endif
      }
    }
  ...
  if ( CellIndexX > 10 )
    std::cout << "HERE check just before return _decoder: _cellIndexX = " << _cellIndexX << std::endl;
#endif
