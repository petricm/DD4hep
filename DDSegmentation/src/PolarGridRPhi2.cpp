...
PolarGridRPhi2::PolarGridRPhi2( const std::string& cellEncoding ) : PolarGrid( cellEncoding ) {
  // define type and description
  _type        = "PolarGridRPhi2";
  _description = "Polar RPhi segmentation in the local XY-plane";

  // register all necessary parameters
  registerParameter( "grid_r_values", "Vector of R values", _gridRValues, std::vector<double>(),
                     SegmentationParameter::LengthUnit, true );
  registerParameter( "grid_phi_values", "Cell size in Phi", _gridPhiValues, std::vector<double>(),
                     SegmentationParameter::AngleUnit );
  registerParameter( "offset_r", "Cell offset in R", _offsetR, double( 0. ), SegmentationParameter::LengthUnit, true );
  registerParameter( "offset_phi", "Cell offset in Phi", _offsetPhi, double( 0. ), SegmentationParameter::AngleUnit,
                     true );
...
PolarGridRPhi2::PolarGridRPhi2( BitField64* decode ) : PolarGrid( decode ) {
  // define type and description
  _type        = "PolarGridRPhi2";
  _description = "Polar RPhi segmentation in the local XY-plane";
...
Vector3D PolarGridRPhi2::position( const CellID& cID ) const {
  ...
  double    phi  = binToPosition( ( *_decoder )[ _phiId ].value(), _gridPhiValues[ rBin ],
                              _offsetPhi + _gridPhiValues[ rBin ] * 0.5 );
...
CellID PolarGridRPhi2::cellID( const Vector3D& localPosition, const Vector3D& /* globalPosition */,
