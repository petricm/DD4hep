...
WaferGridXY::WaferGridXY( const std::string& cellEncoding ) : CartesianGrid( cellEncoding ) {
  // define type and description
  _type = "WaferGridXY";
  _description =
      "Cartesian segmentation in the local XY-plane for both Normal wafer and Magic wafer(depending on the layer "
      "dimensions)";
  ...
  registerParameter( "identifier_groupMGWafer", "Cell encoding identifier for Magic Wafer group",
                     _identifierMGWaferGroup, std::string( "layer" ), SegmentationParameter::NoUnit, true );
...
Vector3D WaferGridXY::position( const CellID& cID ) const {
  ...
  if ( _waferOffsetX[ _groupMGWaferIndex ][ _waferIndex ] > 0 ||
       _waferOffsetX[ _groupMGWaferIndex ][ _waferIndex ] < 0 ) {
    cellPosition.X = binToPosition( ( *_decoder )[ _xId ].value(), _gridSizeX,
                                    _offsetX + _waferOffsetX[ _groupMGWaferIndex ][ _waferIndex ] );
  } else {
    cellPosition.X = binToPosition( ( *_decoder )[ _xId ].value(), _gridSizeX, _offsetX );
  }

  if ( _waferOffsetY[ _groupMGWaferIndex ][ _waferIndex ] > 0 ||
       _waferOffsetY[ _groupMGWaferIndex ][ _waferIndex ] < 0 ) {
    cellPosition.Y = binToPosition( ( *_decoder )[ _yId ].value(), _gridSizeY,
                                    _offsetY + _waferOffsetY[ _groupMGWaferIndex ][ _waferIndex ] );
  } else {
...
CellID WaferGridXY::cellID( const Vector3D& localPosition, const Vector3D& /* globalPosition */,
  ...
  if ( _waferOffsetX[ _groupMGWaferIndex ][ _waferIndex ] > 0 ||
       _waferOffsetX[ _groupMGWaferIndex ][ _waferIndex ] < 0 ) {
    ( *_decoder )[ _xId ] =
        positionToBin( localPosition.X, _gridSizeX, _offsetX + _waferOffsetX[ _groupMGWaferIndex ][ _waferIndex ] );
  } else {
  ...
  if ( _waferOffsetY[ _groupMGWaferIndex ][ _waferIndex ] > 0 ||
       _waferOffsetY[ _groupMGWaferIndex ][ _waferIndex ] < 0 ) {
    ( *_decoder )[ _yId ] =
        positionToBin( localPosition.Y, _gridSizeY, _offsetY + _waferOffsetY[ _groupMGWaferIndex ][ _waferIndex ] );
  } else {
