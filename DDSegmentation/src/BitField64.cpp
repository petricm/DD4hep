...
BitFieldValue::BitFieldValue( long64& bitfield, const std::string& fieldName, unsigned fieldOffset, int signedWidth )
    : _b( bitfield ),
      _mask( 0 ),
      _name( fieldName ),
      _offset( fieldOffset ),
  ...
  if ( _offset > 63 || _offset + _width > 64 ) {
    std::stringstream s;
    s << " BitFieldValue '" << _name << "': out of range -  offset : " << _offset << " width " << _width;
...
void BitField64::addField( const std::string& name, unsigned offset, int width ) {
  ...
  if ( _joined & bfv->mask() ) {
    std::stringstream s;
    s << " BitFieldValue::addField(" << name << "): bits already used " << std::hex << _joined << " for mask "
      << bfv->mask();
  ...
  for ( unsigned i = 0; i < fieldDescriptors.size(); i++ ) {
    ...
    switch ( subfields.size() ) {
      ...
      default:
        ...
        std::stringstream s;
        s << " BitField64: invalid number of subfields " << fieldDescriptors[ i ];
...
std::ostream& operator<<( std::ostream& os, const BitField64& b ) {
  ...
  for ( BitField64::IndexMap::const_iterator it = b._map.begin(); it != b._map.end(); ++it ) {
    os << "  " << it->first << " [" << b[ it->second ].offset() << ":";

    if ( b[ it->second ].isSigned() )
      os << "-";

    os << b[ it->second ].width() << "]  : ";

    os << b[ it->second ].value() << std::endl;
  }
