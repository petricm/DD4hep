...
void Segmentation::registerIdentifier( const std::string& idName, const std::string& idDescription,
                                       std::string& identifier, const std::string& defaultValue ) {
  StringParameter idParameter = new TypedSegmentationParameter<std::string>(
      idName, idDescription, identifier, defaultValue, SegmentationParameter::NoUnit, true );
...
int Segmentation::positionToBin( double position, std::vector<double> const& cellBoundaries, double offset ) {
  ...
  if ( fabs( position / cellBoundaries.front() - 1.0 ) < 3e-12 )
    return 0;

  // include the upper edge of the last bin to the segmentation, deal with numerical issues
  if ( fabs( position / cellBoundaries.back() - 1.0 ) < 3e-12 )
  ...
  std::vector<double>::const_iterator bin =
      std::upper_bound( cellBoundaries.begin(), cellBoundaries.end(), position - offset );
