...
namespace DD4hep {
  namespace DDSegmentation {
    namespace Util {
      ...
      double radiusFromXYZ(const Vector3D& position) { return std::sqrt(position.X * position.X + position.Y * position.Y); }

      /// calculates the polar angle theta from Cartesian coordinates
      double thetaFromXYZ(const Vector3D& position) { return std::acos(position.Z / radiusFromXYZ(position)); }

      /// calculates the azimuthal angle phi from Cartesian coordinates
      double phiFromXYZ(const Vector3D& position) { return std::atan2(position.Y, position.X); }
