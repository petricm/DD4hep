...
namespace DD4hep {
   ...
   */
     ...
     */
      BitField64(const std::string& initString) : _value(0), _joined(0) { init(initString); }
     ...
     */
      ...
      BitField64& operator()(long64 val) {
        setValue(val);
        return *this;
      }
     ...
     */
      BitFieldValue& operator[](size_t index) { return *_fields.at(index); }

      /** Const acces to field through index 
     */
      const BitFieldValue& operator[](size_t index) const { return *_fields.at(index); }
     ...
     */
      BitFieldValue& operator[](const std::string& name) { return *_fields[index(name)]; }
   ...
   */
    ...
    public:
      ...
      StringTokenizer(std::vector<std::string>& tokens, char del) : _tokens(tokens), _del(del), _last(del) {}
