...
namespace DD4hep {
  namespace DDSegmentation {
    ...
    public:
      ...
      ProjectiveCylinder(TYPE cellEncoding, int thetaBins = 1, int phiBins = 1, double offsetTheta = 0.,
                         double offsetPhi = 0., const std::string& thetaField = "theta", const std::string& phiField = "phi",
                         const std::string& layerField = "layer");
      /// destructor
      ...
      int getThetaBins() const { return _thetaBins; }
      /// access the number of bins in theta
      int getPhiBins() const { return _phiBins; }
      /// access the coordinate offset in theta
      double getOffsetTheta() const { return _offsetTheta; }
      /// access the coordinate offset in phi
      double getOffsetPhi() const { return _offsetPhi; }
      /// access the field name used for theta
      std::string getFieldNameTheta() const { return _thetaID; }
      /// access the field name used for phi
      std::string getFieldNamePhi() const { return _phiID; }
      /// set the number of bins in theta
      void setThetaBins(int bins) { _thetaBins = bins; }
      /// set the number of bins in phi
      void setPhiBins(int bins) { _phiBins = bins; }
      /// set the coordinate offset in theta
      void setOffsetTheta(double offset) { _offsetTheta = offset; }
      /// set the coordinate offset in phi
      void setOffsetPhi(double offset) { _offsetPhi = offset; }
      /// set the field name used for theta
      void setFieldNameTheta(const std::string& name) { _thetaID = name; }
