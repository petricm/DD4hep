  ...
  namespace PluginService {
    ...
    namespace Details {
      ...
      public:
        ...
        template <typename S, typename... Args> static typename S::ReturnType create(Args&&... args) {
      ...
      /// </ul>
      template <typename F> inline F getCreator(const std::string& id) {
        union {
          void* src;
          F     dst;
        } p2p;
      ...
      public:
        ...
        struct FactoryInfo {
          FactoryInfo(const std::string& lib, void* p = 0, const std::string& t = "", const std::string& rt = "",
                      const std::string& cn = "", const Properties& props = Properties())
        ...
        template <typename F, typename T, typename I> inline FactoryInfo& add(const I& id, typename F::FuncType ptr) {
          union {
          ...
          std::ostringstream o;
          o << id;
          return add(o.str(), p2p.dst, typeid(typename F::FuncType).name(), typeid(typename F::ReturnType).name(),
                     demangle<T>());
        }
        ...
        inline const FactoryMap& factories() const {
          if (!m_initialized)
            const_cast<Registry*>(this)->initialize();
          return m_factories;
        }
...
#define _INTERNAL_DECLARE_FACTORY(type, id, factory, serial) \
  _INTERNAL_DECLARE_FACTORY_WITH_CREATOR(type, ::Gaudi::PluginService::Details::Factory<type>, id, factory, serial)
