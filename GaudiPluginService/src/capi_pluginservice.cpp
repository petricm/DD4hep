...
cgaudi_factory_t cgaudi_pluginsvc_get_factory_at( cgaudi_pluginsvc_t self, int n ) {
  ...
  for ( Registry::FactoryMap::const_iterator itr = reg->factories().begin(), iend = reg->factories().end(); itr != iend;
        ++itr ) {
...
cgaudi_property_t cgaudi_factory_get_property_at( cgaudi_factory_t self, int n ) {
  ...
  const Registry::FactoryInfo& fi  = reg.getInfo( id );
  int                          i   = 0;
  for ( Registry::Properties::const_iterator itr = fi.properties.begin(), iend = fi.properties.end(); itr != iend;
        ++itr, ++i ) {
    if ( i == n ) {
