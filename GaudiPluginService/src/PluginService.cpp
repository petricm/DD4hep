...
inline void factoryInfoSetHelper( std::string& dest, const std::string& value, const std::string& desc,
  ...
  if ( dest.empty() ) {
    ...
    std::ostringstream o;
    o << "new factory loaded for '" << id << "' with different " << desc << ": " << dest << " != " << value;
          ...
          if ( ( extpos != std::string::npos ) && ( ( extpos + 11 ) == name.size() ) ) {
            ...
            while ( !facts.eof() ) {
              ...
              if ( other_pos == std::string::npos ) {
                std::ostringstream o;
                o << "failed to parse line " << fullPath << ':' << lineCount;
...
Registry::FactoryInfo& Registry::add( const std::string& id, void* factory, const std::string& type,
  ...
  if ( entry == facts.end() ) {
    ...
    entry =
        facts.insert( std::make_pair( id, FactoryInfo( "unknown", factory, type, rtype, className, props ) ) ).first;
#if DEBUG_FOR_MAC
    std::cout << " --  registering factory id: " << id << " class : " << className << " with registry " << this
              << std::endl;
#endif
...
void* Registry::get( const std::string& id, const std::string& type ) const {
  ...
  if ( f != facts.end() ) {
    ...
    if ( f->second.type == type ) {
      ...
      logger().warning( "found factory " + id + ", but of wrong type: " + demangle( f->second.type ) + " instead of " +
                        demangle( type ) );
    }
  }
...
std::set<Registry::KeyType> Registry::loadedFactories() const {
  ...
  const FactoryMap& facts = factories();
  std::set<KeyType> l;
  for ( FactoryMap::const_iterator f = facts.begin(); f != facts.end(); ++f ) {
    if ( f->second.ptr )
      l.insert( f->first );
  }
  return l;
