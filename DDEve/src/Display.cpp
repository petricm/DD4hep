...
Display::CalodataContext::CalodataContext() : slice( 0 ), calo3D( 0 ), caloViz( 0 ), eveHist( 0 ), config() {
}
...
void Display::MessageBox( PrintLevel level, const string& text, const string& title ) const {
  ...
  new TGMsgBox( gClient->GetRoot(), 0, title.c_str(), text.c_str(), pic, kMBDismiss, 0, kVerticalFrame,
...
string Display::OpenXmlFileDialog( const string& default_dir ) const {
  static const char* evtFiletypes[] = {"xml files", "*.xml", "XML files", "*.XML", "All files", "*", 0, 0};
  TGFileInfo         fi;
...
string Display::OpenEventFileDialog( const string& default_dir ) const {
  static const char* evtFiletypes[] = {"ROOT files", "*.root",    "SLCIO files", "*.slcio", "LCIO files",
                                       "*.lcio",     "All files", "*",           0,         0};
  TGFileInfo fi;
  fi.fFileTypes = evtFiletypes;
  fi.fIniDir    = StrDup( default_dir.c_str() );
...
void Display::OnNewEvent( EventHandler* handler ) {
  ...
  for ( Types::const_iterator ityp = types.begin(); ityp != types.end(); ++ityp ) {
    ...
    for ( Collections::const_iterator j = colls.begin(); j != colls.end(); ++j ) {
      ...
      if ( len > 0 ) {
        ...
        if ( typ == EventHandler::CALO_HIT_COLLECTION || typ == EventHandler::TRACKER_HIT_COLLECTION ) {
  ...
  for ( Calodata::iterator i = m_calodata.begin(); i != m_calodata.end(); ++i ) {
    ...
    ctx.eveHist->DataChanged();
    printout( INFO, "FillEtaPhiHistogram", "+++ %s: Filled %ld hits from %s....", ctx.calo3D->GetName(), n,
              ctx.config.hits.c_str() );
  }
...
void Display::LoadGeoChildren( TEveElement* start, int levels, bool redraw ) {
  ...
  if ( world.children().size() == 0 ) {
    ...
    if ( 0 == start ) {
      ...
      printout( INFO, "Display", "+++ Load children of %s to %d levels", world.placement().name(), levels );
...
void Display::MakeNodesVisible( TEveElement* e, bool visible, int level ) {
  printout( INFO, "Display", "+++ %s element %s with a depth of %d.", visible ? "Show" : "Hide",
            Utilities::GetName( e ), level );
