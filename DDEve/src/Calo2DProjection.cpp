...
ClassImp( Calo2DProjection ) DECLARE_VIEW_FACTORY( Calo2DProjection )
...
void Calo2DProjection::ConfigureGeometry( const DisplayConfiguration::ViewConfig& config ) {
  ...
  for ( rit = config.subdetectors.rbegin(); rit != config.subdetectors.rend(); ++rit ) {
    ...
    if ( cfg.type == DisplayConfiguration::CALODATA && cfg.use.empty() ) {
      ...
      printout( INFO, "Calo2DProjection", "+++ %s: add detector %s [%s] rmin=%f towerH:%f emax=%f", name().c_str(), n,
                ctx.config.hits.c_str(), calo3d.rmin, calo3d.towerH, calo3d.emax );
    }
  }
}
