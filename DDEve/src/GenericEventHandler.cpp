...
bool GenericEventHandler::Open( const string& file_type, const string& file_name ) {
  ...
  new TGMsgBox( gClient->GetRoot(), 0, "Failed to open event data", err.c_str(), pic, kMBDismiss, 0, kVerticalFrame,
...
bool GenericEventHandler::NextEvent() {
  ...
  try {
    ...
    string err =
        ...
        string( e.what() ) + "\n\n";
    const TGPicture* pic = gClient->GetPicture( path.c_str() );
    new TGMsgBox( gClient->GetRoot(), 0, "Failed to read event", err.c_str(), pic, kMBDismiss, 0, kVerticalFrame,
                  kTextLeft | kTextCenterY );
  }
  return -1;
}
