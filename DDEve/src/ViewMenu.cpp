...
void ViewMenu::Build( TGMenuBar* bar, int hints ) {
  ...
  view_menu->AddEntry( "3&D View", this, &ViewMenu::CreateView,
                       p = new pair<string, string>( "DD4hep_DDEve_View3D", "3D" ) );
  view_menu->AddEntry( "Rho-&Z Projection", this, &ViewMenu::CreateView,
                       p = new pair<string, string>( "DD4hep_DDEve_RhoZProjection", "Rho-Z" ) );
  view_menu->AddEntry( "Rho-&Phi Projection", this, &ViewMenu::CreateView,
                       p = new pair<string, string>( "DD4hep_DDEve_RhoPhiProjection", "Rho-Phi" ) );
  ...
  for ( Display::ViewConfigurations::const_iterator i = vc.begin(); i != vc.end(); ++i ) {
    ...
    view_menu->AddEntry( v.name.c_str(), this, &ViewMenu::CreateView,
                         p = new pair<string, string>( "DD4hep_DDEve_" + v.type, v.name ) );
  }
