...
TEveElementList* View::AddToGlobalItems( const string& nam ) {
  if ( 0 == m_global ) {
    ...
    if ( m_geoScene )
      m_global->AddElement( geoScene() );
    if ( m_eveScene )
      m_global->AddElement( eveScene() );
    if ( m_showGlobal )
...
TEveElement* View::ImportEventElement( TEveElement* el, TEveElementList* list ) {
  ...
  if ( el ) {
    printout( INFO, "View", "ImportElement %s [%s] into list: %s", Utilities::GetName( el ), el->IsA()->GetName(),
              list->GetName() );
    list->AddElement( el );
  }
  return el;
...
pair<bool, TEveElement*> View::GetGlobalGeometry( DetElement de, const DisplayConfiguration::Config& /* cfg */ ) {
...
pair<bool, TEveElement*> View::CreateGeometry( DetElement de, const DisplayConfiguration::Config& cfg ) {
  SensitiveDetector sd    = m_eve->lcdd().sensitiveDetector( de.name() );
    ...
    if ( nam == "global" ) {
      ...
      if ( i != c.end() && cfg.data.defaults.show_evt > 0 ) {
        ...
        if ( sd.isValid() ) {
          ...
          printout( INFO, "View", "+++     Add detector event %s collection:%s data:%p scene:%p", nam.c_str(), coll,
                    child, m_eveScene );
          if ( child )
            ImportEvent( child );
        }
...
View& View::Map( TEveWindow* slot ) {
  ...
  if ( m_geoScene )
    m_view->AddScene( m_geoScene );
  if ( m_eveScene )
    m_view->AddScene( m_eveScene );
  return *this;
}
