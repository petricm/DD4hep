...
ClassImp( ContextMenu )
    ...
    /// Initializing constructor
    ContextMenu::ContextMenu( TClass* cl )
    : m_class( cl ) {
  if ( !cl ) {
...
ContextMenu& ContextMenu::Add( const string& title, Callback cb, void* ud ) {
  ...
  TClassMenuItem*     item    = new TClassMenuItem( TClassMenuItem::kPopupUserFunction, ContextMenuHandler::Class(),
                                             title.c_str(), "Context", handler, "TObject*", 2 );
