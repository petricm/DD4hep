...
template <>
void Converter<ddeve>::operator()( xml_h seq ) const;
template <>
void Converter<display>::operator()( xml_h seq ) const;
template <>
void Converter<view>::operator()( xml_h seq ) const;
template <>
void Converter<panel>::operator()( xml_h seq ) const;
template <>
void Converter<include>::operator()( xml_h seq ) const;
template <>
void Converter<calodata>::operator()( xml_h e ) const;
template <>
void Converter<calodata_configs>::operator()( xml_h e ) const;
template <>
void Converter<collection>::operator()( xml_h e ) const;
template <>
void Converter<collection_configs>::operator()( xml_h e ) const;
template <>
...
static void extract( DisplayConfiguration::Config& c, xml_h e, int typ ) {
  ...
  if ( e.hasAttr( _U( dz ) ) )
    c.data.calo3d.dz = e.attr<float>( _U( dz ) );
  if ( e.hasAttr( _U( rmin ) ) )
    c.data.calo3d.rmin = e.attr<float>( _U( rmin ) );
  if ( e.hasAttr( _U( threshold ) ) )
    c.data.calo3d.threshold = e.attr<float>( _U( threshold ) );
  if ( e.hasAttr( u_hits ) )
...
template <>
void Converter<panel>::operator()( xml_h e ) const {
  Configurations*              configs = (Configurations*)param;
  DisplayConfiguration::Config c;
 ...
 */
template <>
void Converter<view>::operator()( xml_h e ) const {
  ...
  c.name           = e.attr<string>( _U( name ) );
  printout( INFO, "DisplayConfiguration", "+++ View: %s sensitive:%d structure:%d.", c.name.c_str(), c.show_sensitive,
            c.show_structure );
...
void Converter<calodata>::operator()( xml_h e ) const {
  ...
  if ( e.hasAttr( u_use ) ) {
    ...
    if ( e.hasAttr( u_towerH ) )
      c.data.calodata.towerH = e.attr<float>( u_towerH );
    if ( e.hasAttr( _U( threshold ) ) )
      c.data.calodata.threshold = e.attr<float>( _U( threshold ) );
  }
  configs->push_back( c );
}
 ...
 */
template <>
void Converter<include>::operator()( xml_h e ) const {
 ...
 */
template <>
void Converter<display>::operator()( xml_h e ) const {
  Display* d = (Display*)param;
  if ( e.hasAttr( _Unicode( visLevel ) ) )
    d->setVisLevel( e.attr<int>( _Unicode( visLevel ) ) );
  if ( e.hasAttr( _Unicode( eventHandler ) ) )
    d->setEventHandlerName( e.attr<std::string>( _Unicode( eventHandler ) ) );
  if ( e.hasAttr( _Unicode( loadLevel ) ) )
    d->setLoadLevel( e.attr<int>( _Unicode( loadLevel ) ) );
}
 ...
 */
template <>
void Converter<ddeve>::operator()( xml_h e ) const {
