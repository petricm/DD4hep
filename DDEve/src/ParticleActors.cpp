...
static Color_t Colors[] = {kRed,    kBlue, kYellow, kGreen,  kPink,    kAzure,
                           kOrange, kTeal, kViolet, kSpring, kMagenta, kCyan};

/// Standard initializing constructor
MCParticleCreator::MCParticleCreator( TEveTrackPropagator* p, TEveCompound* ps,
...
void MCParticleCreator::operator()( const DDEveParticle& p ) {
  ...
  if ( dir.R() * CM_2_MM > 100e-3 && p.energy > 10e0 ) {
    ...
    TParticle     part( p.pdgID, 0, 0, 0, 0, 0, p.psx * MEV_2_GEV, p.psy * MEV_2_GEV, p.psz * MEV_2_GEV,
                    p.energy * MEV_2_GEV, p.vsx * MM_2_CM, p.vsy * MM_2_CM, p.vsz * MM_2_CM, p.time );
    ...
    t->SetTitle(
        Form( "MCParticle: Track ID=%d Parent:%d\n"
              "Type:%s Charge=%.3f Time:%.3f ns\n"
              "Start(Vx, Vy, Vz, t) = (%.3f, %.3f, %.3f) [cm]\n"
              "End  (Vx, Vy, Vz, t) = (%.3f, %.3f, %.3f) [cm]\n"
              "Length:%.3f [cm]\n"
              "(Px, Py, Pz, E) = (%.3f, %.3f, %.3f, %.3f) [GeV]",
              p.id, p.parent, def ? def->GetName() : "Unknown", def ? def->Charge() : 0.0, p.time, p.vsx * MM_2_CM,
              p.vsy * MM_2_CM, p.vsz * MM_2_CM, p.vex * MM_2_CM, p.vey * MM_2_CM, p.vez * MM_2_CM, dir.R(),
