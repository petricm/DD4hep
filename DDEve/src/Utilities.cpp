...
std::pair<bool, TEveElement*> Utilities::createEveShape( int level, int max_level, TEveElement* p, TGeoNode* n,
                                                         TGeoHMatrix mat, const std::string& nam ) {
  TGeoVolume* vol     = n ? n->GetVolume() : 0;
  bool        created = false;

  if ( 0 == vol || level > max_level )
    return make_pair( created, (TEveElement*)0 );
