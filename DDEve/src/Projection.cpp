...
Projection::Projection( Display* eve, const string& nam ) : View( eve, nam ), m_projMgr( 0 ), m_axis( 0 ) {
}
...
TEveElement* Projection::ImportElement( TEveElement* el, TEveElementList* list ) {
  ...
  printout( INFO, "Projection", "ImportElement %s [%s] into list: %s Projectable:%s [%p]", Utilities::GetName( el ),
            el->IsA()->GetName(), list->GetName(), dynamic_cast<TEveProjectable*>( list ) ? "true" : "false", e );
