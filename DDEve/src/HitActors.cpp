...
void BoxsetCreator::operator()( const DDEveHit& hit ) {
  ...
  cout << "Hit:" << ene << " deposit:" << hit.deposit << " "
       << " emax:" << emax << " towerH:" << towerH << endl;
  ...
  float    s2Z          = s1Z;
  float    coords[ 24 ] = {float( p.X() + s1X ), float( p.Y() + s1Y ), float( p.Z() - s1Z ), float( p.X() + s1X ),
                        float( p.Y() + s1Y ), float( p.Z() + s1Z ), float( p.X() - s2X ), float( p.Y() - s2Y ),
                        float( p.Z() + s2Z ), float( p.X() - s2X ), float( p.Y() - s2Y ), float( p.Z() - s2Z ),
                        float( p.X() + s2X ), float( p.Y() + s2Y ), float( p.Z() - s2Z ), float( p.X() + s2X ),
                        float( p.Y() + s2Y ), float( p.Z() + s2Z ), float( p.X() - s1X ), float( p.Y() - s1Y ),
                        float( p.Z() + s1Z ), float( p.X() - s1X ), float( p.Y() - s1Y ), float( p.Z() - s1Z )};
  ++count;
