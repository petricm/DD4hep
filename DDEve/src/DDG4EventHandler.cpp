...
EventHandler::CollectionType DDG4EventHandler::collectionType( const std::string& collection ) const {
  ...
  if ( i != m_branches.end() ) {
    ...
    if (::strstr( cl, "Simulation::Geant4Calorimeter::Hit" ) )
      return CALO_HIT_COLLECTION;
    else if (::strstr( cl, "Simulation::Geant4Tracker::Hit" ) )
      return TRACKER_HIT_COLLECTION;
    else if (::strstr( cl, "Simulation::Geant4Particle" ) )
      return PARTICLE_COLLECTION;
    // These are OLD types. Eventually remove these lines.....
    else if (::strstr( cl, "Simulation::SimpleCalorimeter::Hit" ) )
      return CALO_HIT_COLLECTION;
    else if (::strstr( cl, "Simulation::SimpleTracker::Hit" ) )
      return TRACKER_HIT_COLLECTION;
    else if (::strstr( cl, "Simulation::Particle" ) )
      return PARTICLE_COLLECTION;
  }
...
Int_t DDG4EventHandler::ReadEvent( Long64_t event_number ) {
  ...
  if ( hasFile() ) {
    if ( event_number >= m_file.second->GetEntries() ) {
      ...
      printout( ERROR, "DDG4EventHandler", "+++ ReadEvent: Cannot read across End-of-file! Reading last event:%d.",
                event_number );
    } else if ( event_number < 0 ) {
      event_number = 0;
      printout( ERROR, "DDG4EventHandler", "+++ nextEvent: Cannot read across Start-of-file! Reading first event:%d.",
                event_number );
    }

    Int_t nbytes = m_file.second->GetEntry( event_number );
    if ( nbytes >= 0 ) {
      printout( ERROR, "DDG4EventHandler", "+++ ReadEvent: Read %d bytes of event data for entry:%d", nbytes,
                event_number );
