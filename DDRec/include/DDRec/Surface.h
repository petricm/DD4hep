...
class VolSurfaceBase : public ISurface {
 ...
 public:
  ...
  VolSurfaceBase( SurfaceType typ, double thickness_inner, double thickness_outer, Vector3D u_val, Vector3D v_val,
...
class VolSurface : public ISurface {
 ...
 public:
  ...
  VolSurface( VolSurfaceBase* p ) : _surf( p ) {
    ++_surf->_refCount;
  }
...
class VolPlaneImpl : public VolSurfaceBase {
 public:
  ...
  VolPlaneImpl( SurfaceType typ, double thickness_inner, double thickness_outer, Vector3D u_val, Vector3D v_val,
                Vector3D n_val, Vector3D o_val, Geometry::Volume vol, int id_val )
      :
...
class VolCylinderImpl : public VolSurfaceBase {
 public:
   ...
   */
  VolCylinderImpl( Geometry::Volume vol, SurfaceType type, double thickness_inner, double thickness_outer,
                   Vector3D origin );
  ...
  /** Convert the global position to the local position (u,v) on the surface - v runs along the axis of the cylinder, u
   * is r*phi */
  virtual Vector2D globalToLocal( const Vector3D& point ) const;

  /** Convert the local position (u,v) on the surface to the global position  - v runs along the axis of the cylinder, u
   * is r*phi*/
  virtual Vector3D localToGlobal( const Vector2D& point ) const;
};
...
class VolConeImpl : public VolSurfaceBase {
 ...
 public:
   ...
   */
  VolConeImpl( Geometry::Volume vol, SurfaceType type, double thickness_inner, double thickness_outer, Vector3D v,
               Vector3D origin );
  ...
  /** Convert the global position to the local position (u,v) on the surface - v runs along the axis of the cone, u is
   * r*phi */
  virtual Vector2D globalToLocal( const Vector3D& point ) const;

  /** Convert the local position (u,v) on the surface to the global position  - v runs along the axis of the cone, u is
   * r*phi*/
  virtual Vector3D localToGlobal( const Vector2D& point ) const;
...
class VolCylinder : public VolSurface {
 public:
  VolCylinder( Geometry::Volume vol, SurfaceType typ_val, double thickness_inner, double thickness_outer,
               Vector3D origin_val )
...
class VolCone : public VolSurface {
 public:
  VolCone( Geometry::Volume vol, SurfaceType typ_val, double thickness_inner, double thickness_outer, Vector3D v_val,
           Vector3D origin_val )
...
class CylinderSurface : public Surface, public ICylinder {
 public:
  ...
  /** Convert the global position to the local position (u,v) on the surface - u runs along the axis of the cylinder, v
   * is r*phi */
  virtual Vector2D globalToLocal( const Vector3D& point ) const;

  /** Convert the local position (u,v) on the surface to the global position  - u runs along the axis of the cylinder, v
   * is r*phi*/
  virtual Vector3D localToGlobal( const Vector2D& point ) const;
