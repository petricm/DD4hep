...
namespace DD4hep {
  namespace DDRec {
    ...
    public:
      /// Default constructor
      Subdetector(const Geometry::DetElement& det) : Geometry::DetElement(det) { getSubdetectorExtension(); }
      ...
      bool isBarrel() const { return _subdetector->isBarrel(); }

      /// Is this an endcap detector
      bool isEndcap() const { return _subdetector->isEndcap(); }

      /// Access to the inner radius
      double getRMin() const { return _subdetector->getRMin(); }

      /// Access to the outer radius
      double getRMax() const { return _subdetector->getRMax(); }

      /// Access to the lower z extent
      double getZMin() const { return _subdetector->getZMin(); }

      /// Access to the upper z extent
      double getZMax() const { return _subdetector->getZMax(); }

      /// Access to the number of sides
      /* Describes the number of corners for a polygon.
	 * Returns 0 in case of a circular shape
	 */
      int getNSides() const { return _subdetector->getNSides(); }
