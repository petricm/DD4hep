...
class LayeredSubdetector : public virtual Geometry::DetElement, public LayeringExtension {
 public:
  /// Default constructor
  LayeredSubdetector( const Geometry::DetElement& det ) : Geometry::DetElement( det ) {
 ...
 private:
  ...
  void getLayeringExtension() {
    ...
    if ( not _layering ) {
      throw invalid_detector_element( "Found no extension of type \"LayeringExtension\"",
                                      Geometry::DetElement( *this ) );
    }
  }
};
