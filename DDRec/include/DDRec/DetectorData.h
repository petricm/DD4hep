...
struct ZPlanarStruct {
  ...
  struct LayerLayout {
    ...
    /// The offset of the ladder support, i.e. the shift in the direction of increasing phi, perpendicular to the
    /// ladders's normal.
    ...
    /// The offset of the ladder sensitive, i.e. the shift in the direction of increasing phi, perpendicular to the
    /// ladders's normal.
...
struct ConicalSupportStruct {
  ...
  struct Section {
    Section() : rInner( 0 ), rOuter( 0 ), zPos( 0 ) {
    }
...
struct LayeredCalorimeterStruct {
  ...
  struct Layer {
    ...
    /// phi0 of layer: potential rotation around normal to absorber plane, e.g. if layers are 'staggered' in phi in fwd.
    /// calos
    double phi0;

    /// thickness of the absorber part of the layer. Consider using inner/outer_nRadiationLengths and
    /// inner/outer_nInteractionLengths
    ...
    /// cell size along the first axis where first is either along the beam (BarrelLayout) or up (EndcapLayout) or the
    /// direction closest to that.
    double cellSize0;
