...
void VolSurfaceBase::setU( const Vector3D& u_val ) {
  _u = u_val;
}
void VolSurfaceBase::setV( const Vector3D& v_val ) {
  _v = v_val;
}
void VolSurfaceBase::setNormal( const Vector3D& n ) {
  _n = n;
}
void VolSurfaceBase::setOrigin( const Vector3D& o ) {
  _o = o;
}

long64 VolSurfaceBase::id() const {
  return _id;
}

const SurfaceType& VolSurfaceBase::type() const {
  return _type;
}
Vector3D VolSurfaceBase::u( const Vector3D& point ) const {
  point.x();
  return _u;
}
Vector3D VolSurfaceBase::v( const Vector3D& point ) const {
  point.x();
  return _v;
}
Vector3D VolSurfaceBase::normal( const Vector3D& point ) const {
  point.x();
  return _n;
}
const Vector3D& VolSurfaceBase::origin() const {
  return _o;
}
...
const IMaterial& VolSurfaceBase::innerMaterial() const {
  return _innerMat;
}
const IMaterial& VolSurfaceBase::outerMaterial() const {
  return _outerMat;
}
double VolSurfaceBase::innerThickness() const {
  return _th_i;
}
double VolSurfaceBase::outerThickness() const {
  return _th_o;
}
...
double VolSurfaceBase::length_along_u() const {
  ...
  if ( volume()->GetShape()->Contains( o.const_array() ) ) {
    ...
    // std::cout << " VolSurfaceBase::length_along_u() : shape contains(o)  =  " << volume()->GetShape()->Contains(
    // o.const_array() )
    // 	  << " dist_p " <<    dist_p
    // 	  << " dist_m " <<    dist_m
    // 	  << std::endl ;
...
double VolSurfaceBase::distance( const Vector3D& point ) const {
  point.x();
  return 1.e99;
}
...
bool VolSurfaceBase::insideBounds( const Vector3D& point, double epsilon ) const {
  ...
  // fixme: older versions of ROOT (~<5.34.10 ) take a non const pointer as argument - therefore use a const cast here
  // for the time being ...
...
long64 VolSurface::id() const {
  return _surf->id();
}
const SurfaceType& VolSurface::type() const {
  return _surf->type();
}
Vector3D VolSurface::u( const Vector3D& point ) const {
  return _surf->u( point );
}
Vector3D VolSurface::v( const Vector3D& point ) const {
  return _surf->v( point );
}
Vector3D VolSurface::normal( const Vector3D& point ) const {
  return _surf->normal( point );
}
const Vector3D& VolSurface::origin() const {
  return _surf->origin();
}
Vector2D VolSurface::globalToLocal( const Vector3D& point ) const {
  return _surf->globalToLocal( point );
}
Vector3D VolSurface::localToGlobal( const Vector2D& point ) const {
  return _surf->localToGlobal( point );
}
const IMaterial& VolSurface::innerMaterial() const {
  return _surf->innerMaterial();
}
const IMaterial& VolSurface::outerMaterial() const {
  return _surf->outerMaterial();
}
double VolSurface::innerThickness() const {
  return _surf->innerThickness();
}
double VolSurface::outerThickness() const {
  return _surf->outerThickness();
}
double VolSurface::length_along_u() const {
  return _surf->length_along_u();
}
double VolSurface::length_along_v() const {
  return _surf->length_along_v();
}
double VolSurface::distance( const Vector3D& point ) const {
  return _surf->distance( point );
}
...
VolCylinderImpl::VolCylinderImpl( Geometry::Volume vol, SurfaceType typ, double thickness_inner, double thickness_outer,
                                  Vector3D o )
    :
...
//================================================================================================================
VolConeImpl::VolConeImpl( Geometry::Volume vol, SurfaceType typ, double thickness_inner, double thickness_outer,
                          Vector3D v_val, Vector3D o_val )
    :
  ...
  if ( std::fabs( dphi ) > 1e-6 ) {
    ...
    sst << "VolConeImpl::VolConeImpl() - incompatibel vector v and o given " << v_val << " - " << o_val;
  ...
  Vector3D vm = -1. * v_val;
  double   dist_m =
      vol->GetShape()->DistFromInside( const_cast<double*>( o_val.const_array() ), const_cast<double*>( vm.array() ) );
...
bool findVolume( PlacedVolume pv, Volume theVol, std::list<PlacedVolume>& volList ) {
  ...
  //   	std::cout << " **" ;
  //   }
  //   std::cout << " searching for volume: " << theVol.name() << " " << std::hex << theVol.ptr() << "  <-> pv.volume :
  //   "  << pv.name() << " " <<  pv.volume().ptr()
  ...
  if ( pv.volume().ptr() == theVol.ptr() ) {
    ...
    for ( Int_t idau = 0, ndau = node->GetNdaughters(); idau < ndau; ++idau ) {
      ...
      if ( !placement.data() ) {
        throw std::runtime_error( "*** findVolume: Invalid not instrumented placement:" +
                                  std::string( daughter->GetName() ) +
...
Surface::Surface( Geometry::DetElement det, VolSurface volSurf )
    : _det( det ), _volSurf( volSurf ), _wtM( 0 ), _id( 0 ), _type( _volSurf.type() ) {
  initialize();
}

long64 Surface::id() const {
  return _id;
}

const SurfaceType& Surface::type() const {
  return _type;
}

Vector3D Surface::u( const Vector3D& point ) const {
  point.x();
  return _u;
}
Vector3D Surface::v( const Vector3D& point ) const {
  point.x();
  return _v;
}
Vector3D Surface::normal( const Vector3D& point ) const {
  point.x();
  return _n;
}
const Vector3D& Surface::origin() const {
  return _o;
}
double Surface::innerThickness() const {
  return _volSurf.innerThickness();
}
double Surface::outerThickness() const {
  return _volSurf.outerThickness();
}
double Surface::length_along_u() const {
  return _volSurf.length_along_u();
}
double Surface::length_along_v() const {
  return _volSurf.length_along_v();
}
...
const IMaterial& Surface::innerMaterial() const {
  ...
  if ( !( mat.Z() > 0 ) ) {
    ...
    _volSurf.setInnerMaterial( materials.size() > 1 ? matMgr.createAveragedMaterial( materials )
                                                    : materials[ 0 ].first );
  }
  return mat;
}
...
const IMaterial& Surface::outerMaterial() const {
  ...
  if ( !( mat.Z() > 0 ) ) {
    ...
    _volSurf.setOuterMaterial( materials.size() > 1 ? matMgr.createAveragedMaterial( materials )
                                                    : materials[ 0 ].first );
  }
  return mat;
}
...
void Surface::initialize() {
  ...
  if ( !findVolume( pv, theVol, pVList ) ) {
    ...
    std::stringstream sst;
    sst << " ***** ERROR: Volume " << theVol.name() << " not found for DetElement " << _det.name() << " with surface ";
    throw std::runtime_error( sst.str() );
  }
...
#if 0  // fixme: which convention to use here - the correct should be wtI, however it is the inverse of what is stored
       // in DetElement ???
...
std::vector<std::pair<Vector3D, Vector3D>> Surface::getLines( unsigned nMax ) {
  ...
  if ( type().isPlane() ) {
    if ( shape->IsA() == TGeoBBox::Class() ) {
      ...
      if ( isYZ || isXZ ||
           isXY ) {  // plane is parallel to one of the box' sides -> need 4 vertices from box dimensions
        ...
        lines.push_back( std::make_pair( _o + boxDim[ uidx ] * ub + boxDim[ vidx ] * vb,
    ...
    for ( unsigned i = 0; i < nMax; ++i ) {
      ...
      double dist = shape->DistFromInside( const_cast<double*>( lo.const_array() ),
                                           const_cast<double*>( luRot.const_array() ), 3, 0.1 );
      ...
      // std::cout << " **** normal:" << ln << " lu:" << lu  << " alpha:" << alpha << " luRot:" << luRot << " lp :" <<
      // lp  << " gp:" << gp << " dist : " << dist
      // 	    << " is point " << gp << " inside : " << shape->Contains( gp.const_array()  )
      // 	    << " dist from outside for lo,lu " <<  shape->DistFromOutside( lo.const_array()  , lu.const_array()   ,
      // 3 )
...
double CylinderSurface::radius() const {
  return _volSurf.origin().rho();
}

Vector3D CylinderSurface::center() const {
  return volumeOrigin();
}
...
Vector3D ConeSurface::center() const {
  return volumeOrigin();
}
