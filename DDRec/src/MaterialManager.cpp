...
const MaterialVec& MaterialManager::materialsBetween( const DDSurfaces::Vector3D& p0, const DDSurfaces::Vector3D& p1,
                                                      double epsilon ) {
  if ( ( p0 != _p0 ) || ( p1 != _p1 ) ) {
    //---------------------------------------
    _mV.clear();
    ...
    if ( !node1 )
      throw std::runtime_error(
          "No geometry node found at given location. Either there is no node placed here or position is outside of top "
          "volume." );
...
// and the next boundary is never reached
      ...
      if ( length < MINSTEP ) {
        _tgeoMgr->SetCurrentPoint( position[ 0 ] + MINSTEP * direction[ 0 ], position[ 1 ] + MINSTEP * direction[ 1 ],
      ...
      if ( currDistance > totDist ) {
        length = sqrt( pow( endpoint[ 0 ] - previouspos[ 0 ], 2 ) + pow( endpoint[ 1 ] - previouspos[ 1 ], 2 ) +
