...
std::ostream& operator<<( std::ostream& io, const ZPlanarData& d ) {
  ...
  io << " Layers : " << std::endl
     << "  nLadder phi0     nSensors    lengthSensor distSupport  thickSupport  offsetSupport widthSupport "
        "zHalfSupport distSense   thickSense   offsetSense   widthSense  zHalfSense"
     << std::endl;
  ...
  for ( unsigned i = 0, N = layers.size(); i < N; ++i ) {
    ...
    io << " " << l.ladderNumber << " " << l.phi0 << " " << l.sensorsPerLadder << " " << l.lengthSensor << " "
       << l.distanceSupport << " " << l.thicknessSupport << " " << l.offsetSupport << " " << l.widthSupport << " "
       << l.zHalfSupport << " " << l.distanceSensitive << " " << l.thicknessSensitive << " " << l.offsetSensitive << " "
       << l.widthSensitive << " " << l.zHalfSensitive << std::endl;
  }
...
std::ostream& operator<<( std::ostream& io, const ZDiskPetalsData& d ) {
  ...
  io << " Layers : " << std::endl
     << " petHalfAngle  alphaPetal  zPosition    nP n d p    phi0        zOffsetSup  distSupport  thickSupport "
        "widthInSup  widthOutSup   lengthSup   zOffsetSense  distSense    thickSense  widthInSense widthOutSense "
        "lenSense"
     << std::endl;
  ...
  for ( unsigned i = 0, N = layers.size(); i < N; ++i ) {
    ...
    io << " " << l.petalHalfAngle << " " << l.alphaPetal << " " << l.zPosition << " " << l.petalNumber << " "
       << l.sensorsPerPetal << " " << l.typeFlags[ ZDiskPetalsData::SensorType::DoubleSided ] << " "
       << l.typeFlags[ ZDiskPetalsData::SensorType::Pixel ] << " " << l.phi0 << " " << l.zOffsetSupport << " "
       << l.distanceSupport << " " << l.thicknessSupport << " " << l.widthInnerSupport << " " << l.widthOuterSupport
       << " " << l.lengthSupport << " " << l.zOffsetSensitive << " " << l.distanceSensitive << " "
       << l.thicknessSensitive << " " << l.widthInnerSensitive << " " << l.widthOuterSensitive << " "
       << l.lengthSensitive << std::endl;
  }
...
std::ostream& operator<<( std::ostream& io, const ConicalSupportData& d ) {
  ...
  io << " Sections : " << std::endl << "rInner rOuter zPos " << std::endl;

  for ( unsigned i = 0, N = layers.size(); i < N; ++i ) {
    ConicalSupportData::Section l = layers[ i ];

    io << " " << l.rInner << " " << l.rOuter << " " << l.zPos << std::endl;
  }
  return io;
}
...
std::ostream& operator<<( std::ostream& io, const LayeredCalorimeterData& d ) {
  ...
  io << "  LayoutType : "
     << ( d.layoutType == LayeredCalorimeterStruct::BarrelLayout ? "BarrelLayout" : "EndcapLayout" ) << std::endl;
  io << "  extent[ rmin, rmax, zmin, zmax ] : " << d.extent[ 0 ] << " " << d.extent[ 1 ] << " " << d.extent[ 2 ] << " "
     << d.extent[ 3 ] << std::endl;
  ...
  for ( unsigned i = 0, N = layers.size(); i < N; ++i ) {
    ...
    io << " " << l.distance << " " << l.inner_nRadiationLengths << " " << l.outer_nRadiationLengths << " "
       << l.inner_nInteractionLengths << " " << l.outer_nInteractionLengths << " " << l.inner_thickness << " "
       << l.outer_thickness << " " << l.sensitive_thickness << std::endl;
  }
