...
static long createGearForCLIC( LCDD& lcdd, int /*argc*/, char** /*argv*/ ) {
  ...
  try {
    ...
    handle->addMaterial( "VXDSupportMaterial", 2.075865162e+01, 1.039383117e+01, 2.765900000e+02, 1.014262421e+03,
                         3.341388059e+03 );
    ...
    //
    // 	handle->addMaterial( "VXDSupportMaterial", mat.A(), mat.Z() , mat.density()/(dd4hep::kg/(dd4hep::g*dd4hep::m3))
      ...
      gear::CalorimeterParametersImpl* gearCalo =
          ( calo->layoutType == LayeredCalorimeterData::BarrelLayout
                ? new gear::CalorimeterParametersImpl( calo->extent[ 0 ] / dd4hep::mm, calo->extent[ 3 ] / dd4hep::mm,
                                                       calo->inner_symmetry, calo->inner_phi0 )
                :
      ...
      for ( unsigned i = 0, nL = calo->layers.size(); i < nL; ++i ) {
        ...
        if ( i == 0 ) {
          ...
          gearCalo->layerLayout().addLayer(
              ( l.inner_thickness + l.outer_thickness + calo->layers[ i - 1 ].outer_thickness -
                calo->layers[ i - 1 ].sensitive_thickness / 2. ) /
                  dd4hep::mm,
              l.cellSize0 / dd4hep::mm, l.cellSize1 / dd4hep::mm,
              ( l.inner_thickness + l.outer_thickness - l.sensitive_thickness + calo->layers[ i - 1 ].outer_thickness -
                calo->layers[ i - 1 ].sensitive_thickness / 2. ) /
                  dd4hep::mm );
        } else {
