...
static long createGearForILD( LCDD& lcdd, int /*argc*/, char** /*argv*/ ) {
  ...
  try {
    crossing_angle = lcdd.constant<double>( "ILC_Main_Crossing_Angle" );
  ...
  try {
    ...
    gear::TPCParametersImpl* gearTPC =
        new gear::TPCParametersImpl( tpc->driftLength / dd4hep::mm, gear::PadRowLayout2D::POLAR );

    gearTPC->setPadLayout( new gear::FixedPadSizeDiskLayout(
        tpc->rMinReadout / dd4hep::mm, tpc->rMaxReadout / dd4hep::mm, tpc->padHeight / dd4hep::mm,
  ...
  try {
    ...
    //      ZPlanarParametersImpl (int type, double shellInnerRadius, double shellOuterRadius, double shellHalfLength,
    //      double shellGap, double shellRadLength)
    int                          vxdType = gear::ZPlanarParameters::CMOS;
    gear::ZPlanarParametersImpl* gearVXD =
    ...
    for ( unsigned i = 0, n = vxd->layers.size(); i < n; ++i ) {
      ...
      gearVXD->addLayer( l.ladderNumber, l.phi0, l.distanceSupport / dd4hep::mm, l.offsetSupport / dd4hep::mm,
                         l.thicknessSupport / dd4hep::mm, l.zHalfSupport / dd4hep::mm, l.widthSupport / dd4hep::mm, 0.,
                         l.distanceSensitive / dd4hep::mm, l.offsetSensitive / dd4hep::mm,
                         l.thicknessSensitive / dd4hep::mm, l.zHalfSensitive / dd4hep::mm,
                         l.widthSensitive / dd4hep::mm, 0. );
    }

    GearHandle* handle = new GearHandle( gearVXD, "VXDParameters" );

    // quick hack for now: add the one material that is needed by KalDet :
    //      handle->addMaterial( "VXDSupportMaterial", 2.075865162e+01, 1.039383117e+01, 2.765900000e+02,
    //      1.014262421e+03, 3.341388059e+03)  ;
    ...
    handle->addMaterial( "VXDSupportMaterial", mat.A(), mat.Z(),
                         mat.density() / ( dd4hep::kg / ( dd4hep::g * dd4hep::m3 ) ),
                         mat.radiationLength() / dd4hep::mm, mat.interactionLength() / dd4hep::mm );
  ...
  try {
    ...
    gear::ZPlanarParametersImpl* gearSIT =
    ...
    for ( unsigned i = 0, n = sit->layers.size(); i < n; ++i ) {
      ...
      // FIXME set rad lengths to 0 -> need to get from DD4hep ....
      gearSIT->addLayer( l.ladderNumber, l.phi0, l.distanceSupport / dd4hep::mm, l.offsetSupport / dd4hep::mm,
                         l.thicknessSupport / dd4hep::mm, l.zHalfSupport / dd4hep::mm, l.widthSupport / dd4hep::mm, 0.,
                         l.distanceSensitive / dd4hep::mm, l.offsetSensitive / dd4hep::mm,
                         l.thicknessSensitive / dd4hep::mm, l.zHalfSensitive / dd4hep::mm,
                         l.widthSensitive / dd4hep::mm, 0. );
  ...
  try {
    ...
    gear::ZPlanarParametersImpl* gearSET =
    ...
    for ( unsigned i = 0, n = set->layers.size(); i < n; ++i ) {
      ...
      // FIXME set rad lengths to 0 -> need to get from DD4hep ....
      gearSET->addLayer( l.ladderNumber, l.phi0, l.distanceSupport / dd4hep::mm, l.offsetSupport / dd4hep::mm,
                         l.thicknessSupport / dd4hep::mm, l.zHalfSupport / dd4hep::mm, l.widthSupport / dd4hep::mm, 0.,
                         l.distanceSensitive / dd4hep::mm, l.offsetSensitive / dd4hep::mm,
                         l.thicknessSensitive / dd4hep::mm, l.zHalfSensitive / dd4hep::mm,
                         l.widthSensitive / dd4hep::mm, 0. );
  ...
  try {
    ...
    for ( unsigned i = 0, n = ftd->layers.size(); i < n; ++i ) {
      ...
      gearFTD->addLayer(
          l.petalNumber, l.sensorsPerPetal, isDoubleSided, sensorType, l.petalHalfAngle, l.phi0, l.alphaPetal,
          l.zPosition / dd4hep::mm, zoffset / dd4hep::mm, signoffset, l.distanceSupport / dd4hep::mm,
          l.thicknessSupport / dd4hep::mm, l.widthInnerSupport / dd4hep::mm, l.widthOuterSupport / dd4hep::mm,
          l.lengthSupport / dd4hep::mm, 0., l.distanceSensitive / dd4hep::mm, l.thicknessSensitive / dd4hep::mm,
          l.widthInnerSensitive / dd4hep::mm, l.widthOuterSensitive / dd4hep::mm, l.lengthSensitive / dd4hep::mm, 0. );
    ...
    try {
      ...
      gear::CalorimeterParametersImpl* gearCalo =
          ( calo->layoutType == LayeredCalorimeterData::BarrelLayout
                ? new gear::CalorimeterParametersImpl( calo->extent[ 0 ] / dd4hep::mm, calo->extent[ 3 ] / dd4hep::mm,
                                                       calo->inner_symmetry, calo->phi0 )
                :
                // CalorimeterParametersImpl (double rMin, double zMax, int symOrder=8, double phi0=0.0) - C'tor for a
                // cylindrical (octagonal) BARREL calorimeter.
                new gear::CalorimeterParametersImpl( calo->extent[ 0 ] / dd4hep::mm, calo->extent[ 1 ] / dd4hep::mm,
                                                     calo->extent[ 2 ] / dd4hep::mm, calo->outer_symmetry,
                                                     calo->phi0 ) );
      // CalorimeterParametersImpl (double rMin, double rMax, double zMin, int symOrder=2, double phi0=0.0) - C'tor for
      // a cylindrical (octagonal) ENDCAP calorimeter.
      ...
      for ( unsigned i = 0, nL = calo->layers.size(); i < nL; ++i ) {
        ...
        // The positioning should come out right, but the absorber thickness should be overestimated due to the presence
        // of
        ...
        if ( i == 0 ) {
          ...
          gearCalo->layerLayout().addLayer(
              ( l.inner_thickness + l.sensitive_thickness / 2. + calo->layers[ i - 1 ].outer_thickness -
                calo->layers[ i - 1 ].sensitive_thickness / 2. ) /
                  dd4hep::mm,
              l.cellSize0 / dd4hep::mm, l.cellSize1 / dd4hep::mm,
              ( l.inner_thickness - l.sensitive_thickness / 2. + calo->layers[ i - 1 ].outer_thickness -
                calo->layers[ i - 1 ].sensitive_thickness / 2. ) /
      ...
      if ( it->first == "BeamCal" ) {
        try {
          ...
          DDSegmentation::DoubleParameter oPPar =
              dynamic_cast<DDSegmentation::DoubleParameter>( seg.parameter( "offset_phi" ) );
  ...
  // 	( calo2.isBarrel()  ?
  // 	  new gear::CalorimeterParametersImpl(  calo2.getRMin()/dd4hep::mm,
  // calo2.getZMax()/dd4hep::mm, calo2.getNSides(),  0. )  :    // fixme: phi 0  is not defined ??
  // 	  new gear::CalorimeterParametersImpl(  calo2.getRMin()/dd4hep::mm, calo2.getRMax()/dd4hep::mm,
  // calo2.getZMin()/dd4hep::mm, calo2.getNSides(),  0. )
  // 	  ) ;

  // for( unsigned i=0, nL = calo2.numberOfLayers() ; i <nL ; ++i ){

  // 	if( i == 0 ) {
  // 	  gearCalo2->layerLayout().positionLayer( calo2.getRMin()/dd4hep::mm, calo2.thickness(i)/dd4hep::mm ,  0.
  // /dd4hep::mm,   0. /dd4hep::mm, calo2.absorberThickness(i)/dd4hep::mm ) ;

  // 	}else{                                                                                        //     fixme:
  // cell sizes  not in API !?

  // 	  gearCalo2->layerLayout().addLayer(                                  calo2.thickness(i)/dd4hep::mm ,  0.
  // /dd4hep::mm,   0. /dd4hep::mm, calo2.absorberThickness(i)/dd4hep::mm ) ;
  // 	}
