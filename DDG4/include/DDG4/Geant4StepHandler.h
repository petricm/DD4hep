...
namespace DD4hep {
  ...
  namespace Simulation {
    ...
    public:
      ...
      int                 trkStatus() const { return track->GetTrackStatus(); }
      bool                trkAlive() const { return track->GetTrackStatus() == fAlive; }
      const G4VProcess*   trkProcess() const { return track->GetCreatorProcess(); }
      ...
      G4VPhysicalVolume* volume(const G4StepPoint* p) const { return p->GetTouchableHandle()->GetVolume(); }
      G4VSolid* solid(const G4StepPoint* p) const { return p->GetTouchableHandle()->GetSolid(); }
      G4VPhysicalVolume* physvol(const G4StepPoint* p) const { return p->GetPhysicalVolume(); }
      ...
      bool isSensitive(const G4LogicalVolume* lv) const { return lv ? (0 != lv->GetSensitiveDetector()) : false; }
      bool isSensitive(const G4VPhysicalVolume* pv) const { return pv ? isSensitive(pv->GetLogicalVolume()) : false; }
      bool isSensitive(const G4StepPoint* point) const { return point ? isSensitive(volume(point)) : false; }
