...
namespace {
  ...
  template <typename P>
  class Factory<P, long(DD4hep::Geometry::LCDD*, const DD4hep::Simulation::Geant4Converter*,
  ...
  public:
    ...
    static void Func(void* retaddr, void*, const std::vector<void*>& arg, void*) {
      *(void**)retaddr = DD4hep::Simulation::Geant4SensitiveDetectorFactory<P>::create(*(std::string*)arg[0],
                                                                                       *(DD4hep::Geometry::LCDD*)arg[1]);
    }
  ...
  public:
    typedef DD4hep::Simulation::G4SDFactory SD;
    static void Func(void* retaddr, void*, const std::vector<void*>& arg, void*) { *(SD**)retaddr = (SD*)new P(); }
  };

  /// Factory to create Geant4 sensitive detectors
  template <typename P>
  class Factory<P, DD4hep::Simulation::Geant4Sensitive*(DD4hep::Simulation::Geant4Context*, std::string,
                                                        DD4hep::Geometry::DetElement*, DD4hep::Geometry::LCDD*)> {
  public:
    typedef DD4hep::Simulation::Geant4Sensitive _S;
    static void Func(void* retaddr, void*, const std::vector<void*>& arg, void*) {
      *(_S**)retaddr = (_S*)new P((DD4hep::Simulation::Geant4Context*)arg[0], *(std::string*)arg[1],
                                  *(DD4hep::Geometry::DetElement*)arg[2], *(DD4hep::Geometry::LCDD*)arg[3]);
    }
  };

  /// Factory to create Geant4 sensitive detectors
  template <typename P>
  class Factory<P, DD4hep::Simulation::Geant4Action*(DD4hep::Simulation::Geant4Context*, std::string)> {
  public:
    typedef DD4hep::Simulation::Geant4Action ACT;
    static void Func(void* retaddr, void*, const std::vector<void*>& arg, void*) {
  ...
  public:
    static void Func(void* ret, void*, const std::vector<void*>&, void*) {
      P::ConstructParticle();
      *(long*)ret = 1L;
    }
  };
...
#define DECLARE_EXTERNAL_GEANT4SENSITIVEDETECTOR(name, func)                                                               \
  ...
  PLUGINSVC_FACTORY_WITH_ID(external_geant4_sd_##name, std::string(#name),                                                 \
                            G4VSensitiveDetector*(std::string, DD4hep::Geometry::LCDD*))
...
#define DECLARE_GEANT4SENSITIVEDETECTOR(name)                                                                     \
  ...
  PLUGINSVC_FACTORY_WITH_ID(geant4_sd_##name, std::string(#name),                                                 \
                            G4VSensitiveDetector*(std::string, DD4hep::Geometry::LCDD*))
#if 0 /* Equivalent:  */
#define DECLARE_GEANT4SENSITIVEDETECTOR(name)                                                             \
  namespace DD4hep {                                                                                      \
    namespace Simulation {                                                                                \
      static G4VSensitiveDetector* __sd_create__##name(const std::string& n, DD4hep::Geometry::LCDD& p) { \
        return new name(n, p);                                                                            \
      }                                                                                                   \
    }                                                                                                     \
  }                                                                                                       \
...
#define DECLARE_GEANT4SENSITIVE(name)                                                                             \
  ...
  PLUGINSVC_FACTORY_WITH_ID(name, std::string(#name),                                                             \
...
#define DECLARE_GEANT4ACTION(name)                    \
  ...
  using DD4hep::Simulation::name;                     \
  PLUGINSVC_FACTORY_WITH_ID(name, std::string(#name), \
                            DD4hep::Simulation::Geant4Action*(DD4hep::Simulation::Geant4Context*, std::string))
/// Plugin definition to create Geant4 stepper objects
#define DECLARE_GEANT4_STEPPER(name) \
  PLUGINSVC_FACTORY_WITH_ID(G4##name, std::string(#name), G4MagIntegratorStepper*(G4EquationOfMotion*))
#define DECLARE_GEANT4_MAGSTEPPER(name) \
  PLUGINSVC_FACTORY_WITH_ID(G4##name, std::string(#name), G4MagIntegratorStepper*(G4Mag_EqRhs*))
/// Plugin definition to create Geant4 equations of motion for magnetic fields
#define DECLARE_GEANT4_MAGMOTION(name) \
  PLUGINSVC_FACTORY_WITH_ID(G4##name, std::string(#name), G4Mag_EqRhs*(G4MagneticField*))
  ...
  namespace DD4hep {                                                                                            \
    namespace Simulation {                                                                                      \
      ...
      long Geant4SetupAction<DD4hep::Simulation::xml_g4_setup_##name>::create(                                  \
          LCDD& l, const DD4hep::Simulation::Geant4Converter& e, const std::map<std::string, std::string>& a) { \
        return func(l, e, a);                                                                                   \
      }                                                                                                         \
    }                                                                                                           \
  }                                                                                                             \
