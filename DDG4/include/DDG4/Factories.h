...
template <typename T>
class Geant4SetupAction : public PluginFactoryBase {
 public:
...
template <typename T>
class Geant4SensitiveDetectorFactory : public PluginFactoryBase {
 public:
...
#define DECLARE_GEANT4_STEPPER( name ) \
  DD4HEP_PLUGINSVC_FACTORY( G4##name, name, _ns::Stepper*(G4EquationOfMotion*), __LINE__ )
#define DECLARE_GEANT4_MAGSTEPPER( name ) \
  DD4HEP_PLUGINSVC_FACTORY( G4##name, name, _ns::Stepper*(G4Mag_EqRhs*), __LINE__ )
/// Plugin definition to create Geant4 equations of motion for magnetic fields
#define DECLARE_GEANT4_MAGMOTION( name ) \
  DD4HEP_PLUGINSVC_FACTORY( G4##name, name, G4Mag_EqRhs*(G4MagneticField*), __LINE__ )
...
#define DECLARE_GEANT4_PHYSICS_LIST( name )                       \
  ...
  DD4HEP_PLUGINSVC_FACTORY( G4_physics_list_##name, name,         \
                            G4VUserPhysicsList*(DS::Geant4PhysicsListActionSequence*, int), __LINE__ )
/// Plugin defintion for setup actions
#define DECLARE_GEANT4_SETUP( name, func )                                                                      \
  DD4HEP_OPEN_PLUGIN( DD4hep, xml_g4_setup_##name ) {                                                           \
    template <>                                                                                                 \
    long Geant4SetupAction<xml_g4_setup_##name>::create( _ns::LCDD& l, const _ns::GH& e, const _ns::STRM& a ) { \
      return func( l, e, a );                                                                                   \
    }                                                                                                           \
    DD4HEP_PLUGINSVC_FACTORY( xml_g4_setup_##name, name##_Geant4_action,                                        \
                              long( _ns::LCDD*, const _ns::GH*, const _ns::STRM* ), __LINE__ )                  \
  }
