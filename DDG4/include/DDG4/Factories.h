...
namespace {
  ...
  DD4HEP_PLUGIN_FACTORY_ARGS_2(DS::Geant4Action*, _ns::CT*, std::string) { return new P(a0, a1); }

  /// Factory to create Geant4 equations of motion for magnetic fields
  DD4HEP_PLUGIN_FACTORY_ARGS_1(G4Mag_EqRhs*, G4MagneticField*) { return new P(a0); }

  /// Factory to create Geant4 steppers
  DD4HEP_PLUGIN_FACTORY_ARGS_1(_ns::Stepper*, G4EquationOfMotion*) { return new P(a0); }

  /// Factory to create Geant4 steppers
  DD4HEP_PLUGIN_FACTORY_ARGS_1(_ns::Stepper*, G4Mag_EqRhs*) { return new P(a0); }

  /// Factory to create Geant4 Processes
  DD4HEP_PLUGIN_FACTORY_ARGS_0(G4VProcess*) { return (G4VProcess*)new P(); }

  /// Factory to create Geant4 Physics objects
  DD4HEP_PLUGIN_FACTORY_ARGS_0(G4VPhysicsConstructor*) { return new P(); }

  /// Factory to access Geant4 Particle definitions
  DD4HEP_PLUGIN_FACTORY_ARGS_0(G4ParticleDefinition*) { return P::Definition(); }

  /// Factory to create Geant4 primary generator objects
  DD4HEP_PLUGIN_FACTORY_ARGS_0(G4VPrimaryGenerator*) { return new P(); }
  ...
  DD4HEP_PLUGIN_FACTORY_ARGS_2(G4VUserPhysicsList*, DS::Geant4PhysicsListActionSequence*, int) {
    DD4hep::printout(DD4hep::INFO, "PhysicsList", "+++ Create physics list of type:%s", DD4hep::typeName(typeid(P)).c_str());
    return new P(a0, a1);
  }

  /// Factory template to create Geant4 event reader objects
  DD4HEP_PLUGIN_FACTORY_ARGS_1(_ns::RDR*, std::string) { return new P(a0); }
}

#define __IMPLEMENT_GEANT4SENSDET(name, func)                                                                      \
  DD4HEP_OPEN_PLUGIN(DD4hep, geant4_sd_##name) {                                                                   \
    template <>                                                                                                    \
    G4VSensitiveDetector* Geant4SensitiveDetectorFactory<geant4_sd_##name>::create(const str_t& n, _ns::LCDD& l) { \
      return func(n, l);                                                                                           \
    }                                                                                                              \
...
#define DECLARE_GEANT4_PHYSICS_LIST(name)                                                                                \
  ...
  DD4HEP_PLUGINSVC_FACTORY(G4_physics_list_##name, name, G4VUserPhysicsList*(DS::Geant4PhysicsListActionSequence*, int), \
                           __LINE__)
/// Plugin defintion for setup actions
#define DECLARE_GEANT4_SETUP(name, func)                                                                                    \
  DD4HEP_OPEN_PLUGIN(DD4hep, xml_g4_setup_##name) {                                                                         \
    template <> long Geant4SetupAction<xml_g4_setup_##name>::create(_ns::LCDD& l, const _ns::GH& e, const _ns::STRM& a) {   \
      return func(l, e, a);                                                                                                 \
    }                                                                                                                       \
    DD4HEP_PLUGINSVC_FACTORY(xml_g4_setup_##name, name##_Geant4_action, long(_ns::LCDD*, const _ns::GH*, const _ns::STRM*), \
                             __LINE__)                                                                                      \
  }
