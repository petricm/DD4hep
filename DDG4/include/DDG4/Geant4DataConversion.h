...
namespace DD4hep {
  ...
  namespace Simulation {
    ...
    Geant4Conversion<OUTPUT, ARGS>::Geant4Conversion() : Geant4ConversionHelper() {}

    template <typename OUTPUT, typename ARGS> Geant4Conversion<OUTPUT, ARGS>::~Geant4Conversion() {}
    ...
    template <typename OUTPUT, typename ARGS>
    const Geant4Conversion<OUTPUT, ARGS>& Geant4Conversion<OUTPUT, ARGS>::converter(const std::type_info& typ) {
...
#define DECLARE_GEANT4_HITCONVERTER(output_type, args_type, tag) \
  DECLARE_GEANT4_DATACONVERTER(output_type, args_type, tag, __LINE__)
