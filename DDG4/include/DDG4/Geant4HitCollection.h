  ...
  namespace Simulation {
    ...
    public:
      ...
      template <typename TYPE> operator TYPE*() const {
        return (TYPE*)m_data.second->cast.apply_dynCast(ComponentCast::instance<TYPE>(), m_data.first);
    ...
    public:
      ...
      void setOptimize(int flag) { m_flags.value |= flag; }
      /// Set the sensitive detector
      void setSensitive(Geant4Sensitive* detector) { m_detector = detector; }
      /// Access the sensitive detector
      Geant4Sensitive* sensitive() const { return m_detector; }
      ...
      template <typename TYPE> TYPE* find(const Compare& cmp) { return (TYPE*)findHit(cmp); }
      /// Release all hits from the Geant4 container and pass ownership to the caller
    ...
    template <typename TYPE, typename POS> void* PositionCompare<TYPE, POS>::operator()(const Geant4HitWrapper& w) const {
      TYPE* h = w;
    ...
    template <typename TYPE> void* CellIDCompare<TYPE>::operator()(const Geant4HitWrapper& w) const {
      TYPE* h = w;
      if (id == h->cellID)
        return h;
      return 0;
