...
template <class SD>
DD4hep::Simulation::Geant4GenericSD<SD>::Geant4GenericSD( const std::string& nam, LCDD& lcdd_ref )
...
template <class SD>
bool DD4hep::Simulation::Geant4GenericSD<SD>::defineCollection( const std::string& coll_name ) {
 ...
 */
template <class SD>
void DD4hep::Simulation::Geant4GenericSD<SD>::Initialize( G4HCofThisEvent* HCE ) {
  this->Geant4SensitiveDetector::Initialize( HCE );
}

/// Method invoked at the end of each event.
template <class SD>
void DD4hep::Simulation::Geant4GenericSD<SD>::EndOfEvent( G4HCofThisEvent* HCE ) {
  this->Geant4SensitiveDetector::EndOfEvent( HCE );
}

/// Method invoked if the event was aborted.
template <class SD>
void DD4hep::Simulation::Geant4GenericSD<SD>::clear() {
  this->Geant4SensitiveDetector::clear();
}

/// Method for generating hit(s) using the information of G4Step object.
template <class SD>
G4bool DD4hep::Simulation::Geant4GenericSD<SD>::ProcessHits( G4Step* step, G4TouchableHistory* history ) {
  return this->Geant4SensitiveDetector::ProcessHits( step, history );
}

/// Method for generating hit(s) using the information of G4Step object.
template <class SD>
bool DD4hep::Simulation::Geant4GenericSD<SD>::buildHits( G4Step*, G4TouchableHistory* ) {
  return true;
}
