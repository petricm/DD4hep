...
namespace DD4hep {
  ...
  namespace Simulation {
    ...
    public:
      ...
      template <typename Q, typename T> void build(Q* p, void (T::*f)()) { m_workerCalls.add(p, f); }
      /// Register callback to setup master. Types Q and T must be polymorph!
      template <typename Q, typename T> void buildMaster(Q* p, void (T::*f)()) { m_masterCalls.add(p, f); }
