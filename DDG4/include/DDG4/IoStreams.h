...
namespace DD4hep {
  ...
  public:
    ...
    /// Copy constructor
    dd4hep_file_source(const dd4hep_file_source<T>& other) : descriptor(other) {}

    /// Constructors taking file desciptors
    explicit dd4hep_file_source(handle_type h, dd4hep_file_flags flags) : descriptor(h, flags) {}

    /// Constructors taking file desciptors
    explicit dd4hep_file_source(const char* name, BOOST_IOS::openmode mode = BOOST_IOS::in) : descriptor(name, mode) {}

    /// open overload taking file desciptors
    void open(handle_type h, dd4hep_file_flags flags) { this->descriptor::open(h, flags); }

    /// open overload taking C-style string
    void open(const char* path, BOOST_IOS::openmode mode = BOOST_IOS::in) { this->descriptor::open(path, mode); }

    /// open overload taking a std::string
    void open(const std::string& path, BOOST_IOS::openmode mode = BOOST_IOS::in) { open(path.c_str(), mode); }
   ...
   *  \see http://www.boost.org/libs/iostreams for further documentation.
   */
  template <typename T> class dd4hep_file_sink : private dd4hep_file<T> {
  public:
    typedef dd4hep_file<T> descriptor;
    struct category : boost::iostreams::output_seekable, boost::iostreams::device_tag, boost::iostreams::closable_tag {};
    typedef typename descriptor::handle_type handle_type;
    typedef typename descriptor::char_type   char_type;
    using descriptor::is_open;
    using descriptor::close;
    ...
    /// Copy constructor
    dd4hep_file_sink(const dd4hep_file_sink<T>& other) : descriptor(other) {}

    /// Constructors taking file desciptors
    explicit dd4hep_file_sink(handle_type fd, dd4hep_file_flags flags) : descriptor(fd, flags) {}
    ...
    explicit dd4hep_file_sink(const char* path, BOOST_IOS::openmode mode = BOOST_IOS::out) : descriptor(path, mode) {}
    ...
    /// open overloads taking file descriptors
    void open(handle_type fd, dd4hep_file_flags flags) { this->descriptor::open(fd, flags); }

    /// open overload taking a std::string
    void open(const std::string& path, BOOST_IOS::openmode mode = BOOST_IOS::out) { open(path.c_str(), mode); }

    /// open overload taking C-style string
    void open(const char* path, BOOST_IOS::openmode mode = BOOST_IOS::out) { this->descriptor::open(path, mode); }
