...
enum Geant4ParticleProperties {
  ...
  ( G4PARTICLE_GEN_EMPTY + G4PARTICLE_GEN_STABLE + G4PARTICLE_GEN_DECAYED + G4PARTICLE_GEN_DOCUMENTATION ),
  G4PARTICLE_GEN_STATUS = 0x3FF,  // Mask for generator status (bit 0...9)

  // Simulation status of a given particle
  G4PARTICLE_SIM_CREATED =
      1 << 10,  // True if the particle has been created by the simulation program (rather than the generator)
  G4PARTICLE_SIM_BACKSCATTER =
      1 << 11,  // True if the particle is the result of a backscatter from a calorimeter shower.
  ...
  G4PARTICLE_SIM_OVERLAY =
      1 << 17,  // True if the particle has been overlayed by the simulation (or digitization)  program.
...
class Geant4ParticleHandle {
 ...
 public:
  ...
  double momentum() const {
    return std::sqrt( momentum2() );
  }
  /// Geant4 charge of the particle
  double charge() const {
    return double( particle->charge );
  }
...
inline Geant4ParticleHandle::Geant4ParticleHandle( const Geant4ParticleHandle& c ) : particle( c.particle ) {
}

/// Initializing constructor
inline Geant4ParticleHandle::Geant4ParticleHandle( Geant4Particle* p ) : particle( p ) {
}
