...
Geant4SurfaceTest::Geant4SurfaceTest( Geant4Context* ctxt, const std::string& nam ) : Geant4EventAction( ctxt, nam ) {
...
void Geant4SurfaceTest::end( const G4Event* evt ) {
  ...
  for ( int j = 0, nCol = hce->GetNumberOfCollections(); j < nCol; ++j ) {
    ...
    if ( !sd.isValid() ) {
      error( "Event:%d: Collection:%s Invalid sensitive detector.", evt->GetEventID(), hc->GetName().c_str() );
      continue;
    }
    IDDescriptor idDesc = sd.readout().idSpec();
    const string idStr  = idDesc.fieldDescription();
    BitField64   idDecoder( idStr );
    info( "Event: %d [%s: %d hits] Desc:%s", evt->GetEventID(), hc->GetName().c_str(), coll->GetSize(), idStr.c_str() );
    for ( size_t i = 0, nHit = coll->GetSize(); i < nHit; ++i ) {
      ...
      string   flag     = "";
      if ( h->flag & Geant4Tracker::Hit::HIT_SECONDARY_TRACK )
        flag += " TRK:SECONDARY";
      if ( h->flag & Geant4Tracker::Hit::HIT_KILLED_TRACK )
        flag += " TRK:KILLED";
      if ( h->flag & Geant4Tracker::Hit::HIT_NEW_TRACK )
        flag += " NEW_TRACK";
      if ( h->flag & Geant4Tracker::Hit::HIT_STARTED_OUTSIDE )
        flag += " START:OUTSIDE";
      if ( h->flag & Geant4Tracker::Hit::HIT_STARTED_INSIDE )
        flag += " START:INSIDE";
      if ( h->flag & Geant4Tracker::Hit::HIT_STARTED_SURFACE )
        flag += " START:SURFACE";
      if ( h->flag & Geant4Tracker::Hit::HIT_ENDED_OUTSIDE )
        flag += " END:OUTSIDE";
      if ( h->flag & Geant4Tracker::Hit::HIT_ENDED_INSIDE )
        flag += " END:INSIDE";
      if ( h->flag & Geant4Tracker::Hit::HIT_ENDED_SURFACE )
        flag += " END:SURFACE";

      sst.str( "" );
      if ( isInside ) {
        sst << hc->GetName() << " "
            << "PASSED: Track:" << trackID << " Hit:" << i
            //<< " Point " << hit_point
            << " is ON SURFACE. "
            << "(FLAG:" << h->flag << flag << ").";
        print( sst.str().c_str() );
        ...
        sst.str( "" );
        sst << hc->GetName() << " FAILED: Track:" << trackID << " Hit:" << i << " Point: " << hit_point;
        error( sst.str().c_str() );
        // sst.str("");
        // sst <<   -> Surface:" <<  *surf;
        // error(sst.str().c_str());
