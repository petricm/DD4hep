...
struct TrackerCombine {
  ...
  Geant4TrackerHit* extractHit( Geant4SensitiveDetector::HitCollection* c ) {
    ...
    Geant4TrackerHit* hit =
        new Geant4TrackerHit( pre.truth.trackID, pre.truth.pdgID, pre.truth.deposit, pre.truth.time );
    hit->cellID   = cellID;
    hit->position = pos;
    hit->momentum = mom;
    hit->length   = path_len;
...
template <>
void Geant4GenericSD<TrackerCombine>::Initialize( G4HCofThisEvent* HCE ) {
...
template <>
void Geant4GenericSD<TrackerCombine>::clear() {
  userData.clear();
...
/// Method for generating hit(s) using the information of G4Step object.
template <>
G4bool Geant4GenericSD<TrackerCombine>::ProcessHits( G4Step* step, G4TouchableHistory* ) {
  StepHandler h( step );
