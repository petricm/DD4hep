...
int Geant4InputAction::readParticles( int evt_number, Vertex& prim_vertex, std::vector<Particle*>& particles ) {
    ...
    try {
      ...
      if ( 0 == m_reader ) {
        ...
        abortRun( issue( evid ) + "Error creating reader plugin.",
                  "Failed to create file reader of type %s. Cannot open dataset %s", tn.first.c_str(),
                  tn.second.c_str() );
...
void Geant4InputAction::operator()( G4Event* event ) {
  ...
  print( "+++ Particle interaction with %d generator particles ++++++++++++++++++++++++", int( primaries.size() ) );
