...
void Geant4AssemblyVolume::imprint( Geant4GeometryInfo& info, const TGeoNode* parent, Chain chain,
                                    Geant4AssemblyVolume* pAssembly, G4LogicalVolume* pMotherLV,
  ...
  for ( unsigned int i = 0; i < triplets.size(); i++ ) {
    ...
    if ( triplets[ i ].GetVolume() ) {
      ...
      pvName << "av_" << GetAssemblyID() << "_impr_" << GetImprintsCount() << "_"
             << triplets[ i ].GetVolume()->GetName().c_str() << "_pv_" << i << std::ends;

      // Generate a new physical volume instance inside a mother
      // (as we allow 3D transformation use G4ReflectionFactory to
      //  take into account eventual reflection)
      //
      G4PhysicalVolumesPair pvPlaced = G4ReflectionFactory::Instance()->Place(
          Tfinal, pvName.str().c_str(), triplets[ i ].GetVolume(), pMotherLV, false, numberOfDaughters + i, surfCheck );
      ...
      imprint( info, parent, new_chain, (Geant4AssemblyVolume*)triplets[ i ].GetAssembly(), pMotherLV, Tfinal,
               i * 100 + copyNumBase, surfCheck );
    } else {
      --level;
      G4Exception( "G4AssemblyVolume::MakeImprint(..)", "GeomVol0003", FatalException,
...
struct MyTransform3D : public G4Transform3D {
  MyTransform3D( double XX, double XY, double XZ, double DX, double YX, double YY, double YZ, double DY, double ZX,
                 double ZY, double ZZ, double DZ )
      : G4Transform3D( XX, XY, XZ, DX, YX, YY, YZ, DY, ZX, ZY, ZZ, DZ ) {
  }
  MyTransform3D( const double* t, const double* r = s_identity_rot )
      : G4Transform3D( r[ 0 ], r[ 1 ], r[ 2 ], t[ 0 ] * CM_2_MM, r[ 3 ], r[ 4 ], r[ 5 ], t[ 1 ] * CM_2_MM, r[ 6 ],
                       r[ 7 ], r[ 8 ], t[ 2 ] * CM_2_MM ) {
  }
};
...
Geant4Converter::Geant4Converter( LCDD& lcdd_ref ) : Geant4Mapping( lcdd_ref ), m_checkOverlaps( true ) {
  ...
  if ( !mat ) {
    ...
    if ( !mat ) {
      ...
      if ( material->IsMixture() ) {
        ...
        mat = new G4Material( name, density, nElements, state, material->GetTemperature(), material->GetPressure() );
        ...
        for ( int i = 0; i < nElements; ++i ) {
          ...
          if ( !g4e ) {
            printout( ERROR, "Material", "Missing component %s for material %s. A=%f W=%f", e->GetName(),
        ...
        if ( a < 0.5000001 )
          a = 1.0;
        mat = new G4Material( name, z, a, density, state, material->GetTemperature(), material->GetPressure() );
      }
      stringstream str;
      str << ( *mat );
      printout( m_outputLevel, "Geant4Converter", "++ Created G4 %s", str.str().c_str() );
...
void* Geant4Converter::handleSolid( const string& name, const TGeoShape* shape ) const {
  ...
  if ( shape ) {
    if ( 0 != ( solid = data().g4Solids[ shape ] ) ) {
      ...
      solid             = new G4Trd( name, s->GetDx1() * CM_2_MM, s->GetDx2() * CM_2_MM, s->GetDy1() * CM_2_MM,
                         s->GetDy2() * CM_2_MM, s->GetDz() * CM_2_MM );
      ...
      solid = new G4Cons( name, s->GetRmin1() * CM_2_MM, s->GetRmax1() * CM_2_MM, s->GetRmin2() * CM_2_MM,
                          s->GetRmax2() * CM_2_MM, s->GetDz() * CM_2_MM, s->GetPhi1() * DEGREE_2_RAD,
                          ( s->GetPhi2() - s->GetPhi1() ) * DEGREE_2_RAD );
      ...
      solid =
          new G4Trap( name, s->GetDz() * CM_2_MM, s->GetTheta(), s->GetPhi(), s->GetH1() * CM_2_MM,
                      s->GetBl1() * CM_2_MM, s->GetTl1() * CM_2_MM, s->GetAlpha1() * DEGREE_2_RAD, s->GetH2() * CM_2_MM,
                      s->GetBl2() * CM_2_MM, s->GetTl2() * CM_2_MM, s->GetAlpha2() * DEGREE_2_RAD );
      ...
      if ( strcmp( ls->ClassName(), "TGeoScaledShape" ) == 0 && strcmp( rs->ClassName(), "TGeoBBox" ) == 0 ) {
        if ( strcmp( ( (TGeoScaledShape*)ls )->GetShape()->ClassName(), "TGeoSphere" ) == 0 ) {
          if ( oper == TGeoBoolNode::kGeoIntersection ) {
            ...
            solid = new G4Ellipsoid( name, sx * radius * CM_2_MM, sy * radius * CM_2_MM, radius * CM_2_MM,
                                     zcut1 * CM_2_MM, zcut2 * CM_2_MM );
            data().g4Solids[ shape ] = solid;
            return solid;
          }
        }
...
void* Geant4Converter::handleVolume( const string& name, const TGeoVolume* volume ) const {
  ...
  if ( volIt == info.g4Volumes.end() ) {
    ...
    if ( lim.isValid() ) {
      ...
      if ( !user_limits ) {
        throw runtime_error( "G4Cnv::volume[" + name +
                             "]:    + FATAL Failed to "
    ...
    printout( lvl, "Geant4Converter", "++ Convert Volume %-32s: %p %s/%s assembly:%s", n.c_str(), v,
              s->IsA()->GetName(), v->IsA()->GetName(), yes_no( assembly ) );
    ...
    if ( region ) {
      printout( m_outputLevel, "Geant4Converter", "++ Volume     + Apply REGION settings: %s to volume %s.", reg.name(),
                _v.name() );
    ...
    for ( Int_t i = 0; i < mot_vol->GetNdaughters(); ++i ) {
      ...
      if ( dau_vol->IsA() == TGeoVolumeAssembly::Class() ) {
        ...
        if ( assIt == info.g4AssemblyVolumes.end() ) {
          printout( FATAL, "Geant4Converter", "+++ Invalid child assembly at %s : %d  parent: %s child:%s", __FILE__,
                    __LINE__, name.c_str(), d->GetName() );
        }
        g4->placeAssembly( d, ( *assIt ).second, transform );
        printout( m_outputLevel, "Geant4Converter",
                  "+++ Assembly: AddPlacedAssembly : dau:%s "
                  "to mother %s Tr:x=%8.3f y=%8.3f z=%8.3f",
                  dau_vol->GetName(), mot_vol->GetName(), transform.dx(), transform.dy(), transform.dz() );
      } else {
        Geant4GeometryMaps::VolumeMap::iterator volIt = info.g4Volumes.find( dau_vol );
        if ( volIt == info.g4Volumes.end() ) {
          printout( FATAL, "Geant4Converter", "+++ Invalid child volume at %s : %d  parent: %s child:%s", __FILE__,
                    __LINE__, name.c_str(), d->GetName() );
        }
        g4->placeVolume( d, ( *volIt ).second, transform );
        printout( m_outputLevel, "Geant4Converter",
                  "+++ Assembly: AddPlacedVolume : dau:%s "
                  "to mother %s Tr:x=%8.3f y=%8.3f z=%8.3f",
                  dau_vol->GetName(), mot_vol->GetName(), transform.dx(), transform.dy(), transform.dz() );
      }
    }
...
void* Geant4Converter::handlePlacement( const string& name, const TGeoNode* node ) const {
  ...
  if ( !g4 ) {
    ...
    if ( !tr ) {
      printout( FATAL, "Geant4Converter",
                "++ Attempt to handle placement without transformation:%p %s of type %s vol:%p", node, node->GetName(),
      ...
      if ( mother_is_assembly ) {
        ...
        printout( m_outputLevel, "Geant4Converter",
                  "+++ Assembly: **** : dau:%s "
                  "to mother %s Tr:x=%8.3f y=%8.3f z=%8.3f",
                  vol->GetName(), mot_vol->GetName(), transform.dx(), transform.dy(), transform.dz() );
        return 0;
        ...
        //
        printout( m_outputLevel, "Geant4Converter",
                  "+++ Assembly: makeImprint: dau:%s in mother %s "
                  "Tr:x=%8.3f y=%8.3f z=%8.3f",
                  node->GetName(), mot_vol->GetName(), transform.dx(), transform.dy(), transform.dz() );
    ...
    printout( ERROR, "Geant4Converter", "++ Attempt to DOUBLE-place physical volume: %s No:%d", name.c_str(),
              node->GetNumber() );
  }
  return g4;
}
...
void Geant4Converter::printSensitive( SensitiveDetector sens_det, const set<const TGeoVolume*>& /* volumes */ ) const {
  ...
  printout( INFO, "Geant4Converter", "++ SensitiveDetector: %-18s %-20s Hits:%-16s", sd.name(),
            ( "[" + sd.type() + "]" ).c_str(), sd.hitsCollection().c_str() );
  str << "                    | "
      << "Cutoff:" << setw( 6 ) << left << sd.energyCutoff() << setw( 5 ) << right << volset.size() << " volumes ";
  ...
  for ( ConstVolumeSet::iterator i = volset.begin(); i != volset.end(); ++i ) {
    ...
    str.str( "" );
    str << "                                   | "
        << "Volume:" << setw( 24 ) << left << vol->GetName() << " " << vol->GetNoDaughters() << " daughters.";
    printout( INFO, "Geant4Converter", str.str().c_str() );
  }
}
...
string printSolid( G4VSolid* sol ) {
  ...
  if ( typeid( *sol ) == typeid( G4Box ) ) {
    ...
    str << " Tubs: Ri=" << t->GetInnerRadius() << " Ra=" << t->GetOuterRadius() << " z/2=" << t->GetZHalfLength()
        << " Phi=" << t->GetStartPhiAngle() << "..." << t->GetDeltaPhiAngle();
  }
  return str.str();
}
...
void* Geant4Converter::printPlacement( const string& name, const TGeoNode* node ) const {
  ...
  stringstream str;
  str << "G4Cnv::placement: + " << name << " No:" << node->GetNumber() << " Vol:" << vol->GetName()
      << " Solid:" << sol->GetName();
  ...
  str << "                  |"
      ...
      << " Mat:" << vol->GetMaterial()->GetName() << " Mother:" << (char*)( mot ? mot->GetName().c_str() : "---" );
  printout( m_outputLevel, "G4Placement", str.str().c_str() );
  str.str( "" );
  str << "                  |"
      << " SD:" << sd->GetName();
