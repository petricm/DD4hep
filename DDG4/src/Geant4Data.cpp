...
/// Default constructor
SimpleRun::SimpleRun() : runID( -1 ), numEvents( 0 ) {
...
/// Default constructor
SimpleEvent::SimpleEvent() : runID( -1 ), eventID( -1 ) {
  InstanceCount::increment( this );
}
...
/// Default constructor
Geant4HitData::Geant4HitData() : cellID( 0 ), flag( 0 ), g4ID( 0 ), extension() {
  InstanceCount::increment( this );
}
...
Geant4HitData::Contribution Geant4HitData::extractContribution( const G4Step* step ) {
  ...
  double deposit = ( h.trackDef() == G4OpticalPhoton::OpticalPhotonDefinition() ) ? h.trkEnergy() : h.totalEnergy();
  const G4ThreeVector& pre   = h.prePosG4();
  const G4ThreeVector& post  = h.postPosG4();
  float                pos[] = {float( ( pre.x() + post.x() ) / 2.0 ), float( ( pre.y() + post.y() ) / 2.0 ),
                 float( ( pre.z() + post.z() ) / 2.0 )};
...
/// Default constructor
Geant4Tracker::Hit::Hit() : Geant4HitData(), position(), momentum(), length( 0.0 ), truth(), energyDeposit( 0.0 ) {
  InstanceCount::increment( this );
}
...
Geant4Tracker::Hit::Hit( int track_id, int pdg_id, double deposit, double time_stamp )
    : Geant4HitData(),
      ...
      momentum(),
      length( 0.0 ),
      truth( track_id, pdg_id, deposit, time_stamp ),
...
Geant4Calorimeter::Hit::Hit() : Geant4HitData(), position(), truth(), energyDeposit( 0 ) {
  InstanceCount::increment( this );
}

/// Standard constructor
Geant4Calorimeter::Hit::Hit( const Position& pos ) : Geant4HitData(), position( pos ), truth(), energyDeposit( 0 ) {
  InstanceCount::increment( this );
}
