...
void Geant4ParticleGenerator::printInteraction( Geant4PrimaryInteraction* inter ) const {
  ...
  for ( _V::const_iterator iv = inter->vertices.begin(); iv != inter->vertices.end(); ++iv ) {
    ...
    print( "+-> Interaction [%d] %.3f GeV %s pos:(%.3f %.3f %.3f)[mm]", count, m_energy / CLHEP::GeV,
           m_particleName.c_str(), v->x / CLHEP::mm, v->y / CLHEP::mm, v->z / CLHEP::mm );
    ++count;
...
void Geant4ParticleGenerator::operator()( G4Event* ) {
  ...
  for ( int i = 0; i < m_multiplicity; ++i ) {
    ...
    vtx->out.insert( p->id );
    printout( INFO, name(), "Particle [%d] %s %.3f GeV direction:(%6.3f %6.3f %6.3f)", p->id, m_particleName.c_str(),
              momentum / CLHEP::GeV, direction.X(), direction.Y(), direction.Z() );
  }
}
