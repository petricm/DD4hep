...
void Geant4DataDump::print( PrintLevel level, Geant4ParticleHandle p ) const {
  ...
  printout( level, m_tag, "   +++ TrackID: %6d %12d %6d %-7s %3s %5d %6s %8.3g %-4s %-7s %-7s %-3s", p->id, p->pdgID,
            parent, yes_no( mask.isSet( G4PARTICLE_PRIMARY ) ), yes_no( mask.isSet( G4PARTICLE_HAS_SECONDARIES ) ),
            int( p->daughters.size() ), yes_no( mask.isSet( G4PARTICLE_ABOVE_ENERGY_THRESHOLD ) ), p.energy(),
...
void Geant4DataDump::print( PrintLevel level, const std::string& container, const Particles* parts ) {
  if ( parts ) {
    printout( level, m_tag, "+++ Track container: %-21s --------------- Track KEEP reasoning ---------------",
              container.c_str() );
    printout( level, m_tag,
...
void Geant4DataDump::print( PrintLevel level, const TrackerHit* h ) {
  ...
  printout( level, m_tag,
            "   +++ Hit: Cell: %016llX Pos:(%9.3g,%9.3g,%9.3g) Len:%9.3g [mm] E:%9.3g MeV TrackID:%6d PDG:%12d "
            "dep:%9.3g time:%9.3g [ns]",
            h->cellID, h->position.x(), h->position.y(), h->position.z(), h->length, h->energyDeposit, t.trackID,
            t.pdgID, t.deposit, t.time );
}
...
void Geant4DataDump::print( PrintLevel level, const CalorimeterHit* h ) {
  ...
  for ( SimpleHit::Contributions::const_iterator i = t.begin(); i != t.end(); ++i, ++cnt ) {
    ...
    printout( PrintLevel( level - 1 ), m_tag, "       Contribution #%3d TrackID:%6d PDG:%12d %9.3g MeV %9.3g ns", cnt,
              c.trackID, c.pdgID, c.deposit, c.time );
  }
}
