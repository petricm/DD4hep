...
Geant4ParticleHandler::Geant4ParticleHandler( Geant4Context* ctxt, const string& nam )
  ...
  declareProperty( "MinDistToParentVertex",
                   m_minDistToParentVertex = 2.2e-14 * CLHEP::mm );  // default tolerance for g4ThreeVector isNear
  m_needsControl = true;
}
...
void Geant4ParticleHandler::end( const G4Track* track ) {
  ...
  if ( theLastPostStepPoint && ( theLastPostStepPoint->GetStepStatus() == fWorldBoundary  // particle left world volume
  ...
  if ( !mask.isNull() ) {
    ...
    if ( ip == m_particleMap.end() )
      part = m_particleMap[ g4_id ] = new Particle();
    else
      part = ( *ip ).second;
    ...
    for ( ip = m_particleMap.find( pid ); ip == m_particleMap.end(); ip = m_particleMap.find( pid ) ) {
      if ( ( iequiv = m_equivalentTracks.find( pid ) ) == iend )
        break;  // ERROR
      pid = ( *iequiv ).second;
    }
    if ( ip != m_particleMap.end() )
...
void Geant4ParticleHandler::rebaseSimulatedTracks( int ) {
  ...
  for ( TrackEquivalents::iterator ie = m_equivalentTracks.begin(), ie_end = m_equivalentTracks.end(); ie != ie_end;
        ++ie ) {
  ...
  for ( ParticleMap::const_iterator j, i = m_particleMap.begin(); i != m_particleMap.end(); ++i ) {
    ...
    if ( !mask.isSet( G4PARTICLE_PRIMARY ) && !status.anySet( G4PARTICLE_GEN_GENERATOR ) ) {
      ...
      if ( !in_map || !in_parent_list ) {
        ...
        error( "+++ Particle:%d Parent %d (G4id:%d)  In record:%s In parent list:%s [%s]", p->id, parent_id,
               p->g4Parent, yes_no( in_map ), yes_no( in_parent_list ), parent_list );
      }
    }
  }
