...
Geant4Particle* DD4hep::Simulation::createPrimary( int particle_id, const Geant4Vertex* v,
...
static void collectPrimaries( Geant4PrimaryMap* pm, Geant4PrimaryInteraction* interaction,
                              Geant4Vertex* particle_origine, G4PrimaryParticle* gp ) {
...
Geant4PrimaryInteraction* DD4hep::Simulation::createPrimary( int mask, Geant4PrimaryMap* pm,
...
static void appendInteraction( const Geant4Action* caller, Geant4PrimaryInteraction* output,
...
int DD4hep::Simulation::boostInteraction( const Geant4Action* caller, Geant4PrimaryEvent::Interaction* inter,
                                          double alpha ) {
#define SQR( x ) ( x * x )
...
int DD4hep::Simulation::smearInteraction( const Geant4Action* caller, Geant4PrimaryEvent::Interaction* inter, double dx,
...
int DD4hep::Simulation::generatePrimaries( const Geant4Action* caller, const Geant4Context* context, G4Event* event ) {
  ...
  if ( interaction->locked ) {
    ...
    for ( Interaction::VertexMap::const_iterator iend = vm.end(), i = vm.begin(); i != iend; ++i ) {
      ...
      caller->print( "+++++ G4PrimaryVertex at (%+.2e,%+.2e,%+.2e) [mm] %+.2e [ns]", v->x / CLHEP::mm, v->y / CLHEP::mm,
                     v->z / CLHEP::mm, v->time / CLHEP::ns );
