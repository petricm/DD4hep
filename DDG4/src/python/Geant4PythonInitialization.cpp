...
void Geant4PythonInitialization::build() const {
  info( "+++ Worker:%ld Build PYTHON Worker %s....", context()->kernel().id(),
        m_workerSetup.isValid() ? "[empty]" : "" );
...
void Geant4PythonInitialization::buildMaster() const {
  ...
  info( "+++ Build PYTHON Master [id:%ld] %s ....", context()->kernel().id(),
        m_masterSetup.isValid() ? "[empty]" : "" );
