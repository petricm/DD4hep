...
Geant4PythonDetectorConstruction::Geant4PythonDetectorConstruction( Geant4Context* ctxt, const string& nam )
    : Geant4DetectorConstruction( ctxt, nam ), m_constructSD(), m_constructFLD(), m_constructGEO() {
  m_needsControl = true;
}
...
void Geant4PythonDetectorConstruction::constructGeo( Geant4DetectorConstructionContext* ) {
  info( "+++ Worker:%ld Execute PYTHON constructGeo %s....", context()->kernel().id(),
        m_constructGEO.isValid() ? "" : "[empty]" );
...
void Geant4PythonDetectorConstruction::constructField( Geant4DetectorConstructionContext* ) {
  info( "+++ Worker:%ld Execute PYTHON constructField %s....", context()->kernel().id(),
        m_constructFLD.isValid() ? "" : "[empty]" );
...
void Geant4PythonDetectorConstruction::constructSensitives( Geant4DetectorConstructionContext* ) {
  info( "+++ Worker:%ld Execute PYTHON constructSensitives %s....", context()->kernel().id(),
        m_constructSD.isValid() ? "" : "[empty]" );
