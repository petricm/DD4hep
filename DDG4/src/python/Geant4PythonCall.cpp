...
Geant4PythonCall::Geant4PythonCall() : m_callable( 0 ), m_arguments( 0 ) {
  DDPython::instance();
}
...
RETURN Geant4PythonCall::execute() const {
  DDPython::GILState state( 0 );
...
/// Execute command in the python interpreter.
template <typename RETURN>
RETURN Geant4PythonCall::execute( PyObject* method ) const {
  DDPython::GILState state( 0 );
...
/// Execute command in the python interpreter.
template <typename RETURN>
RETURN Geant4PythonCall::execute( PyObject* method, PyObject* args ) const {
  DDPython::GILState state( 0 );
