...
Geant4Random::Geant4Random( Geant4Context* ctxt, const std::string& nam )
    : Geant4Action( ctxt, nam ), m_engine( 0 ), m_rootRandom( 0 ), m_rootOLD( 0 ), m_inited( false ) {
...
void Geant4Random::showStatus() const {
  ...
  if ( !m_file.empty() )
    ...
    printP2( "   Special instance created of type:%s @ 0x%p", m_engineType.c_str(), m_engine );
  else
    printP2( "   Reused HepRandom engine instance %s @ 0x%p", m_engine ? m_engine->name().c_str() : "???", m_engine );

  if ( m_engine == CLHEP::HepRandom::getTheEngine() )
    printP2( "   Instance is identical to Geant4's HepRandom instance." );

  printP2( "   Instance is %sidentical to ROOT's gRandom instance.", gRandom == m_rootRandom ? "" : "NOT " );
