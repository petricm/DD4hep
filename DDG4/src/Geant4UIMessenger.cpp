  ...
  if ( m_properties && i != m_propertyCmd.end() ) {
    ...
    try {
      if ( !v.empty() ) {
        ...
        p.str( v );
        printout( INFO, "Geant4UIMessenger", "+++ %s> Setting property value %s = %s  native:%s.", m_name.c_str(),
                  n.c_str(), v.c_str(), p.str().c_str() );
      } else {
      ...
      printout( INFO, "Geant4UIMessenger", "+++ %s> Exception: Failed to change property %s = '%s'.", m_name.c_str(),
                n.c_str(), v.c_str() );
    ...
    if ( j != m_actionCmd.end() ) {
      try {
        ...
        printout( INFO, "Geant4UIMessenger", "+++ %s> Exception: Failed to exec action '%s' [%s].", m_name.c_str(),
                  c->GetCommandName().c_str(), c->GetCommandPath().c_str() );
