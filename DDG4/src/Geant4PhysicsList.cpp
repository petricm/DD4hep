...
void _findDef( const string& expression, vector<G4ParticleDefinition*>& results ) {
  ...
  while ( ( *iter )() ) {
    ...
    if ( !ret )
      ...
      ::regfree( &reg );
      throw runtime_error(
          format( "Geant4PhysicsList", "REGEX: Failed to match particle name %s err=%s", exp.c_str(), msgbuf ) );
    }
  }
  ::regfree( &reg );
}
...
Geant4PhysicsList::Geant4PhysicsList( Geant4Context* ctxt, const string& nam ) : Geant4Action( ctxt, nam ) {
...
void Geant4PhysicsList::addParticleProcess( const std::string& part_name, const std::string& proc_name,
  ...
  for ( PhysicsConstructors::iterator i = m_physics.begin(); i != m_physics.end(); ++i ) {
    ...
    if ( 0 == ctor.pointer ) {
      ...
      if ( !p ) {
            ...
            "Failed to create the physics entities "
            "for the G4VPhysicsConstructor '%s'",
            ctor.c_str() );
      }
      ctor.pointer = p;
    }
    ...
    for ( vector<G4ParticleDefinition*>::const_iterator id = defs.begin(); id != defs.end(); ++id ) {
      ...
      for ( ParticleProcesses::const_iterator ip = procs.begin(); ip != procs.end(); ++ip ) {
        ...
        if ( !g4 ) {  // Error no factory for this process
          except( "Particle:%s -> [%s] Cannot create physics process %s", part_name.c_str(),
                  particle->GetParticleName().c_str(), p.name.c_str() );
        }
        mgr->AddProcess( g4, p.ordAtRestDoIt, p.ordAlongSteptDoIt, p.ordPostStepDoIt );
        info( "Particle:%s -> [%s] added process %s with flags (%d,%d,%d)", part_name.c_str(),
              particle->GetParticleName().c_str(), p.name.c_str(), p.ordAtRestDoIt, p.ordAlongSteptDoIt,
              p.ordPostStepDoIt );
      }
    }
  }
...
G4VUserPhysicsList* Geant4PhysicsListActionSequence::extensionList() {
  G4VModularPhysicsList* physics =
      ( m_extends.empty() ) ? new EmptyPhysics() : G4PhysListFactory().GetReferencePhysList( m_extends );
...
void Geant4PhysicsListActionSequence::installCommandMessenger() {
  control()->addCall( "dump", "Dump content of " + name(),
                      Callback( this ).make( &Geant4PhysicsListActionSequence::dump ) );
}
