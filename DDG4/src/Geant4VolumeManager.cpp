...
struct Populator {
  ...
  void populate( DetElement e ) {
    ...
    for ( DetElement::Children::const_iterator i = c.begin(); i != c.end(); ++i ) {
      ...
      printout( WARNING, "Geant4VolumeManager", "++ Detector element %s of type %s has no placement.", de.name(),
                de.type().c_str() );
    }
  }
    ...
    if ( i == m_entries.end() ) {
      ...
      for ( Chain::const_reverse_iterator k = nodes.rbegin(), kend = nodes.rend(); k != kend; ++k ) {
        ...
        if ( g4pit != m_geo.g4Placements.end() ) {
          ...
          printout( print_chain, "Geant4VolumeManager", "+++     Chain: Node OK: %s [%s]", node->GetName(),
                    ( *g4pit ).second->GetName().c_str() );
          continue;
        }
        ...
        if ( iVolImp != m_geo.g4VolumeImprints.end() ) {
          ...
          for ( Imprints::const_iterator iImp = imprints.begin(); iImp != imprints.end(); ++iImp ) {
            ...
            if ( c.size() <= control.size() && control == c ) {
              ...
              printout( print_chain, "Geant4VolumeManager", "+++     Chain: Node OK: %s %s -> %s", node->GetName(),
                        DetectorTools::placementPath( c, false ).c_str(), ( *iImp ).second->GetName().c_str() );
              control.clear();
              break;
            }
          }
      ...
      if ( control.empty() ) {
        ...
        printout( print_res, "Geant4VolumeManager", "+++     Map %016X to Geant4 Path:%s", (void*)code,
                  placementPath( path ).c_str() );
      ...
      printout( INFO, "Geant4VolumeManager", "Control block has still %d entries:%s", int( control.size() ),
                DetectorTools::placementPath( control, true ).c_str() );
      goto Err;
    }
  ...
  Err:
    ...
    if ( !nodes.empty() )
      printout( ERROR, "Geant4VolumeManager", "     TGeo path: %s",
                DetectorTools::placementPath( nodes, false ).c_str() );
...
Geant4VolumeManager::Geant4VolumeManager( LCDD& lcdd, Geant4GeometryInfo* info ) : Base( info ), m_isValid( false ) {
  ...
  throw runtime_error(
      format( "Geant4VolumeManager", "Attempt populate from invalid Geant4 geometry info [Invalid-Info]" ) );
}
...
bool Geant4VolumeManager::checkValidity() const {
  if ( m_isValid ) {
    ...
    throw runtime_error(
        format( "Geant4VolumeManager", "Attempt to use invalid Geant4 volume manager [Invalid-Handle]" ) );
  } else if ( !ptr()->valid ) {
    throw runtime_error(
        format( "Geant4VolumeManager", "Attempt to use invalid Geant4 geometry info [Invalid-Info]" ) );
  }
  m_isValid = true;
  return m_isValid;
}
