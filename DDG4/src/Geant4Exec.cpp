...
class SequenceHdl {
 public:
  ...
  /// Default constructor
  SequenceHdl() : m_sequence( 0 ), m_activeContext( 0 ) {
  }
...
class Geant4UserRunAction : public G4UserRunAction, public SequenceHdl<Geant4RunActionSequence> {
 public:
  ...
  Geant4UserRunAction( Geant4Context* ctxt, Geant4RunActionSequence* seq ) : Base( ctxt, seq ), eventAction( 0 ) {
    updateContext( ctxt );
    configureFiber( ctxt );
  }
  /// Default destructor
...
class Geant4UserEventAction : public G4UserEventAction, public SequenceHdl<Geant4EventActionSequence> {
 public:
  ...
  /// Standard constructor
  Geant4UserEventAction( Geant4Context* ctxt, Geant4EventActionSequence* seq ) : Base( ctxt, seq ), runAction( 0 ) {
    updateContext( ctxt );
    configureFiber( ctxt );
  }
  /// Default destructor
...
class Geant4UserTrackingAction : public G4UserTrackingAction, public SequenceHdl<Geant4TrackingActionSequence> {
 public:
  /// Standard constructor
  Geant4UserTrackingAction( Geant4Context* ctxt, Geant4TrackingActionSequence* seq ) : Base( ctxt, seq ) {
    updateContext( ctxt );
    configureFiber( ctxt );
  }
  /// Default destructor
...
class Geant4UserStackingAction : public G4UserStackingAction, public SequenceHdl<Geant4StackingActionSequence> {
 public:
  /// Standard constructor
  Geant4UserStackingAction( Geant4Context* ctxt, Geant4StackingActionSequence* seq ) : Base( ctxt, seq ) {
    updateContext( ctxt );
    configureFiber( ctxt );
  }
  /// Default destructor
...
class Geant4UserSteppingAction : public G4UserSteppingAction, public SequenceHdl<Geant4SteppingActionSequence> {
 public:
  /// Standard constructor
  Geant4UserSteppingAction( Geant4Context* ctxt, Geant4SteppingActionSequence* seq ) : Base( ctxt, seq ) {
    updateContext( ctxt );
    configureFiber( ctxt );
  }
  /// Default destructor
 ...
 * @version 1.0
 */
class Geant4UserDetectorConstruction : public G4VUserDetectorConstruction,
 ...
 public:
  ...
  Geant4UserDetectorConstruction( Geant4Context* ctxt, Geant4DetectorConstructionSequence* seq )
      : G4VUserDetectorConstruction(), Base( ctxt, seq ), m_ctxt( ctxt->kernel().lcdd(), this ) {
  }
  /// Default destructor
 ...
 * @version 1.0
 */
class Geant4UserActionInitialization : public G4VUserActionInitialization,
...
void Geant4UserRunAction::BeginOfRunAction( const G4Run* run ) {
  ...
  if ( m_sequence )
    m_sequence->begin( run );  // Action not mandatory
}

/// End-of-run callback
void Geant4UserRunAction::EndOfRunAction( const G4Run* run ) {
  if ( m_sequence )
    m_sequence->end( run );  // Action not mandatory
...
void Geant4UserEventAction::BeginOfEventAction( const G4Event* evt ) {
  ...
  if ( m_sequence )
    m_sequence->begin( evt );  // Action not mandatory
}

/// End-of-event callback
void Geant4UserEventAction::EndOfEventAction( const G4Event* evt ) {
  if ( m_sequence )
    m_sequence->end( evt );  // Action not mandatory
...
void Geant4UserActionInitialization::Build() const {
  ...
  if ( m_sequence ) {
    ...
    m_sequence->info(
        ...
        "Context:%p Kernel:%p [%ld]",
        (void*)ctx, (void*)&krnl, krnl.id() );
...
Geant4DetectorConstructionSequence* Geant4Compatibility::buildDefaultDetectorConstruction( Geant4Kernel& kernel ) {
  ...
  cr = PluginService::Create<Geant4Action*>( "Geant4DetectorSensitivesConstruction", ctx,
                                             string( "ConstructSensitives" ) );
  det_cr = dynamic_cast<Geant4DetectorConstruction*>( cr );
  if ( det_cr )
    seq->adopt( det_cr );
  else
