...
Geant4TestBase::Geant4TestBase( Geant4Action* a, const std::string& typ ) : m_type( typ ) {
...
void Geant4TestGeneratorAction::operator()( G4Event* evt ) {
  PRINT( "%s> calling Geant4TestGeneratorAction(event_id=%d Context: run=%p evt=%p)", m_type.c_str(), evt->GetEventID(),
         context()->runPtr(), context()->eventPtr() );
}
...
void Geant4TestRunAction::end( const G4Run* run ) {
  PRINT( "%s> calling end(run_id=%d, num_event=%d Context:%p)", m_type.c_str(), run->GetRunID(),
         run->GetNumberOfEvent(), &context()->run() );
}

/// begin-of-event callback
void Geant4TestRunAction::beginEvent( const G4Event* evt ) {
  PRINT( "%s> calling beginEvent(event_id=%d Context: run=%p evt=%p)", m_type.c_str(), evt->GetEventID(),
         context()->runPtr(), context()->eventPtr() );
}

/// End-of-event callback
void Geant4TestRunAction::endEvent( const G4Event* evt ) {
  PRINT( "%s> calling endEvent(event_id=%d Context: run=%p evt=%p)", m_type.c_str(), evt->GetEventID(),
         context()->runPtr(), context()->eventPtr() );
}
...
void Geant4TestEventAction::begin( const G4Event* evt ) {
  PRINT( "%s> calling begin(event_id=%d Context:  run=%p (%d) evt=%p (%d))", m_type.c_str(), evt->GetEventID(),
         &context()->run(), context()->run().run().GetRunID(), &context()->event(),
         context()->event().event().GetEventID() );
}

/// End-of-event callback
void Geant4TestEventAction::end( const G4Event* evt ) {
  PRINT( "%s> calling end(event_id=%d Context:  run=%p (%d) evt=%p (%d))", m_type.c_str(), evt->GetEventID(),
         &context()->run(), &context()->event(), &context()->run(), context()->run().run().GetRunID(),
         &context()->event(), context()->event().event().GetEventID() );
}

/// begin-of-run callback
void Geant4TestEventAction::beginRun( const G4Run* run ) {
  PRINT( "%s> calling beginRun(run_id=%d,num_event=%d Context:%p)", m_type.c_str(), run->GetRunID(),
         run->GetNumberOfEventToBeProcessed(), context()->runPtr() );
}

/// End-of-run callback
void Geant4TestEventAction::endRun( const G4Run* run ) {
  PRINT( "%s> calling endRun(run_id=%d, num_event=%d Context:%p)", m_type.c_str(), run->GetRunID(),
...
void Geant4TestTrackAction::begin( const G4Track* trk ) {
  PRINT( "%s> calling begin(track=%d, parent=%d, position=(%f,%f,%f) Context: run=%p evt=%p)", m_type.c_str(),
         trk->GetTrackID(), trk->GetParentID(), trk->GetPosition().x(), trk->GetPosition().y(), trk->GetPosition().z(),
         &context()->run(), &context()->event() );
}

/// End-of-tracking callback
void Geant4TestTrackAction::end( const G4Track* trk ) {
  PRINT( "%s> calling end(track=%d, parent=%d, position=(%f,%f,%f) Context: run=%p evt=%p)", m_type.c_str(),
         trk->GetTrackID(), trk->GetParentID(), trk->GetPosition().x(), trk->GetPosition().y(), trk->GetPosition().z(),
         &context()->run(), &context()->event() );
}
...
void Geant4TestSensitive::begin( G4HCofThisEvent* hce ) {
  ...
  PRINT( "%s> calling begin(num_coll=%d, coll=%s Context: run=%p evt=%p)", m_type.c_str(),
         hce->GetNumberOfCollections(), c ? c->GetName().c_str() : "None", &context()->run(), &context()->event() );
}

/// End-of-tracking callback
void Geant4TestSensitive::end( G4HCofThisEvent* hce ) {
  Geant4HitCollection* c = collection( m_collectionID );
  PRINT( "%s> calling end(num_coll=%d, coll=%s Context: run=%p evt=%p)", m_type.c_str(), hce->GetNumberOfCollections(),
