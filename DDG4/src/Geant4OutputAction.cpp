...
void Geant4OutputAction::end( const G4Event* evt ) {
  ...
  if ( hce ) {
    ...
    try {
      ...
      if ( m_truth && !m_truth->isValid() ) {
        m_truth = 0;
        printout( WARNING, name(),
                  "+++ [Event:%d] No valid MC truth info present. "
                  "Is a Particle handler installed ?",
                  evt->GetEventID() );
      }
      try {
        saveEvent( ctxt );
        ...
        printout( ERROR, name(), "+++ [Event:%d] Exception while saving event:%s", evt->GetEventID(), e.what() );
        if ( m_errorFatal )
          throw;
      } catch ( ... ) {
        printout( ERROR, name(), "+++ [Event:%d] UNKNWON Exception while saving event", evt->GetEventID() );
        if ( m_errorFatal )
          throw;
      }
      commit( ctxt );
  ...
  printout( WARNING, "Geant4OutputAction", "+++ The value of G4HCofThisEvent is NULL. No collections saved!" );
}
