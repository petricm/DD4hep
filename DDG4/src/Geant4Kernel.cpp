...
Geant4Kernel& Geant4Kernel::registerGlobalAction( Geant4Action* action ) {
  if ( action ) {
    ...
    if ( i == m_globalActions.end() ) {
      ...
      printout( INFO, "Geant4Kernel", "++ Registered global action %s of type %s", nam.c_str(),
                typeName( typeid( *action ) ).c_str() );
      return *this;
    }
    throw runtime_error( format( "Geant4Kernel",
                                 "DDG4: The action '%s' is already globally "
                                 "registered. [Action-Already-Registered]",
                                 nam.c_str() ) );
  }
  throw runtime_error( format( "Geant4Kernel",
...
Geant4Kernel& Geant4Kernel::registerGlobalFilter( Geant4Action* filter ) {
  if ( filter ) {
    ...
    throw runtime_error( format( "Geant4Kernel",
                                 ...
                                 "registered. [Filter-Already-Registered]",
                                 nam.c_str() ) );
  }
  throw runtime_error( format( "Geant4Kernel",
                               "DDG4: Attempt to globally register an invalid "
