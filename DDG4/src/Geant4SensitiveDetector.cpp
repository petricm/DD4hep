...
bool Geant4SensitiveDetector::defineCollection( const string& coll_name ) {
  if ( coll_name.empty() ) {
    throw runtime_error( "Geant4SensitiveDetector: No collection defined for " + name() + " of type " +
                         string( m_sensitive.type() ) );
  }
...
template <>
Geant4CalorimeterHit* Geant4SensitiveDetector::find<Geant4CalorimeterHit>(
    const HitCollection* c, const HitCompare<Geant4CalorimeterHit>& cmp ) {
...
#if DEBUG
  std::cout << " *** too small energy deposit : " << step->GetTotalEnergyDeposit() << " < " << ene_cut << "    at "
            << step->GetPreStepPoint()->GetPosition() << std::endl;
#endif
  return false;
}
...
void Geant4SensitiveDetector::dumpStep( G4Step* st, G4TouchableHistory* /* history */ ) {
  ...
  printout( INFO, "G4Step", "  Track:%08ld Pos:(%8f %8f %8f) -> (%f %f %f)  Mom:%7.0f %7.0f %7.0f", long( step.track ),
            pos1.X(), pos1.Y(), pos1.Z(), pos2.X(), pos2.Y(), pos2.Z(), mom.X(), mom.Y(), mom.Z() );
  printout( INFO, "G4Step", "                pre-Vol: %s  Status:%s", step.preVolume()->GetName().c_str(),
            step.preStepStatus() );
