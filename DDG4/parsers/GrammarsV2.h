...
#define REGISTER_GRAMMAR(ResultType, GrammarName)                                                 \
  template <typename Iterator, typename Skipper> struct Grammar_<Iterator, ResultType, Skipper> { \
    ...
    struct StringGrammar : qi::grammar<Iterator, std::string(), qi::locals<char>, Skipper> {
      ...
      StringGrammar() : StringGrammar::base_type(str) {
        ...
        str = qi::lexeme[begin_quote[qi::_a = qi::_1] > *((enc::char_('\\') >> quote(qi::_a))[qi::_val += qi::_a] |
    ...
    //==============================================================================
    template <typename Iterator, typename Skipper> struct CharGrammar : qi::grammar<Iterator, char(), Skipper> {
    ...
    //==============================================================================
    template <typename Iterator, typename Skipper> struct BoolGrammar : qi::grammar<Iterator, bool(), Skipper> {
    ...
    //==============================================================================
    template <typename Iterator, typename RT, typename Skipper> struct IntGrammar : qi::grammar<Iterator, RT(), Skipper> {
      typedef RT ResultT;
    ...
    //==============================================================================
    template <typename Iterator, typename RT, typename Skipper> struct RealGrammar : qi::grammar<Iterator, RT(), Skipper> {
      typedef RT ResultT;
    ...
    struct VectorGrammar : qi::grammar<Iterator, VectorT(), qi::locals<char>, Skipper> {
      ...
      VectorGrammar() : VectorGrammar::base_type(vec) {
        begin = enc::char_('[')[qi::_val = ']'] | enc::char_('{')[qi::_val = '}'] | enc::char_('(')[qi::_val = ')'];
    ...
    struct MapGrammar : qi::grammar<Iterator, MapT(), Skipper> {
      ...
      struct Operations {
        ...
        void operator()(ResultT& res, const VectorPairT& vec) const {
          for (typename VectorPairT::const_iterator cur = vec.begin(); cur != vec.end(); cur++) {
            res.insert(*cur);
          }
        }
      ...
      MapGrammar() : MapGrammar::base_type(map) {
        pair = key[op(qi::_val, qi::_1, tag_key())] > (qi::lit(':') | '=') > value[op(qi::_val, qi::_1, tag_mapped())];
        list = -(pair % enc::char_(','));
