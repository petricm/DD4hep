...
class EventStream {
 public:
  ...
  EventStream( istream& in )
      : instream( in ), mom_unit( 0.0 ), pos_unit( 0.0 ), io_type( 0 ), xsection( 0.0 ), xsection_err( 0.0 ) {
    use_default_units();
  }
...
Geant4EventReaderHepMC::Geant4EventReaderHepMC( const string& nam )
  ...
  if ( not m_input.is_open() ) {
    string err =
        "+++ Geant4EventReaderHepMC: Failed to open input stream:" + nam + " Error:" + string( strerror( errno ) );
...
Geant4EventReader::EventReaderStatus Geant4EventReaderHepMC::moveToEvent( int event_number ) {
...
/// Read an event and fill a vector of MCParticles.
Geant4EventReaderHepMC::EventReaderStatus Geant4EventReaderHepMC::readParticles( int /* ev_id */,
  ...
  if ( !m_events->ok() ) {
    ...
    for ( Particles::const_iterator k = output.begin(); k != output.end(); ++k ) {
      ...
      printout( VERBOSE, m_name,
                ...
                "", p->id, p->status, p->pdgID, p->psx / MeV, p->psy / MeV, p->psz / MeV, p->time / ns,
                p->daughters.size(), p->parents.size() );
      // output.push_back(p);
    }
    ++m_currEvent;
    return EVENT_READER_OK;
...
int HepMC::read_particle( EventStream& info, istringstream& input, Geant4Particle* p ) {
  ...
  status.clear();
  if ( stat == 0 )
    status.set( G4PARTICLE_GEN_EMPTY );
  else if ( stat == 0x1 )
    status.set( G4PARTICLE_GEN_STABLE );
  else if ( stat == 0x2 )
    status.set( G4PARTICLE_GEN_DECAYED );
  else if ( stat == 0x3 )
    status.set( G4PARTICLE_GEN_DOCUMENTATION );
  else if ( stat == 0x4 )
    status.set( G4PARTICLE_GEN_DOCUMENTATION );
  else if ( stat == 0xB )
    status.set( G4PARTICLE_GEN_DOCUMENTATION );
...
int HepMC::read_vertex( EventStream& info, istream& is, istringstream& input ) {
  ...
  input >> id >> dummy >> v->x >> v->y >> v->z >> v->time >> num_orphans_in >> num_particles_out >> weights_size;
...
int HepMC::read_heavy_ion( EventStream&, istringstream& input ) {
  ...
  int   nh = 0, np = 0, nt = 0, nc = 0, neut = 0, prot = 0, nw = 0, nwn = 0, nwnw = 0;
