...
void Geant4DetectorSensitivesConstruction::constructSensitives( Geant4DetectorConstructionContext* ctxt ) {
  ...
  for ( _SV::const_iterator iv = vols.begin(); iv != vols.end(); ++iv ) {
    ...
    G4VSensitiveDetector*       g4sd = PluginService::Create<G4VSensitiveDetector*>( typ, nam, &ctxt->lcdd );
    if ( !g4sd ) {
      string tmp = typ;
      tmp[ 0 ]   = ::toupper( tmp[ 0 ] );
      typ        = "Geant4" + tmp;
      ...
      if ( !g4sd ) {
        ...
        if ( !g4sd ) {
          throw runtime_error(
              "ConstructSDandField: FATAL Failed to "
    ...
    for ( VolSet::const_iterator i = sens_vols.begin(); i != sens_vols.end(); ++i ) {
      ...
      if ( !g4v ) {
        throw runtime_error( "ConstructSDandField: Failed to access G4LogicalVolume for SD " + nam + " of type " + typ +
                             "." );
      }
