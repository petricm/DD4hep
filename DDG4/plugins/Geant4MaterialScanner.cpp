...
/// Copy constructor
Geant4MaterialScanner::StepInfo::StepInfo( const StepInfo& c ) : pre( c.pre ), post( c.post ), volume( c.volume ) {
}
...
void Geant4MaterialScanner::end( const G4Track* track ) {
  ...
  if ( !m_steps.empty() ) {
    ...
    ::printf(
        ...
        "\n%s",
        line, pre.X() / cm, pre.Y() / cm, pre.Z() / cm, post.X() / cm, post.Y() / cm, post.Z() / cm,
    ...
    for ( Steps::const_iterator i = m_steps.begin(); i != m_steps.end(); ++i, ++count ) {
      ...
      m_sumLambda += nLambda;
      m_sumPath += length;
      ::printf( fmt, count, material->GetName().c_str(), Zeff, Aeff, density, radLen, intLen, length, m_sumPath,
                m_sumX0, m_sumLambda, postPos.X() / cm, postPos.Y() / cm, postPos.Z() / cm );
      // cout << *m << endl;
    }
    for_each( m_steps.begin(), m_steps.end(), DestroyObject<StepInfo*>() );
    m_steps.clear();
