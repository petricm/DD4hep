...
Geant4EventReaderHepEvt::Geant4EventReaderHepEvt( const string& nam, int format )
  ...
  if ( !m_input.good() ) {
    string err =
        "+++ Geant4EventReaderHepEvt: Failed to open input stream:" + nam + " Error:" + string( strerror( errno ) );
...
Geant4EventReader::EventReaderStatus Geant4EventReaderHepEvt::moveToEvent( int event_number ) {
...
/// Read an event and fill a vector of MCParticles.
Geant4EventReader::EventReaderStatus Geant4EventReaderHepEvt::readParticles( int /* event_number */,
  ...
  for ( int IHEP = 0; IHEP < NHEP; IHEP++ ) {
    if ( m_format == HEPEvtShort )
      m_input >> ISTHEP >> IDHEP >> JDAHEP1 >> JDAHEP2 >> PHEP1 >> PHEP2 >> PHEP3 >> PHEP5;
    else
    ...
    if ( ( fd > -1 ) && ( ld > -1 ) ) {
      if ( ld >= fd ) {
        for ( int id = fd; id < ld + 1; id++ ) {
          ...
          if ( !part.findParent( mcp ) )
            part.addParent( mcp );
        }
      } else {  //  Same logic, discrete cases
        ParticleHandler part_fd( particles[ fd ] );
        if ( !part_fd.findParent( mcp ) )
          part_fd.addParent( mcp );

        ParticleHandler part_ld( particles[ ld ] );
        if ( !part_ld.findParent( mcp ) )
          part_ld.addParent( mcp );
      }
    } else if ( fd > -1 ) {
