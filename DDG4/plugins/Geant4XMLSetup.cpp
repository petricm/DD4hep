...
static Action _convertSensitive( lcdd_t& lcdd, xml_h e, const string& detector ) {
  ...
  printout( INFO, "Geant4Setup", "+++ Added sensitive element %s of type %s", tn.second.c_str(), tn.first.c_str() );
  ...
  if ( !action ) {
    action = ( what == SENSITIVE ) ? Action( _convertSensitive( lcdd, a, seqType ) )
                                   : ( what == ACTION ) ? _convertAction( lcdd, a )
                                                        : ( what == FILTER ) ? _convertAction( lcdd, a ) : Action();
    if ( !action ) {
      throw runtime_error( format( "Geant4ActionSequence",
                                   "DDG4: The action '%s'"
                                   " cannot be created. [Action-Missing]",
                                   nam.c_str() ) );
    }
  }
  return action;
 ...
 */
template <>
void Converter<Action>::operator()( xml_h e ) const {
 ...
 */
template <>
void Converter<Filter>::operator()( xml_h e ) const {
  Action a = _convertAction( lcdd, e );
 ...
 */
template <>
void Converter<Phase>::operator()( xml_h e ) const {
  ...
  if ( nam == "RunAction/begin" ) {
    ...
    if ( !det.isValid() ) {
      throw runtime_error( format( "Phase",
    ...
    if ( tn.second == "begin" )
      sdSeq->callAtBegin( ( p = kernel.addPhase<G4HCofThisEvent*>( tn.second ) ).get(), &PH::call<G4HCofThisEvent*> );
    else if ( tn.second == "end" )
      sdSeq->callAtEnd( ( p = kernel.addPhase<G4HCofThisEvent*>( tn.second ) ).get(), &PH::call<G4HCofThisEvent*> );
    else if ( tn.second == "clear" )
      sdSeq->callAtClear( ( p = kernel.addPhase<G4HCofThisEvent*>( tn.second ) ).get(), &PH::call<G4HCofThisEvent*> );
 ...
 */
template <>
void Converter<ActionSequence>::operator()( xml_h e ) const {
  xml_comp_t   seq( e );
  SensitiveSeq sdSeq;
  string       seqNam;
  TypeName     seqType;
  ...
  if ( seq.hasAttr( _U( sd ) ) ) {
    ...
    if ( !sensitive.isValid() ) {
      printout( ALWAYS, "Geant4Setup",
                "+++ ActionSequence %s is defined, "
                "but no sensitive detector present.",
                seqNam.c_str() );
  ...
  printout( INFO, "Geant4Setup", "+++ ActionSequence %s of type %s added.", seqType.second.c_str(),
            seqType.first.c_str() );
  ...
  for ( xml_coll_t a( seq, _Unicode( action ) ); a; ++a ) {
    ...
    if ( seqType.second == "RunAction" )
      ...
      throw runtime_error( format( "ActionSequence",
                                   "DDG4: The action '%s'"
                                   " cannot be attached to any sequence '%s'."
                                   " [Sequence-Missing]",
                                   nam.c_str(), seqNam.c_str() ) );
    }
    printout( INFO, "Geant4Setup", "+++ ActionSequence %s added filter object:%s", seqType.second.c_str(),
              action->name().c_str() );
  }
  if ( what == SENSITIVE ) {
 ...
 */
template <>
void Converter<Geant4PhysicsList::ParticleProcesses>::operator()( xml_h e ) const {
  xml_comp_t                            part( e );
  string                                part_name = part.nameStr();
  Geant4PhysicsList::ParticleProcesses& procs     = _object<Geant4PhysicsList>().processes( part_name );
 ...
 */
template <>
void Converter<Geant4PhysicsList::ParticleConstructor>::operator()( xml_h e ) const {
 ...
 */
template <>
void Converter<Geant4PhysicsList::PhysicsConstructor>::operator()( xml_h e ) const {
...
template <>
void Converter<PhysicsListExtension>::operator()( xml_h e ) const {
  Kernel& kernel = Kernel::instance( lcdd );
...
template <>
void Converter<PhysicsList>::operator()( xml_h e ) const {
  string      name   = e.attr<string>( _U( name ) );
  Kernel&     kernel = Kernel::instance( lcdd );
  PhysicsList handle( kernel, name );
  _setAttributes( handle, e );
  xml_coll_t( e, _Unicode( particles ) )
      .for_each( _Unicode( construct ), Converter<Geant4PhysicsList::ParticleConstructor>( lcdd, handle.get() ) );
  xml_coll_t( e, _Unicode( processes ) )
      .for_each( _Unicode( particle ), Converter<Geant4PhysicsList::ParticleProcesses>( lcdd, handle.get() ) );
  xml_coll_t( e, _Unicode( physics ) )
      .for_each( _Unicode( construct ), Converter<Geant4PhysicsList::PhysicsConstructor>( lcdd, handle.get() ) );
...
template <>
void Converter<Kernel>::operator()( xml_h e ) const {
  Kernel&    kernel = Kernel::instance( lcdd );
  xml_comp_t k( e );
...
template <>
void Converter<XMLSetup>::operator()( xml_h seq ) const {
  ...
  xml_coll_t( compact, _Unicode( sequences ) )
      .for_each( _Unicode( sequence ), Converter<ActionSequence>( lcdd, param ) );
