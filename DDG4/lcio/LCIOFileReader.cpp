...
/// Initializing constructor
DD4hep::Simulation::LCIOFileReader::LCIOFileReader( const std::string& nam ) : LCIOEventReader( nam ) {
...
Geant4EventReader::EventReaderStatus DD4hep::Simulation::LCIOFileReader::moveToEvent( int event_number ) {
...
/// Read an event and fill a vector of MCParticles.
Geant4EventReader::EventReaderStatus DD4hep::Simulation::LCIOFileReader::readParticleCollection(
  ...
  if ( evt ) {
    *particles = evt->getCollection( LCIO::MCPARTICLE );
    if ( *particles ) {
      printout( INFO, "LCIOFileReader", "read collection %s from event %d in run %d ", LCIO::MCPARTICLE,
                evt->getEventNumber(), evt->getRunNumber() );
      return EVENT_READER_OK;
    }
  }
