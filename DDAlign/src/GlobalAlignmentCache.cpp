...
GlobalAlignmentCache::~GlobalAlignmentCache() {
  ...
  m_cache.clear();
  printout( INFO, "GlobalAlignmentCache", "Destroy cache for subdetector %s [%d section(s), %d entrie(s)]",
...
bool GlobalAlignmentCache::insert( GlobalAlignment alignment ) {
  ...
  printout( ALWAYS, "GlobalAlignmentCache", "Section: %s adding entry: %s", name().c_str(), alignment->GetName() );
  if ( i == m_cache.end() ) {
    m_cache[ index ] = pn;
    return true;
  }
...
GlobalAlignmentCache* GlobalAlignmentCache::section( const string& path_name ) const {
  ...
  if ( ( idq = path_name.find( '/', idx + 1 ) ) != string::npos )
    --idq;
