...
template <>
AlignmentsManager AlignmentsManager::from<LCDD>( LCDD& host ) {
  Object* obj = host.extension<Object>();
  if ( obj )
    return AlignmentsManager( obj );
...
void AlignmentsManagerObject::to_world( AlignContext& new_alignments, UserPool& pool, DetElement det,
...
void AlignmentsManagerObject::compute( AlignContext& new_alignments, UserPool& pool, DetElement det, int level ) const {
  ...
  if ( ent ) {
    ...
    printout( DEBUG, "ComputeAlignment", fmt, det.level(), "", has_cond ? "NO " : "YES", det.key(), det.path().c_str(),
              cond.iov().str().c_str() );
  } else {
...
void AlignmentsManager::newEntry( const Context& context, DetElement& det, const Dependency* dep,
