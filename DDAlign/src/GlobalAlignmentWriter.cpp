...
GlobalAlignmentWriter::GlobalAlignmentWriter( LCDD& lcdd ) : m_lcdd( lcdd ) {
...
void GlobalAlignmentWriter::addNode( XML::Element elt, GlobalAlignment a ) const {
  ...
  // mat.Print();
  if ( fabs( t[ 0 ] ) > numeric_limits<double>::epsilon() || fabs( t[ 1 ] ) > numeric_limits<double>::epsilon() ||
  ...
  if ( mat.IsRotation() ) {
    ...
    if ( fabs( rot.X() ) > numeric_limits<double>::epsilon() || fabs( rot.Y() ) > numeric_limits<double>::epsilon() ||
...
XML::Document GlobalAlignmentWriter::dump( DetElement top, bool enable_transactions ) const {
  ...
  if ( enable_transactions )
    root.append( XML::Element( doc, _ALU( open_transaction ) ) );
  if ( ( elt = scan( doc, top ) ) )
    elements.append( elt );
  if ( enable_transactions )
    root.append( XML::Element( doc, _ALU( close_transaction ) ) );
  return doc;
}
