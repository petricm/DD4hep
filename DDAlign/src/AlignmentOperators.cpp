...
template <>
void AlignmentActor<DDAlign_standard_operations::node_print>::init() {
  printout( ALWAYS, "GlobalAlignmentCache", "++++++++++++++++++++++++ Summary ++++++++++++++++++++++++" );
}

template <>
void AlignmentActor<DDAlign_standard_operations::node_print>::operator()( Nodes::value_type& n ) const {
  TGeoPhysicalNode* p = n.second.first;
  Entry*            e = n.second.second;
  printout( ALWAYS, "GlobalAlignmentCache", "Need to reset entry:%s - %s [needsReset:%s, hasMatrix:%s]", p->GetName(),
            e->path.c_str(), yes_no( e->needsReset() ), yes_no( e->hasMatrix() ) );
}

template <>
void AlignmentActor<DDAlign_standard_operations::node_delete>::operator()( Nodes::value_type& n ) const {
  delete n.second.second;
  n.second.second = 0;
}

template <>
void AlignmentActor<DDAlign_standard_operations::node_reset>::operator()( Nodes::value_type& n ) const {
  TGeoPhysicalNode* p = n.second.first;
  string            np;
  if ( p->IsAligned() ) {
...
template <>
void AlignmentActor<DDAlign_standard_operations::node_align>::operator()( Nodes::value_type& n ) const {
  Entry&     e       = *n.second.second;
  bool       overlap = e.overlapDefined();
  DetElement det     = e.detector;

  if ( !det->global_alignment.isValid() && !e.hasMatrix() ) {
    printout( WARNING, "AlignmentActor", "++++ SKIP Alignment %s DE:%s Valid:%s Matrix:%s", e.path.c_str(),
              det.placementPath().c_str(), yes_no( det->global_alignment.isValid() ), yes_no( e.hasMatrix() ) );
    return;
  }
  if ( GlobalDetectorAlignment::debug() ) {
    printout( INFO, "AlignmentActor", "++++ %s DE:%s Matrix:%s", e.path.c_str(), det.placementPath().c_str(),
              yes_no( e.hasMatrix() ) );
  }
  ...
  if ( delta.checkFlag( Delta::HAVE_ROTATION | Delta::HAVE_PIVOT | Delta::HAVE_TRANSLATION ) )
    trafo =
        Transform3D( Translation3D( delta.translation ) * delta.pivot * delta.rotation * ( delta.pivot.Inverse() ) );
