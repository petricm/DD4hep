...
void usage_default( const char* name ) {
  ...
  print_default_args() << "        -load_only   [OPTIONAL]     Dry-run to only load geometry without     \n"
                          "                                    starting the dispay.                      \n"
                       << endl;
  exit( EINVAL );
}
...
struct Args {
  ...
  long run( LCDD& lcdd, const char* name ) {
    ...
    for ( size_t i = 0; i < plugins.size(); ++i ) {
      ...
      if ( result == EINVAL ) {
        cout << "FAILED to execute DD4hep plugin: '" << plug[ 0 ] << "' with args (" << num_args << ") :[ ";
      ...
      cout << "Executed DD4hep plugin: '" << plug[ 0 ] << "' with args (" << num_args << ") :[ ";
      for ( size_t j = 1; j < plug.size(); ++j ) {
        if ( plug[ j ] )
          cout << plug[ j ] << " ";
      }
...
int main_default( const char* name, int argc, char** argv ) {
  ...
  if ( args.ui )
    run_plugin( lcdd, "DD4hepROOTUI", 0, 0 );
