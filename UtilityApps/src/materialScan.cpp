...
int main( int argc, char** argv ) {
  ...
  ::printf(
      " |     \\   %-11s        Atomic                 Radiation   Interaction               Path   Integrated  "
      "Integrated    Material\n",
      "Material" );
  ::printf(
      " | Num. \\  %-11s   Number/Z   Mass/A  Density    Length       Length    Thickness   Length      X0        "
      "Lambda      Endpoint  \n",
      "Name" );
  ::printf(
      " | Layer \\ %-11s            [g/mole]  [g/cm3]     [cm]        [cm]          [cm]      [cm]     [cm]        "
      "[cm]     (     cm,     cm,     cm)\n",
      "" );
  ::printf( "%s", line );
  ...
  for ( unsigned i = 0, n = materials.size(); i < n; ++i ) {
    ...
    ::printf( fmt, i + 1, mat->GetName(), mat->GetZ(), mat->GetA(), mat->GetDensity(), mat->GetRadLen(),
              mat->GetIntLen(), length, path_length, sum_x0, sum_lambda, end[ 0 ], end[ 1 ], end[ 2 ] );
    // mat->Print();
  }
  printf( "%s", line );
  ...
  ::printf( fmt, 0, "Average Material", avg.Z(), avg.A(), avg.density(), avg.radiationLength(), avg.interactionLength(),
            path_length, path_length, path_length / avg.radiationLength(), path_length / avg.interactionLength(),
            end[ 0 ], end[ 1 ], end[ 2 ] );
  printf( "%s", line );
  return 0;
}
