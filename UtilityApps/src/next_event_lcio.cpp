...
template <class T>
TEveElement* createPointSet( lcio::LCCollection* col, const std::string& name, unsigned color = kMagenta,
                             unsigned size = 1, unsigned style = 4 );
...
void next_event() {
  ...
  if ( count == 1 ) {
    ...
    try {
      ...
      std::cout << " ------------------------------------------------------------------------------------------------ "
                << std::endl
                << "*** file " << lcioFileName << " does not exist - can't read LCIO events  !                       "
                << std::endl
  ...
  if ( evt != 0 ) {
    ...
    for ( unsigned icol = 0, ncol = colNames.size(); icol < ncol; ++icol ) {
      ...
      if ( col->getTypeName() == LCIO::SIMTRACKERHIT ) {
        MultiView::instance()->ImportEvent(
            createPointSet<EVENT::SimTrackerHit>( col, colNames[ icol ], kMagenta + 2, 1, 4 ) );
      } else if ( col->getTypeName() == LCIO::SIMCALORIMETERHIT ) {
        MultiView::instance()->ImportEvent(
            createPointSet<EVENT::SimCalorimeterHit>( col, colNames[ icol ], kMagenta + 4, 1, 4 ) );
      } else if ( col->getTypeName() == LCIO::TRACKERHIT ) {
        MultiView::instance()->ImportEvent(
            createPointSet<EVENT::TrackerHit>( col, colNames[ icol ], kBlue + 2, 1, 4 ) );
      } else if ( col->getTypeName() == LCIO::CALORIMETERHIT ) {
        MultiView::instance()->ImportEvent(
            createPointSet<EVENT::CalorimeterHit>( col, colNames[ icol ], kBlue + 4, 1, 4 ) );
      }
    }
...
//=====================================================================================
template <class T>
TEveElement* createPointSet( lcio::LCCollection* col, const std::string& name, unsigned color, unsigned size,
                             unsigned style ) {
