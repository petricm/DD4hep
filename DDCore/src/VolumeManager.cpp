...
struct Populator {
  ...
  void populate( DetElement e ) {
    ...
    for ( DetElement::Children::const_iterator i = c.begin(); i != c.end(); ++i ) {
      ...
      printout( WARNING, "VolumeManager", "++ Detector element %s of type %s has no placement.", de.name(),
                de.type().c_str() );
    }
  }
  ...
  size_t scanPhysicalVolume( DetElement& parent, DetElement e, PlacedVolume pv, Encoding parent_encoding,
    ...
    if ( node ) {
      ...
      if ( sd.isValid() && !pv_ids.empty() ) {
        ...
        if ( ro.isValid() ) {
          ...
          printout( WARNING, "VolumeManager",
                    "%s: Strange constellation volume %s is sensitive, but has no readout! sd:%p", parent.name(),
      ...
      for ( Int_t idau = 0, ndau = node->GetNdaughters(); idau < ndau; ++idau ) {
        ...
        if ( placement.data() ) {
          ...
          except( "VolumeManager", "Invalid not instrumented placement:" + string( daughter->GetName() ) +
      ...
      if ( sd.isValid() ) {
        if ( !have_encoding ) {
          printout( ERROR, "VolumeManager", "%s: Missing SD encoding. Volume manager won't work!", e.path().c_str() );
        }
        ...
        if ( add_det_vol_id ) {
          ...
          printout( VERBOSE, "VolumeManager", "++++ %-11s  SD:%s VolID=%p Mask=%p", e.path().c_str(),
                    have_encoding ? "RECUPERATED" : "REGULAR", sd.name(), (void*)vol_encoding.first,
    ...
    if ( node ) {
      ...
      if ( count == 0 ) {
        ...
        if ( ro.isValid() ) {
          ...
          printout( VERBOSE, "VolumeManager", "++++ %-11s  SD:%s VolID=%p Mask=%p", e.path().c_str(),
                    got_readout ? "RECUPERATED" : "REGULAR", sd.name(), (void*)det_encoding.first,
  ...
  void add_entry( SensitiveDetector sd, DetElement parent, DetElement e, const TGeoNode* n, const Encoding& code,
                  Chain& nodes ) {
    if ( sd.isValid() ) {
  ...
  void print_node( SensitiveDetector sd, DetElement parent, DetElement e, const TGeoNode* n, const VolIDs& ids,
                   const Chain& /* nodes */ ) const {
    ...
    ++s_count;
    stringstream log;
    log << s_count << ": " << parent.name() << ": " << e.name() << " ro:" << ro.ptr() << " pv:" << n->GetName()
        << " id:" << (void*)volume_id << " : ";
    for ( VolIDs::const_iterator i = ids.begin(); i != ids.end(); ++i ) {
      const PlacedVolume::VolID& id    = ( *i );
  ...
  void print_node( SensitiveDetector sd, DetElement parent, DetElement e, const TGeoNode* n, const Encoding& code,
                   const Chain& /* nodes */ ) const {
    static int   s_count   = 0;
    Readout      ro        = sd.readout();
    PlacedVolume pv        = Ref_t( n );
    ...
    stringstream log;
    log << s_count << ": " << parent.name() << ": " << e.name() << " ro:" << ro.ptr() << " pv:" << n->GetName()
        << " id:" << (void*)volume_id << " Sensitive:" << yes_no( sensitive );
    printout( DEBUG, "VolumeManager", log.str().c_str() );
#if 0
      log.str("");
      log << s_count << ": " << e.name() << " Detector GeoNodes:";
...
VolumeManager VolumeManager::addSubdetector( DetElement det, Readout ro ) {
  if ( isValid() ) {
    ...
    if ( i == o.subdetectors.end() ) {
      ...
      if ( !field ) {
        throw runtime_error( "DD4hep: VolumeManager::addSubdetector: IdDescriptor of " + string( det.name() ) +
                             " has no field " + id.first );
      }
      Object& mo             = m._data();
      mo.top                 = o.top;
      mo.flags               = o.flags;
      ...
      det.callAtUpdate( DetElement::PLACEMENT_CHANGED | DetElement::PLACEMENT_DETECTOR, &mo, &Object::update );
    }
    return ( *i ).second;
  }
  throw runtime_error(
...
bool VolumeManager::adoptPlacement( VolumeID /* sys_id */, Context* context ) {
  ...
  if ( i == o.volumes.end() ) {
    ...
    err << "Inserted new volume:" << setw( 6 ) << left << o.volumes.size() << " Ptr:" << (void*)pv.ptr() << " ["
        << pv.name() << "]"
        << " ID:" << (void*)context->identifier << " Mask:" << (void*)context->mask;
    printout( VERBOSE, "VolumeManager", err.str().c_str() );
    return true;
  }
  err << "+++ Attempt to register duplicate volID " << (void*)context->identifier << " Mask:" << (void*)context->mask
      << " to detector " << o.detector.name() << " ptr:" << (void*)pv.ptr() << " Name:" << pv.name()
  ...
  err << " !!!!!               +++ Clashing volID " << (void*)context->identifier << " Mask:" << (void*)context->mask
      << " to detector " << o.detector.name() << " ptr:" << (void*)pv.ptr() << " Name:" << pv.name()
      << " Sensitive:" << yes_no( pv.volume().isSensitive() ) << endl;
  printout( ERROR, "VolumeManager", "%s", err.str().c_str() );
  err.str( "" );
...
std::ostream& DD4hep::Geometry::operator<<( std::ostream& os, const VolumeManager& m ) {
  ...
  string prefix( isTop ? "" : "++  " );
  os << prefix << ( isTop ? "TOP Level " : "Secondary " ) << "Volume manager:" << &o << " " << o.detector.name()
     << " IDD:" << o.id.toString() << " SysID:" << (void*)o.sysID << " " << o.managers.size() << " subsections "
  ...
  for ( VolumeManager::Volumes::const_iterator i = o.volumes.begin(); i != o.volumes.end(); ++i ) {
    ...
    os << prefix << "PV:" << setw( 32 ) << left << pv.name() << " id:" << setw( 18 ) << left << (void*)c->identifier
