...
/// Copy constructor
IOV::IOV( const IOVType* t, const Key& k ) : iovType( t ), keyData( k ), optData( 0 ) {
  type = t ? t->type : int( IOVType::UNKNOWN_IOV );
}
...
string IOV::str() const {
  ...
  if ( iovType ) {
    if ( iovType->name[ 0 ] != 'e' ) {
      ::snprintf( text, sizeof( text ), "%s(%d):[%ld-%ld]", iovType->name.c_str(), int( iovType->type ), keyData.first,
                  keyData.second );
      ...
      ::snprintf( text, sizeof( text ), "%s(%d):[%s - %s]", iovType->name.c_str(), iovType->type, c_since, c_until );
    } else {
      ::snprintf( text, sizeof( text ), "%s(%d):[%ld-%ld]", iovType->name.c_str(), int( iovType->type ), keyData.first,
                  keyData.second );
    }
