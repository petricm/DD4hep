...
/// Clone constructor
DetElement::DetElement( Object* det_data, const string& det_name, const string& det_type ) : RefObject( det_data ) {
...
DetElement& DetElement::add( DetElement sdet ) {
  if ( isValid() ) {
    ...
    throw runtime_error( "DD4hep: DetElement::add: Element " + string( sdet.name() ) + " is already present in path " +
                         path() + " [Double-Insert]" );
  }
