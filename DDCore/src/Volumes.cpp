...
struct DDExtension {
  ...
  DDExtension( const DDExtension& c ) : m_extension( 0 ) {
    if ( c.m_extension )
      m_extension = c.m_extension->Grab();
  }
  virtual ~DDExtension() {
    if ( m_extension )
      m_extension->Release();
  ...
  DDExtension& operator=( const DDExtension& c ) {
    if ( this != &c )
      SetUserExtension( c.GetUserExtension() );
    return *this;
  }
...
struct DD_TGeoNodeMatrix : public TGeoNodeMatrix, public DDExtension {
 private:
  DD_TGeoNodeMatrix& operator=( const DD_TGeoNodeMatrix& ) {
    return *this;
  }

 public:
  DD_TGeoNodeMatrix( const TGeoVolume* v, const TGeoMatrix* m ) : TGeoNodeMatrix( v, m ), DDExtension() {
    INCREMENT_COUNTER;
  }
  DD_TGeoNodeMatrix( const DD_TGeoNodeMatrix& c ) : TGeoNodeMatrix( c.GetVolume(), c.GetMatrix() ), DDExtension( c ) {
    INCREMENT_COUNTER;
  }
...
template <class T>
struct _VolWrap : public T, public DDExtension {
 public:
...
template <>
_VolWrap<TGeoVolume>::_VolWrap( const char* name ) : TGeoVolume( name, 0, 0 ), DDExtension() {
  INCREMENT_COUNTER;
}
...
struct TGeoVolumeValue : public _VolWrap<TGeoVolume> {
  ...
  TGeoVolume* _copyVol( TGeoShape* newshape ) const {
    ...
    if ( m_extension )
      vol->m_extension = m_extension->Grab();
    // vol->copy(*this);
    return vol;
  }
...
template <typename T>
static typename T::Object* _userExtension( const T& v ) {
...
/// Default constructor
PlacedVolumeExtension::PlacedVolumeExtension() : TGeoExtension(), magic( 0 ), refCount( 0 ), volIDs() {
  magic = magic_word();
  INCREMENT_COUNTER;
}
...
/// Lookup volume ID
vector<PlacedVolumeExtension::VolID>::const_iterator PlacedVolumeExtension::VolIDs::find( const string& name ) const {
...
std::pair<vector<PlacedVolumeExtension::VolID>::iterator, bool> PlacedVolumeExtension::VolIDs::insert(
...
string PlacedVolume::toString() const {
  stringstream s;
  Object*      obj = _data( *this );
  s << m_element->GetName() << ":  vol='" << m_element->GetVolume()->GetName() << "' mat:'"
    << m_element->GetMatrix()->GetName() << "' volID[" << obj->volIDs.size() << "] ";
...
const Volume& Volume::setVisAttributes( const VisAttr& attr ) const {
  if ( attr.isValid() ) {
    ...
    printout( DEBUG, "setVisAttributes", "Set color %3d [%02X,%02X,%02X] DrawingStyle:%9s LineStyle:%6s for volume %s",
              int( vis->color ), c ? int( 255 * c->GetRed() ) : 0xFF, c ? int( 255 * c->GetGreen() ) : 0xFF,
              c ? int( 255 * c->GetBlue() ) : 0xFF, draw_style == VisAttr::SOLID ? "Solid" : "Wireframe",
              line_style == VisAttr::SOLID ? "Solid" : "Dashed", name() );
