...
long int DD4hep::makeTime( int year, int month, int day, int hour, int minutes, int seconds ) {
  struct tm tm_init;
  ...
  if ( ti >= 0 )
    return ti;
  except( "DD4hep", "Invalid time data given for conversion to epoch: %d-%d-%d %02d:%02d:%02d", year, month, day, hour,
          minutes, seconds );
  return ti;
}
...
long int DD4hep::makeTime( const std::string& date, const char* fmt ) {
  ...
  if ( 0 == c ) {
    except( "DD4hep", "Invalid time format given for update:%s should be: %s", date.c_str(), fmt );
  }
  long ti = ::mktime( &tm );
  if ( ti >= 0 )
    return ti;
  except( "DD4hep", "Invalid time string given for conversion to epoch: %s (fmt='%s')", date.c_str(), fmt );
  return ti;
}
...
void DD4hep::invalidHandleError( const std::type_info& type ) {
  throw invalid_handle_exception( "Attempt to access invalid object of type " + typeName( type ) +
                                  " [Invalid Handle]" );
}
...
template <typename T>
std::string Primitive<T>::toString( T value ) {
  char text[ 1024 ];
...
template <>
std::string Primitive<const char*>::toString( const char* value ) {
  if ( value ) {
    return value;
  }
...
/// Convert string to string
template <>
std::string Primitive<char*>::toString( char* value ) {
  if ( value ) {
    return value;
  }
  throw std::runtime_error( "Failed to convert (char*)NULL to std::string!" );
...
/// Convert string to string
template <>
std::string Primitive<std::string>::toString( std::string value ) {
  return value;
}
...
#ifndef __APPLE__
static inline void* cast_wrap( const void* p, const abi::__class_type_info* src, const abi::__class_type_info* dst,
                               ptrdiff_t src2dst ) {
