...
void LCDDImp::declareMotherVolume( const std::string& detector_name, const Volume& vol ) {
  if ( !detector_name.empty() ) {
    ...
    throw runtime_error( "LCDD: Attempt to register invalid mother volume for detector:" + detector_name +
                         " [Invalid-Handle]." );
  }
...
LCDD& LCDDImp::addDetector( const Ref_t& ref_det ) {
  ...
  if ( existing_det.isValid() ) {
    ...
    if ( sd.isValid() ) {
      stringstream str;
      str << "LCDD: The sensitive sub-detectors " << det_element.name() << " and " << existing_det.name()
          << " have the identical ID:" << det_element.id() << ".";
...
vector<DetElement> LCDDImp::detectors( const string& type1, const string& type2, const string& type3,
                                       const string& type4, const string& type5 ) {
  if ( m_manager->IsClosed() ) {
    vector<DetElement>              v;
  ...
  throw runtime_error( "detectors(" + type1 + "," + type2 +
...
Handle<TObject> LCDDImp::getRefChild( const HandleMap& e, const string& name, bool do_throw ) const {
  ...
  if ( do_throw ) {
    int cnt = 0;
    cout << "GetRefChild: Failed to find child with name: " << name << " Map contains " << e.size() << " elements."
         << endl;
...
void LCDDImp::init() {
  if ( !m_world.isValid() ) {
    ...
    printout( INFO, "LCDD", " *********** created World volume with size : %7.0f %7.0f %7.0f", worldSolid->GetDX(),
              worldSolid->GetDY(), worldSolid->GetDZ() );
...
long LCDDImp::apply( const char* factory_type, int argc, char** argv ) {
  ...
  try {
    ...
    if ( 0 == result ) {
      ...
      if ( 0 == result ) {
        throw runtime_error( "DD4hep: apply-plugin: Failed to locate plugin " + fac + ". " +
                             dbg.missingFactory( fac ) );
      }
    }
    result = *(long*)result;
    if ( result != 1 ) {
