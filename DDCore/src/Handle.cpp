...
template <>
char DD4hep::_multiply<char>( const string& left, const string& right ) {
  double val = _toDouble( left + "*" + right );
  if ( val >= double( SCHAR_MIN ) && val <= double( SCHAR_MAX ) )
    return (char)(int)val;
  except( "_multiply<char>", "Multiplication %e = %s * %s out of bounds for conversion to char.", val, left.c_str(),
          right.c_str() );
  return 0;
}

template <>
unsigned char DD4hep::_multiply<unsigned char>( const string& left, const string& right ) {
  double val = _toDouble( left + "*" + right );
  if ( val >= 0 && val <= double( UCHAR_MAX ) )
    return (unsigned char)(int)val;
  except( "_multiply<char>", "Multiplication %e = %s * %s out of bounds for conversion to unsigned char.", val,
          left.c_str(), right.c_str() );
  return 0;
}

template <>
short DD4hep::_multiply<short>( const string& left, const string& right ) {
  double val = _toDouble( left + "*" + right );
  if ( val >= double( SHRT_MIN ) && val <= double( SHRT_MAX ) )
    return (short)val;
  except( "_multiply<char>", "Multiplication %e = %s * %s out of bounds for conversion to short.", val, left.c_str(),
          right.c_str() );
  return 0;
}

template <>
unsigned short DD4hep::_multiply<unsigned short>( const string& left, const string& right ) {
  double val = _toDouble( left + "*" + right );
  if ( val >= 0 && val <= double( USHRT_MAX ) )
    return (unsigned short)val;
  except( "_multiply<char>", "Multiplication %e = %s * %s out of bounds for conversion to unsigned short.", val,
          left.c_str(), right.c_str() );
  return 0;
}

template <>
int DD4hep::_multiply<int>( const string& left, const string& right ) {
  return (int)_toDouble( left + "*" + right );
}

template <>
unsigned int DD4hep::_multiply<unsigned int>( const string& left, const string& right ) {
  return (unsigned int)_toDouble( left + "*" + right );
}

template <>
long DD4hep::_multiply<long>( const string& left, const string& right ) {
  return (long)_toDouble( left + "*" + right );
}

template <>
unsigned long DD4hep::_multiply<unsigned long>( const string& left, const string& right ) {
  return (unsigned long)_toDouble( left + "*" + right );
}

template <>
float DD4hep::_multiply<float>( const string& left, const string& right ) {
  return _toFloat( left + "*" + right );
}

template <>
double DD4hep::_multiply<double>( const string& left, const string& right ) {
...
template <typename T>
static inline string __to_string( T value, const char* fmt ) {
  char text[ 128 ];
...
namespace DD4hep {
template <>
void Handle<_Segmentation>::assign( _Segmentation* s, const std::string& n, const std::string& ) {
  this->m_element = s;
  s->setName( n );
}
template <>
const char* Handle<_Segmentation>::name() const {
