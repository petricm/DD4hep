...
ConditionKey::ConditionKey( const string& value ) : name( value ), hash( hashCode( value ) ) {
}
...
Condition& Condition::rebind() {
  ...
  printout( INFO, "Condition", "+++ condition:%s rebinding value:%s", name().c_str(), o->value.c_str() );
  return *this;
}
