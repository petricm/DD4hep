...
XmlElement* node_first( XmlElement* e, const Tag_t& t ) {
  if ( t.str() == "*" )
    return e ? (XmlElement*)_E( e )->FirstChildElement() : 0;
...
size_t node_count( XmlElement* elt, const Tag_t& t ) {
  ...
  if ( t.str() == "*" )
    ...
    for ( e = e->FirstChildElement( t.str() ); e; e = e->NextSiblingElement( t.str() ) )
      ++cnt;
  return cnt;
}
}
...
/// Copy constructor
NodeList::NodeList( const NodeList& copy ) : m_tag( copy.m_tag ), m_node( copy.m_node ), m_ptr( 0 ) {
  reset();
}

/// Initializing constructor
NodeList::NodeList( XmlElement* node, const XmlChar* tag_value ) : m_tag( tag_value ), m_node( node ), m_ptr( 0 ) {
  reset();
}
...
#ifdef DD4HEP_USE_TINYXML
  for ( TiXmlNode* n = _E( m_node )->FirstChildElement( tag_value ); n;
        n            = _E( m_node )->FirstChildElement( tag_value ) )
    n->RemoveChild( n );
#else
  ...
  if ( n ) {
    ...
    switch ( n->Type() ) {
      case TiXmlNode::ELEMENT: {
        ...
        for ( ; p; p = p->Next() )
          m.insert( make_pair( p->Name(), p->Value() ) );
...
Element::Element( const Document& doc, const XmlChar* type ) : m_element( Xml( doc.createElt( type ) ).xe ) {
}
...
RefElement::RefElement( const Document& doc, const XmlChar* typ, const XmlChar* nam ) : Element( doc, typ ) {
...
/// Copy constructor
RefElement::RefElement( const RefElement& e ) : Element( e ), m_name( e.m_name ) {
}
...
#ifndef DD4HEP_USE_TINYXML
Collection_t::Collection_t( Handle_t element, const XmlChar* tag_value ) : m_children( element, tag_value ) {
  m_node = m_children.reset();
}
#endif

Collection_t::Collection_t( Handle_t element, const char* tag_value ) : m_children( element, Strng_t( tag_value ) ) {
  m_node = m_children.reset();
}

/// Constructor over XmlElements in a node list
Collection_t::Collection_t( NodeList node_list ) : m_children( node_list ) {
  m_node = m_children.reset();
}
