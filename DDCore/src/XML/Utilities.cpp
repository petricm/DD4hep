...
Geometry::Solid DD4hep::XML::createShape( Geometry::LCDD& lcdd, const std::string& shape_type, XML::Element element ) {
  ...
  if ( !solid.isValid() ) {
    ...
    except( "XML::createShape", "Failed to create solid of type %s [%s]", shape_type.c_str(),
            dbg.missingFactory( shape_type ).c_str() );
  }
  return solid;
}
