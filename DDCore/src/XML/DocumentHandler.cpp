...
bool DocumentErrorHandler::handleError( const DOMError& domError ) {
  ...
  printout( FATAL, "DocumentErrorHandler", "+++ %s %s: %s", err.c_str(), _toString( domError.getType() ).c_str(),
            _toString( domError.getMessage() ).c_str() );
  DOMLocator* loc = domError.getLocation();
  if ( loc ) {
    printout( FATAL, "DocumentErrorHandler", "+++ Location: Line:%d Column: %d", int( loc->getLineNumber() ),
...
void DocumentErrorHandler::error( const SAXParseException& e ) {
  ...
  printout( ERROR, "XercesC", "+++ Error at file \"%s\", Line %d Column: %d Message:%s", sys.c_str(),
            int( e.getLineNumber() ), int( e.getColumnNumber() ), m.c_str() );
}
/// Fatal error handler
void DocumentErrorHandler::fatalError( const SAXParseException& e ) {
  string m( _toString( e.getMessage() ) );
  string sys( _toString( e.getSystemId() ) );
  printout( FATAL, "XercesC", "+++ FATAL Error at file \"%s\", Line %d Column: %d Message:%s", sys.c_str(),
            int( e.getLineNumber() ), int( e.getColumnNumber() ), m.c_str() );
}
...
static string _clean_fname( const string& s ) {
  ...
  if ( strncmp( temp2.c_str(), "file:", 5 ) == 0 )
...
Document DocumentHandler::load( const std::string& fname, UriReader* reader ) const {
  ...
  if ( reader ) {
    printout( WARNING, "DocumentHandler", "+++ Loading document URI: %s %s", fname.c_str(),
  ...
  try {
    ...
    if ( !result ) {
      if ( doc->Error() ) {
        ...
        printout( FATAL, "DocumentHandler", "+++ Document:%s Location Line:%d Column:%d", doc->Value(), doc->ErrorRow(),
                  doc->ErrorCol() );
...
/// Parse a standalong XML string into a document.
Document DocumentHandler::parse( const char* bytes, size_t /* length */, const char* /* sys_id */,
                                 UriReader*  reader ) const {
  if ( reader ) {
