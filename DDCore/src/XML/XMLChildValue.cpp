...
#define childValueDefault( name, type, def )               \
  ...
  if ( __h.ptr() && __h.hasAttr( Unicode_value ) )         \
    return __h.attr<type>( Unicode_value );                \
  return def;

#define XML_ATTR_ACCESSOR( type, name ) \
  type ChildValue::name() const {       \
    return childValue( name, type );    \
  }
