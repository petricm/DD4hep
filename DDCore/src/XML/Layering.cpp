...
Layer::Layer( const Layer& c ) : _thickness( c._thickness ), _preOffset( c._preOffset ), _slices( c._slices ) {
}
...
double LayerStack::sectionThickness( size_t is, size_t ie ) const {
  ...
  if ( is > ie )
    ...
    throw runtime_error( "LayerStack::sectionThickness: First index (" + _toString( is ) + ") out of range. #layers=" +
                         _toString( _layers.size() ) + ")." );
  else if ( ie > _layers.size() )
    throw runtime_error( "LayerStack::sectionThickness: Second index (" + _toString( is ) + ") out of range. #layers=" +
                         _toString( _layers.size() ) + ")." );
