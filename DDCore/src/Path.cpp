...
Path Path::normalize() const {
  ...
  for ( vector<string>::const_iterator itr( start ); itr != stop; ++itr ) {
    ...
    Path itr_path( *itr );
    if ( itr_path.native().size() == 1 && ( itr_path.native() )[ 0 ] == dot && itr != start && itr != last )
      continue;
    ...
    if ( temp.empty() && itr_path.find( colon ) != string::npos ) {
      temp = itr_path;
      continue;
    } else if ( !temp.empty() && itr_path.native().size() == 2 && ( itr_path.native() )[ 0 ] == dot &&
                ( itr_path.native() )[ 1 ] == dot )  // dot dot
    {
      string lf( temp.filename().native() );
      if ( lf.size() > 0 && ( lf.size() != 1 || ( lf[ 0 ] != dot && lf[ 0 ] != separator ) ) &&
...
size_t Path::parent_path_end() const {
  ...
  for ( ; end_pos > 0 && ( end_pos - 1 ) != root_dir_pos && is_separator( this->at( end_pos - 1 ) ); --end_pos ) {
  }
...
Path Path::filename() const {
  ...
  return ( native().size() && pos && is_separator( at( pos ) ) && !is_root_separator( native(), pos ) )
             ? detail::dot_path()
