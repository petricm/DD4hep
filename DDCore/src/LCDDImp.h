...
namespace DetDesc {
  ...
  namespace Geometry {
    ...
    public:
      ...
      virtual LimitSet limitSet(const std::string& name) const { return getRefChild(m_limits, name); }
      virtual VisAttr visAttributes(const std::string& name) const { return getRefChild(m_display, name, false); }
      virtual Material material(const std::string& name) const { return getRefChild(m_materials, name); }
      virtual Region region(const std::string& name) const { return getRefChild(m_regions, name); }
      virtual RefElement idSpec(const std::string& name) const { return getRefChild(m_idDict, name); }
      virtual Volume volume(const std::string& name) const { return getRefChild(m_structure, name); }
      virtual Rotation rotation(const std::string& name) const { return getRefChild(m_rotations, name); }
      virtual Position position(const std::string& name) const { return getRefChild(m_positions, name); }
      virtual Solid solid(const std::string& name) const { return getRefChild(solids(), name); }
      virtual Constant constant(const std::string& name) const { return getRefChild(m_define, name); }
      virtual Readout readout(const std::string& name) const { return getRefChild(m_readouts, name); }
      virtual Subdetector detector(const std::string& name) const { return getRefChild(m_detectors, name); }
...
#define __R return *this
      ...
      // These not:
      virtual LCDD& addConstant(const RefElement& x) {
        m_define.append(x);
        __R;
      }
      virtual LCDD& addMaterial(const RefElement& x) {
        m_materials.append(x);
        __R;
      }
      virtual LCDD& addLimitSet(const RefElement& x) {
        m_limits.append(x);
        __R;
      }
      virtual LCDD& addRegion(const RefElement& x) {
        m_regions.append(x);
        __R;
      }
      virtual LCDD& addIDSpec(const RefElement& x) {
        m_idDict.append(x);
        __R;
      }
      virtual LCDD& addReadout(const RefElement& x) {
        m_readouts.append(x);
        __R;
      }
      virtual LCDD& addVisAttribute(const RefElement& x) {
        m_display.append(x);
        __R;
      }
      virtual LCDD& addSensitiveDetector(const RefElement& x) {
        m_sensitive.append(x);
        __R;
      }
      virtual LCDD& addDetector(const RefElement& x) {
