...
namespace DD4hep {
  ...
  namespace Geometry {
    ...
    public:
      ...
      virtual void fromCompact(const std::string& fname, LCDDBuildType type = BUILD_DEFAULT) { fromXML(fname, type); }
      ...
      virtual void fromXML(const std::string& fname, UriReader* entity_resolver, LCDDBuildType type = BUILD_DEFAULT);
      ...
      virtual IDDescriptor idSpecification(const std::string& name) const { return getRefChild(m_idDict, name); }
      ...
      virtual AlignmentEntry alignment(const std::string& path) const { return getRefChild(alignments(), path); }
      /// Retrieve a subdetector element by it's name from the detector description
      virtual DetElement detector(const std::string& name) const { return getRefChild(m_detectors, name); }
      ...
      virtual const HandleMap& idSpecifications() const { return m_idDict; }
      ...
      */
       ...
       */
      virtual std::vector<DetElement> detectors(unsigned int includeFlag, unsigned int excludeFlag = 0) const;
...
#define __R return *this
      ...
      virtual LCDD& add(SensitiveDetector x) { return addSensitiveDetector(x); }
