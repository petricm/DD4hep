...
struct VolIDTest {
  ...
  struct FND {
    ...
    bool operator()( const VolIDs::value_type& c ) const {
      return c.first == test;
    }
  };
...
VolIDTest::VolIDTest( LCDD& lcdd, DetElement sdet, size_t depth ) : m_det( sdet ) {
  ...
  if ( !lcdd.sensitiveDetector( m_det.name() ).isValid() ) {
    stringstream err;
    err << "The sensitive detector of subdetector " << m_det.name() << " is not known to the geometry.";
    printout( INFO, "VolIDTest", err.str().c_str() );
    throw runtime_error( err.str() );
  }
...
void VolIDTest::checkVolume( DetElement e, PlacedVolume pv, const VolIDs& child_ids ) const {
  ...
  try {
    ...
    if ( pv.ptr() != det_place.ptr() ) {
      ...
      err << "Wrong associated detector element vid=" << (void*)vid << " got " << det_elem.path() << " ("
          << (void*)det_elem.ptr() << ") "
  ...
  for ( size_t fi = 0; fi < m.size(); ++fi ) {
    ...
    if ( find_if( child_ids.begin(), child_ids.end(), FND( fld->name() ) ) == child_ids.end() )
      continue;
