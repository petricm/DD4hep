...
template <>
void Converter<AlignmentEntry>::operator()( xml_h element ) const;
template <>
...
template <>
void Converter<GdmlFile>::operator()( xml_h element ) const;
template <>
void Converter<XMLFile>::operator()( xml_h element ) const;
template <>
void Converter<AlignmentFile>::operator()( xml_h element ) const;
template <>
...
void Converter<Material>::operator()( xml_h e ) const {
  ...
  if ( 0 == mat ) {
    ...
    if ( dens_unit != 1.0 ) {
      ...
      if ( dens_unit != 1.0 )
        cout << " " << density.attr<string>( _U( unit ) );
    ...
    if ( has_density ) {
      ...
      printout( WARNING, "Compact",
                "++ Material: %s with NO density. "
                "Set density to %7.3 g/cm**3",
                matname, dens );
      mix->SetDensity( dens );
    }
  }
...
void Converter<Atom>::operator()( xml_h e ) const {
  ...
  if ( !element ) {
    ...
    tab->AddElement( elem.attr<string>( _U( name ) ).c_str(), elem.attr<string>( _U( formula ) ).c_str(),
                     elem.attr<int>( _U( Z ) ), atom.attr<int>( _U( value ) ) );
 ...
 */
template <>
void Converter<AlignmentEntry>::operator()( xml_h e ) const {
  xml_comp_t     child( e );
  ...
  if ( segment.isValid() ) {
    ...
    for ( xml_coll_t sub( seg, _U( segmentation ) ); sub; ++sub ) {
      ...
      if ( sub_object.first.isValid() ) {
        ...
        printout( DEBUG, "Compact", "++ Segmentation [%s/%s]: Add sub-segmentation %s [%s]", name.c_str(), type.c_str(),
                  sub_seg->segmentation->name().c_str(), sub_seg->segmentation->type().c_str() );
...
void Converter<Readout>::operator()( xml_h e ) const {
  ...
  for ( xml_coll_t colls( e, _U( hits_collections ) ); colls; ++colls ) {
    ...
    for ( xml_coll_t coll( colls, _U( hits_collection ) ); coll; ++coll ) {
      ...
      printout( DEBUG, "Compact", "++ Readout[%s]: Add hit collection %s [%s]  %d-%d", ro.name(), coll_name.c_str(),
                coll_key.c_str(), key_min, key_max );
...
void Converter<CartesianField>::operator()( xml_h e ) const {
  ...
  printout( INFO, "Compact", "++ Converted field: Successfully %s field %s [%s]", msg.c_str(), name.c_str(),
            type.c_str() );
}
...
void Converter<SensitiveDetector>::operator()( xml_h element ) const {
  ...
  try {
    ...
    printout( ERROR, "Compact", "++ FAILED    to convert sensitive detector: %s: %s", name.c_str(),
              "UNKNONW Exception" );
  }
}
  ...
  try {
    ...
    if ( attr_par ) {
      ...
      if ( !parent_volume.isValid() ) {
        except( "Compact", "Failed to access valid parent volume of %s from %s", name.c_str(), par_name.c_str() );
      }
