...
void LCDDConverter::GeometryInfo::check( const string& name, const TNamed* n, map<string, const TNamed*>& m ) const {
  ...
  if ( i != m.end() ) {
    ...
    cout << isa << "(position):  duplicate entry with name:" << name << " " << (void*)n << " " << (void*)( *i ).second
         << endl;
  }
...
xml_h LCDDConverter::handleSolid( const string& name, const TGeoShape* shape ) const {
  ...
  if ( !shape ) {
    ...
    if ( isa == TGeoBBox::Class() ) {
      ...
      solid.setAttr( _U( dz ), s->GetDz() );
      solid.setAttr( _U( lunit ), "cm" );
    } else if ( isa == TGeoCompositeShape::Class() || isa == TGeoUnion::Class() || isa == TGeoIntersection::Class() ||
...
xml_h LCDDConverter::handleSegmentation( Segmentation seg ) const {
  ...
  if ( seg.isValid() ) {
    ...
    for ( _P::const_iterator i = p.begin(); i != p.end(); ++i ) {
      ...
      if ( v->name() == "lunit" ) {
        string val = v->value() == _toDouble( "mm" )
                         ? "mm"
                         : v->value() == _toDouble( "cm" )
                               ? "cm"
                               : v->value() == _toDouble( "m" )
                                     ? "m"
                                     : v->value() == _toDouble( "micron" )
                                           ? "micron"
...
xml_h LCDDConverter::handleField( const std::string& /* name */, OverlayedField f ) const {
  ...
  if ( !field ) {
    ...
    if ( !fld.isValid() ) {
      ...
      throw runtime_error( "Failed to locate plugin to convert electromagnetic field:" + string( f->GetName() ) +
                           " of type " + type + ". " + dbg.missingFactory( type ) );
    }
...
static long create_visASCII( LCDD& lcdd, int /* argc */, char** argv ) {
  ...
  for ( xml_coll_t c( geo.doc_structure, _U( volume ) ); c; ++c ) {
    ...
    os << "vol:" << vol.nameStr() << sep << "vis:" << vis.nameStr() << sep << "visible:" << vis.visible() << sep
       << "r:" << col.R() << sep << "g:" << col.G() << sep << "b:" << col.B() << sep << "alpha:" << col.alpha() << sep
