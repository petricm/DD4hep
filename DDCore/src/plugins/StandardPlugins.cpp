...
static long load_compact( LCDD& lcdd, int argc, char** argv ) {
  if ( argc > 0 ) {
    ...
    if ( argc > 1 ) {
      ...
      printout( INFO, "CompactLoader", "+++ Processing compact file: %s with flag %s", input.c_str(), argv[ 1 ] );
...
static long load_xml( LCDD& lcdd, int argc, char** argv ) {
  if ( argc > 0 ) {
    ...
    if ( argc > 1 ) {
      type = build_type( argv[ 1 ] );
      printout( INFO, "XMLLoader", "+++ Processing XML file: %s with flag %s", input.c_str(), argv[ 1 ] );
  ...
  if ( argc > 0 ) {
    ...
    if ( imp ) {
      ...
      if ( input.ptr() ) {
        if ( argc > 1 ) {
          type = build_type( argv[ 1 ] );
          printout( INFO, "XMLLoader", "+++ Processing XML element: %s with flag %s", input.tag().c_str(), argv[ 1 ] );
        }
...
static long dump_volume_tree( LCDD& lcdd, int argc, char** argv ) {
  struct Actor {
    ...
    Actor( int ac, char** av ) : m_printVolIDs( false ), m_printPositions( false ), m_printSensitivesOnly( false ) {
      for ( int i = 0; i < ac; ++i ) {
        char c = ::tolower( av[ i ][ 0 ] );
        if ( c == 'v' )
          m_printVolIDs = true;
        else if ( c == 'p' )
          m_printPositions = true;
        else if ( c == 's' )
          m_printSensitivesOnly = true;
      }
    }
    ...
    long dump( TGeoNode* ideal, TGeoNode* aligned, int level, VIDs volids ) const {
      ...
      if ( !m_printSensitivesOnly || ( m_printSensitivesOnly && sensitive ) ) {
        ...
        if ( ideal == aligned ) {
          ::snprintf( fmt, sizeof( fmt ), "%03d %%-%ds %%s (%%s: %%s) \t[%p] %c %%s", level + 1, 2 * level + 1,
                      (void*)ideal, sens );
        } else {
          ::snprintf( fmt, sizeof( fmt ), "%03d %%-%ds %%s (%%s: %%s) Ideal:%p Aligned:%p %c %%s", level + 1,
                      2 * level + 1, (void*)ideal, (void*)aligned, sens );
        }
        printout( INFO, "VolumeDump", fmt, "", aligned->GetName(), volume->GetTitle(),
                  volume->GetShape()->IsA()->GetName(), opt_info.c_str() );
      }
 ...
 */
template <int flag>
long dump_detelement_tree( LCDD& lcdd, int argc, char** argv ) {
  struct Actor {
    static long dump( DetElement de, int level, bool sensitive_only ) {
      ...
      if ( !sensitive_only || 0 != de.volumeID() ) {
        ...
        switch ( value ) {
          case 0:
            ::snprintf( fmt, sizeof( fmt ), "%03d %%-%ds %%s #Dau:%%d VolID:%%08X Place:%%p  %%c", level + 1,
                        2 * level + 1 );
            printout( INFO, "DetectorDump", fmt, "", de.path().c_str(), int( c.size() ), (unsigned long)de.volumeID(),
                      (void*)node, sens );
            break;
          case 1:
...
static long detectortype_cache( LCDD& lcdd, int, char** ) {
  ...
  for ( vector<string>::const_iterator i = v.begin(); i != v.end(); ++i ) {
    ...
    for ( vector<DetElement>::const_iterator j = vv.begin(); j != vv.end(); ++j )
      printout( INFO, "DetectorTypes", "\t\t %-16s --> %s  [%s]", ( *i ).c_str(), ( *j ).name(),
                ( *j ).type().c_str() );
  }
  return 1;
}
