...
SensitiveDetectorObject::SensitiveDetectorObject()
    : NamedObject(),
      ObjectExtensions( typeid( SensitiveDetectorObject ) ),
      magic( magic_word() ),
      verbose( 0 ),
      combineHits( 0 ),
      ecut( 0.0 ),
...
DetElementObject::DetElementObject()
    : NamedObject(),
      ObjectExtensions( typeid( DetElementObject ) ),
      magic( magic_word() ),
      flag( 0 ),
      id( 0 ),
      combineHits( 0 ),
...
void DetElementObject::revalidate( TGeoHMatrix* parent_world_trafo ) {
  ...
  printout( ( idealPlace.ptr() != node.ptr() ) ? INFO : DEBUG, "DetElement",
            "+++ Invalidate chache of %s -> %s Placement:%p --> %p %s", det.path().c_str(),
            DetectorTools::placementPath( par_path ).c_str(), placement.ptr(), node.ptr(),
            ( placement.ptr() == node.ptr() ) ? "" : "[UPDATE]" );
  ...
  if ( ( flag & HAVE_PARENT_TRAFO ) && print )
    worldTrafo.Print();
  deletePtr( referenceTrafo );
...
void DetElementObject::update( unsigned int tags, void* param ) {
  ...
  if ( ( tags & DetElement::PLACEMENT_CHANGED ) == DetElement::PLACEMENT_CHANGED &&
    ...
    printout( INFO, "DetElement", "+++ Need to update chaches and child caches of %s", det.path().c_str() );
    DetElement   par                = det.parent();
