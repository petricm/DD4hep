...
#define IMPLEMENT_MAPPED_PARSERS(type, X)                                                                    \
  int parse(std::type<int, X>& result, const std::string& input) { return parse_(result, input); }           \
  int parse(std::type<unsigned long, X>& result, const std::string& input) { return parse_(result, input); } \
  int parse(std::type<std::string, X>& result, const std::string& input) { return parse_(result, input); }
...
namespace DD4hep {
  namespace Parsers {
    ...
    inline int parse_(ROOT::Math::PositionVector3D<T1, T2>& result, const std::string& input) {
      Skipper skipper;
