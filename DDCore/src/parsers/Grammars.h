...
namespace DD4hep {
  namespace Parsers {
     ...
     *  @date 2006-05-14
     */
    template <typename T> struct ClosureGrammar : public boost::spirit::closure<ClosureGrammar<T>, T> {
    ...
    struct AttributesClosureGrammar : public boost::spirit::closure<AttributesClosureGrammar<T1, T2>, T1, T2> {
     ...
     *  @date 2006-05-14
     */
    class BoolGrammar : public grammar<BoolGrammar, ClosureGrammar<bool>::context_t> {
    public:
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(BoolGrammar const& self) {
          boolean_literal = true_literal[self.val = true] | false_literal[self.val = false];
          true_literal  = str_p("true") | str_p("True") | str_p("TRUE") | str_p("1");
          false_literal = str_p("false") | str_p("False") | str_p("FALSE") | str_p("0");
        }
        rule<ScannerT> const& start() const { return boolean_literal; }
    ...
    class CharGrammar : public grammar<CharGrammar<RT>, typename ClosureGrammar<RT>::context_t> {
    public:
      typedef RT ResultT;
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(CharGrammar<RT> const& self) {
          char_literal = int_parser<RT>()[self.val = arg1] |
                         ('\'' >> (str_p("\\'")[self.val = '\''] | (anychar_p[self.val = arg1] - '\'')) >> '\'');
        }
        rule<ScannerT> const& start() const { return char_literal; }
     ...
     *  @date 2006-05-14
     */
    template <typename RT = int> class IntGrammar : public grammar<IntGrammar<RT>, typename ClosureGrammar<RT>::context_t> {
    public:
      typedef RT ResultT;
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(IntGrammar<RT> const& self) {
          int_literal = lexeme_d[int_parser<RT>()[self.val = arg1] >> !(ch_p('u') | ch_p('U') | ch_p('l') | ch_p('L'))];
        }
    ...
    class RealGrammar : public grammar<RealGrammar<RT>, typename ClosureGrammar<RT>::context_t> {
    public:
      typedef RT ResultT;
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(RealGrammar const& self) {
          real_literal = lexeme_d[real_parser<RT, real_parser_policies<RT>>()[self.val = arg1] >>
                                  !(ch_p('f') | ch_p('F') | ch_p('l') | ch_p('L'))];
        }
        rule<ScannerT> const& start() const { return real_literal; }
    ...
    class StringGrammar : public grammar<StringGrammar, ClosureGrammar<std::string>::context_t> {
    public:
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(StringGrammar const& self) {
          string_literal =
              (lexeme_d[('"' >> (*(str_p("\\\"") | (anychar_p - '"')))[self.val = construct_<std::string>(arg1, arg2)] >>
                         '"') |
    ...
    public:
       ...
       */
      SkipperGrammar(const bool skipnewline = true) : m_skipnewline(skipnewline) {}
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(SkipperGrammar const& self) {
          if (self.skipnewline()) {
            skip = space_p | comment_p("//")  // C++ comment
                   | comment_p("/*", "*/")    // C comment
                ;
          } else {
            skip = (space_p - eol_p) | comment_p("//")  // C++ comment
                   | comment_p("/*", "*/")              // C comment
                ;
          }
        }
    ...
    class PairGrammar
        : public grammar<PairGrammar<KeyGrammarT, ValueGrammarT>,
                         typename ClosureGrammar<
                             std::pair<typename KeyGrammarT::ResultT, typename ValueGrammarT::ResultT>>::context_t> {
    public:
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(PairGrammar const& self) {
          para = (str_p("(") >> (grkey[boost::bind(&PairGrammar::matchFirst, &self, _1)]) >> self.delim().c_str() >>
                  (grvalue[boost::bind(&PairGrammar::matchSecond, &self, _1)]) >> str_p(")"));
        }
    ...
    class VectorGrammar : public grammar<VectorGrammar<GrammarT>,
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(SelfT const& self) {
          inner = !(gr[boost::bind(&VectorGrammar::matchItem, &self, _1)] >>
                    *(',' >> gr[boost::bind(&VectorGrammar::matchItem, &self, _1)]));
          vec = '[' >> inner >> ']' |  // a'la python list
    ...
    template <typename KeyGrammarT, typename ValueGrammarT>
    class MapGrammar
        : public grammar<MapGrammar<KeyGrammarT, ValueGrammarT>,
                         typename AttributesClosureGrammar<
                             std::map<typename KeyGrammarT::ResultT, typename ValueGrammarT::ResultT>,
                             std::pair<typename KeyGrammarT::ResultT, typename ValueGrammarT::ResultT>>::context_t> {
    public:
      typedef typename KeyGrammarT::ResultT   KeyT;
      typedef typename ValueGrammarT::ResultT ValueT;
    ...
    public:
      template <typename ScannerT> struct definition {
        definition(MapGrammar const& self) {
          vec        = ('{' >> inner_list >> '}') | ('[' >> inner_list >> ']');
          inner_list = !(inner[boost::bind(&MapGrammar::matchItem, &self)] >>
                         *(ch_p(',') >> inner[boost::bind(&MapGrammar::matchItem, &self)]));
          inner = grKey[boost::bind(&MapGrammar::matchFirst, &self, _1)] >> (ch_p('=') | ch_p(':')) >>
