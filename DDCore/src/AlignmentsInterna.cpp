...
/// Standard constructor
AlignmentContainer::AlignmentContainer( Geometry::DetElementObject* par ) : NamedObject(), detector( par ), keys() {
  InstanceCount::increment( this );
}
...
void AlignmentContainer::addKey( const string& key_val ) {
  ...
  if ( !keys.insert( make_pair( hash, make_pair( hash, key_val ) ) ).second ) {
    except( "AlignmentContainer",
            "++ Key[%08X]: %s already present. "
            "Duplicate insertions inhibited!",
            hash, key_val.c_str() );
  }
}
