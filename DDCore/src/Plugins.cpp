...
string PluginDebug::missingFactory( const string& name ) const {
  ...
  string              msg         = "\n\t\tNo factory with name " + factoryname + " for type " + name +
               " found.\n\t\tPlease check library load path.";
  return msg;
}

/// Dummy functions of the plugin service
void* PluginService::getCreator( const std::string&, const std::type_info& ) {
  return 0;
}
...
struct PluginInterface {
  ...
  void ( *add )( const char* identifier, void* creator_stub, const char* signature, const char* return_type );
  PluginInterface();
...
static inline T get_func( void* handle, const char* plugin, const char* entry ) {
  ...
  if ( !fp.fptr.ptr )
    fp.fptr.ptr = ::dlsym( 0, entry );
#endif
  if ( 0 == fp.fptr.ptr ) {
    string err =
        "DD4hep:PluginService: Failed to access symbol "
        "\"" +
        string( entry ) + "\" in plugin library " + string( plugin ) + " [" + string(::strerror( errno ) ) + "]";
...
PluginInterface::PluginInterface() : getDebug( 0 ), setDebug( 0 ), create( 0 ), add( 0 ) {
  ...
  create   = get_func<void* (*)( const char*, const char* )>( handle, plugin_name, "dd4hep_pluginmgr_create" );
  add      = get_func<void ( * )( const char* identifier, void* creator_stub, const char* signature,
...
void PluginService::addFactory( const std::string& id, stub_t stub, const std::type_info& signature_type,
                                const std::type_info& return_type ) {
  if ( PluginService::debug() ) {
    printout( INFO, "PluginService", "+++ Declared factory[%s] with signature %s type:%s.", id.c_str(),
              signature_type.name(), return_type.name() );
  }
...
DD4HEP_IMPLEMENT_PLUGIN_REGISTRY( long, (Geometry::LCDD*, const Geometry::GeoHandler*,
