...
std::string DD4hep::Geometry::toStringSolid( const TGeoShape* shape, int precision ) {
  ...
  if ( cl == TGeoBBox::Class() ) {
    ...
    log << " x:" << s->GetDX() << " y:" << s->GetDY() << " z:" << s->GetDZ();
    ...
    log << " rmin:" << s->GetRmin() << " rmax:" << s->GetRmax() << " dz:" << s->GetDz() << " Phi1:" << s->GetPhi1()
        << " Phi2:" << s->GetPhi2();
    ...
    log << " x1:" << s->GetDx1() << " x2:" << s->GetDx2() << " y1:" << s->GetDy1() << " y2:" << s->GetDy2()
        << " z:" << s->GetDz();
  } else if ( cl == TGeoHype::Class() ) {
    const TGeoHype* s = (const TGeoHype*)shape;
    log << " rmin:" << s->GetRmin() << " rmax:" << s->GetRmax() << " dz:" << s->GetDz() << " StIn:" << s->GetStIn()
        << " StOut:" << s->GetStOut();
  } else if ( cl == TGeoPgon::Class() ) {
    const TGeoPgon* s = (const TGeoPgon*)shape;
    log << " Phi1:" << s->GetPhi1() << " dPhi:" << s->GetDphi() << " NEdges:" << s->GetNedges() << " Nz:" << s->GetNz();
    for ( int i = 0, n = s->GetNz(); i < n; ++i ) {
      log << " i=" << i << " z:" << s->GetZ( i ) << " r:[" << s->GetRmin( i ) << "," << s->GetRmax( i ) << "]";
    }
    ...
    log << " rmin1:" << s->GetRmin1() << " rmax1:" << s->GetRmax1() << " rmin2:" << s->GetRmin2()
        << " rmax2:" << s->GetRmax2() << " dz:" << s->GetDz() << " Phi1:" << s->GetPhi1() << " Phi2:" << s->GetPhi2();
    ...
    log << " rmin:" << s->GetRmin() << " rmax:" << s->GetRmax() << " Phi1:" << s->GetPhi1() << " Phi2:" << s->GetPhi2()
        << " Theta1:" << s->GetTheta1() << " Theta2:" << s->GetTheta2();
  } else if ( cl == TGeoTorus::Class() ) {
    const TGeoTorus* s = (const TGeoTorus*)shape;
    log << " rmin:" << s->GetRmin() << " rmax:" << s->GetRmax() << " r:" << s->GetR() << " Phi1:" << s->GetPhi1()
        << " dPhi:" << s->GetDphi();
  } else if ( cl == TGeoTrap::Class() ) {
    const TGeoTrap* s = (const TGeoTrap*)shape;
    log << " dz:" << s->GetDz() << " Theta:" << s->GetTheta() << " Phi:" << s->GetPhi() << " H1:" << s->GetH1()
        << " Bl1:" << s->GetBl1() << " Tl1:" << s->GetTl1() << " Alpha1:" << s->GetAlpha1() << " H2:" << s->GetH2()
        << " Bl2:" << s->GetBl2() << " Tl2:" << s->GetTl2() << " Alpha2:" << s->GetAlpha2();
...
template <typename T>
const char* Solid_type<T>::name() const {
...
ConeSegment::ConeSegment( double dz, double rmin1, double rmax1, double rmin2, double rmax2, double phi1,
                          double phi2 ) {
  _assign( new TGeoConeSeg( dz, rmin1, rmax1, rmin2, rmax2, phi1 / dd4hep::deg, phi2 / dd4hep::deg ), "",
           "cone_segment", true );
}
...
Sphere::Sphere( double rmin, double rmax, double theta, double delta_theta, double phi, double delta_phi ) {
  _assign( new TGeoSphere( rmin, rmax, theta / dd4hep::deg, delta_theta / dd4hep::deg, phi / dd4hep::deg,
                           delta_phi / dd4hep::deg ),
           "", "sphere", true );
}

/// Set the Sphere dimensions
Sphere& Sphere::setDimensions( double rmin, double rmax, double theta, double delta_theta, double phi,
                               double delta_phi ) {
...
Trap::Trap( double z, double theta, double phi, double y1, double x1, double x2, double alpha1, double y2, double x3,
            double x4, double alpha2 ) {
  _assign( new TGeoTrap( z, theta, phi, y1, x1, x2, alpha1 / dd4hep::deg, y2, x3, x4, alpha2 / dd4hep::deg ), "",
           "trap", true );
}
...
void Trap::make( double pz, double py, double px, double pLTX ) {
  ...
  _assign( new TGeoTrap( z, theta, phi, y1, x1, x2, alpha1 / dd4hep::deg, y1, x1, x2, alpha1 / dd4hep::deg ), "",
           "trap", true );
}

/// Set the trap dimensions
Trap& Trap::setDimensions( double z, double theta, double phi, double y1, double x1, double x2, double alpha1,
