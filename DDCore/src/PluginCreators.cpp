...
static inline ComponentCast* component( void* p ) {
...
void* createPlugin( const std::string& factory, Geometry::LCDD& lcdd, int argc, char** argv, void* ( *cast )(void*)) {
  ...
  if ( !object ) {
    ...
    if ( !object ) {
      except( "ConditionsManager", "DD4hep: plugin: Failed to locate plugin %s. [%s].", factory.c_str(),
              dbg.missingFactory( factory ).c_str() );
    }
  }
  if ( cast ) {
