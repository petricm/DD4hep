...
template <typename TYPE>
void Property::set( const TYPE& val ) {
...
template <typename TYPE>
Property& Property::operator=( const TYPE& val ) {
  this->set( val );
  return *this;
}
...
template <typename TYPE>
void Property::value( TYPE& val ) const {
  const PropertyGrammar& g = grammar();
  if ( g.type() == typeid( TYPE ) )
    val = *(TYPE*)m_par;
