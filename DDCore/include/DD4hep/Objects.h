...
#define DD4HEP_BYTE_ALIGN(x) x __attribute__((__packed__))
    ...
    DD4HEP_BYTE_ALIGN(struct) Position {
      ...
      Position& set(double xv, double yv, double zv) {
        x = xv;
        y = yv;
        z = zv;
        return *this;
...
#pragma pack(push, DD4Hep_Objects_Rotation, 1)
    ...
    inline Position operator+(const Position& l, const Position& r) { return Position(l.x + r.x, l.y + r.y, l.z + r.z); }
    /// Subtraction of to positions
    inline Position operator-(const Position& l, const Position& r) { return Position(l.x - r.x, l.y - r.y, l.z - r.z); }
    /// Dot product of 3-vectors.
    inline double operator*(const Position& l, const Position& r) { return sqrt(l.x * r.x + l.y * r.y + l.z * r.z); }
    /// Positions scaling from left
    inline Position operator*(double l, const Position& r) { return Position(r.x * l, r.y * l, r.z * l); }
    /// Positions scaling from right
    inline Position operator*(const Position& l, double r) { return Position(l.x * r, l.y * r, l.z * r); }
    /// Positions scaling from right
    inline Position operator/(const Position& l, double r) { return Position(l.x / r, l.y / r, l.z / r); }
    /// Calculate the mean length of two vectors
    inline double mean_length(const Position& p1, const Position& p2) { return 0.5 * (p1.length() + p2.length()) / 2.0; }
    /// Calculate the mean direction of two vectors
    inline Position mean_direction(const Position& p1, const Position& p2) { return 0.5 * (p1 + p2); }
    ...
    DD4HEP_BYTE_ALIGN(struct) Rotation {
      ...
      /// Initializer for all member variables
      Rotation& set(double th, double ph, double ps) {
        theta = th;
        phi   = ph;
        psi   = ps;
        return *this;
...
#undef DD4HEP_BYTE_ALIGN
    ...
    struct VisAttr : public Ref_t {
      ...
      enum DrawingStyle { WIREFRAME = 0x2, LAST_DRAWING_STYLE };
      enum LineStyle { SOLID = 0x1, DASHED = 0x2, LAST_LINE_STYLE };
      /// Default constructor
      VisAttr() : Ref_t() {}
    ...
    struct IDSpec : public Ref_t {
      /// Constructor to be used when reading the already parsed DOM tree
      template <typename Q> IDSpec(const Handle<Q>& e) : Ref_t(e) {}
      /// Constructor to be used when creating a new DOM tree
