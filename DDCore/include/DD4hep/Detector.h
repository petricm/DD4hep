...
class SensitiveDetector : public Handle<SensitiveDetectorObject> {
 ...
 public:
  ...
  template <typename Q>
  SensitiveDetector( const Handle<Q>& e ) : RefObject( e ) {
  }
...
class DetElement : public Handle<DetElementObject> {
 ...
 public:
  ...
  /// Templated constructor for handle conversions
  template <typename Q>
  DetElement( const Handle<Q>& e ) : RefObject( e ) {
  }
  ...
  DetElement& setAttributes( const LCDD& lcdd, const Volume& volume, const std::string& region,
