...
template <typename T>
inline long SurfaceInstaller::run( Geometry::LCDD& lcdd, int argc, char** argv ) {
...
class Installer : public DD4hep::SurfaceInstaller {
 ...
 public:
  ...
  template <typename T>
  bool checkShape( const T& shape ) const {
    if ( shape.isValid() )
      return true;
...
/// Default (empty argument handler
template <typename UserData>
void Installer<UserData>::handle_arguments( int, char** ) {
}
#ifndef SURFACEINSTALLER_DATA
template <>
void Installer<SURFACEINSTALLER_DATA>::handle_arguments( int, char** ) {
}
#endif
}
