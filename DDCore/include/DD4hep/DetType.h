...
namespace DD4hep {
  ...
  public:
    ...
    inline void set(unsigned long prop) { _type |= prop; }

    /// unset the given properties
    inline void unset(unsigned long prop) { _type &= ~prop; }

    /// true if detector has all properties
    inline bool is(unsigned long prop) const { return (_type & prop) == prop; }
  ...
  inline std::ostream& operator<<(std::ostream& os, const DetType& t) {
    ...
    if (t.is(DetType::TRACKER))
      os << "TRACKER, ";
    if (t.is(DetType::CALORIMETER))
      os << "CALORIMETER, ";
    if (t.is(DetType::CHERENKOV))
      os << "CHERENKOV, ";
    if (t.is(DetType::ENDCAP))
      os << "ENDCAP, ";
    if (t.is(DetType::BARREL))
      os << "BARREL, ";
    if (t.is(DetType::FORWARD))
      os << "FORWARD, ";
    if (t.is(DetType::VERTEX))
      os << "VERTEX, ";
    if (t.is(DetType::STRIP))
      os << "STRIP, ";
    if (t.is(DetType::PIXEL))
      os << "PIXEL, ";
    if (t.is(DetType::GASEOUS))
      os << "GASEOUS, ";
    if (t.is(DetType::WIRE))
      os << "WIRE, ";
    if (t.is(DetType::ELECTROMAGNETIC))
      os << "ELECTROMAGNETIC, ";
    if (t.is(DetType::HADRONIC))
      os << "HADRONIC, ";
    if (t.is(DetType::MUON))
      os << "MUON, ";
