...
namespace DD4hep {
  ...
  enum PrintLevel { NOLOG = 0, VERBOSE = 1, DEBUG = 2, INFO = 3, WARNING = 4, ERROR = 5, FATAL = 6, ALWAYS };
  ...
  template <typename T> struct Printer {
    ...
    Printer(const Geometry::LCDD* l, std::ostream& stream, const std::string& p = "") : lcdd(l), os(stream), prefix(p) {}
    /// Callback operator to be specialized depending on the element type
    void operator()(const T& value) const;
  };

  template <typename T>
  inline std::ostream& print(const T& object, std::ostream& os = std::cout, const std::string& indent = "") {
