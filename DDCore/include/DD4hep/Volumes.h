  ...
  class VolIDs : public std::vector<VolID> {
   public:
    ...
    VolIDs& operator=( const VolIDs& c ) {
      if ( &c != this )
        this->std::vector<VolID>::operator=( c );
      return *this;
    }
    /// Find entry
...
class PlacedVolume : public Handle<TGeoNode> {
 public:
  ...
  template <typename T>
  PlacedVolume( const Handle<T>& e ) : Handle<TGeoNode>( e ) {
  }
...
class Volume : public Handle<TGeoVolume> {
 ...
 public:
  ...
  /// Copy from handle
  Volume( const TGeoVolume* v ) : Base( v ) {
  }
  ...
  template <typename T>
  Volume( const Handle<T>& v ) : Base( v ) {
  }
...
class Assembly : public Volume {
 public:
  ...
  /// Copy from arbitrary Element
  template <typename T>
  Assembly( const Handle<T>& v ) : Volume( v ) {
  }
