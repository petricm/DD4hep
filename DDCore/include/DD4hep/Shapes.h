...
namespace DD4hep {
  ...
  namespace Geometry {
    ...
    public:
      ...
      std::string toString(int precision = 2) const { return toStringSolid(this->m_element, precision); }
    };
    ...
    public:
      ...
      /// Constructor to create an anonymous new box object (retrieves name from volume)
      template <typename X, typename Y, typename Z> Box(const X& x_val, const Y& y_val, const Z& z_val) {
    ...
    public:
      ...
      Tube(const std::string& nam, double rmin, double rmax, double z) { make(nam, rmin, rmax, z, 0, 2 * M_PI); }
    ...
    public:
      ...
      /// Constructor to create a new anonymous tube object with attribute initialization
      template <typename A, typename B, typename DZ> EllipticalTube(const A& a, const B& b, const DZ& dz) {
