...
namespace DD4hep {
  ...
  namespace Geometry {
    ...
    public:
      ...
      Box(LCDD& lcdd, const std::string& name) { make(lcdd, name, 0, 0, 0); }

      /// Constructor to be used when creating a new box object
      Box(LCDD& lcdd, const std::string& name, double x, double y, double z) { make(lcdd, name, x, y, z); }
    ...
    struct Polycone : public Solid_type<TGeoPcon> {
      ...
      Polycone(LCDD& lcdd, const std::string& name, double start, double delta, const std::vector<double>& rmin,
    ...
    public:
      ...
      Tube(LCDD& lcdd, const std::string& name) { make(lcdd, name, 0, 0, 0, 0); }
    ...
    public:
      ...
      Cone(LCDD& lcdd, const std::string& name) { make(lcdd, name, 0, 0, 0, 0, 0); }

      /// Constructor to be used when creating a new object with attribute initialization
      Cone(LCDD& lcdd, const std::string& name, double z, double rmin1, double rmax1, double rmin2, double rmax2) {
        make(lcdd, name, z, rmin1, rmax1, rmin2, rmax2);
      }

      template <typename Z, typename RMIN1, typename RMAX1, typename RMIN2, typename RMAX2>
      Cone(LCDD& lcdd, const std::string& name, const Z& z, const RMIN1& rmin1, const RMAX1& rmax1, const RMIN2& rmin2,
           const RMAX2& rmax2) {
    ...
    struct Trap : public Solid_type<TGeoTrap> {
      ...
      /// Constructor to be used when creating a new object with attribute initialization
      Trap(LCDD& lcdd, const std::string& name, double z, double theta, double phi, double y1, double x1, double x2,
           double alpha1, double y2, double x3, double x4, double alpha2);
    ...
    struct SubtractionSolid : public BooleanSolid {
      ...
      /// Constructor to be used when creating a new object
      SubtractionSolid(LCDD& lcdd, const std::string& name, const Solid& shape1, const Solid& shape2, const Position& pos,
                       const Rotation& rot);
    };
