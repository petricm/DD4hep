...
namespace DD4hep {
  ...
  public:
    ...
    /// User constructor
    template <typename TYPE> Property(TYPE& val) : m_par(0), m_hdl(0) { make(val); }
    /// Property type name
  ...
  public:
    ...
    template <typename T> void value(TYPE& val) const { this->Property::value(val); }
  ...
  public:
    ...
    /// Add a new property
    template <typename T> void add(const std::string& name, T& value) { add(name, Property(value)); }
  ...
  /// Declare property
  template <typename T> void PropertyConfigurable::declareProperty(const std::string& nam, T& val) {
    m_properties.add(nam, val);
  }

  /// Declare property
  template <typename T> void PropertyConfigurable::declareProperty(const char* nam, T& val) { m_properties.add(nam, val); }
