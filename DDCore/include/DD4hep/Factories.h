...
template <typename T>
class LCDDConstructionFactory : public PluginFactoryBase {
 public:
  static void* create( Geometry::LCDD& lcdd, int argc, char** argv );
};
template <typename T>
class SegmentationFactory : public PluginFactoryBase {
 public:
 ...
 */
template <typename T>
class ApplyFactory : public PluginFactoryBase {
 public:
 ...
 */
template <typename T>
class TranslationFactory : public PluginFactoryBase {
 public:
 ...
 */
template <typename T>
class XMLElementFactory : public PluginFactoryBase {
 public:
 ...
 */
template <typename T>
class XMLDocumentReaderFactory : public PluginFactoryBase {
 public:
 ...
 */
template <typename T>
class XMLConversionFactory : public PluginFactoryBase {
 public:
 ...
 */
template <typename T>
class DetElementFactory : public PluginFactoryBase {
 public:
...
#define DECLARE_SEGMENTATION( name, func )                                                             \
  DD4HEP_OPEN_PLUGIN( DD4hep, name ) {                                                                 \
    template <>                                                                                        \
    Geometry::SegmentationObject* SegmentationFactory<name>::create( DDSegmentation::BitField64* d ) { \
      return func( d );                                                                                \
    }                                                                                                  \
...
#define DECLARE_APPLY( name, func )                                                        \
  DD4HEP_OPEN_PLUGIN( DD4hep, name ) {                                                     \
    template <>                                                                            \
    long ApplyFactory<name>::create( Geometry::LCDD& l, int n, char** a ) {                \
      return func( l, n, a );                                                              \
    }                                                                                      \
...
#define DECLARE_CONSTRUCTOR( name, func )                                \
  DD4HEP_OPEN_PLUGIN( DD4hep, name ) {                                   \
    template <>                                                          \
    void* ConstructionFactory<name>::create( const char* n ) {           \
      return func( n );                                                  \
    }                                                                    \
...
#define DECLARE_LCDD_CONSTRUCTOR( name, func )                                            \
  DD4HEP_OPEN_PLUGIN( DD4hep, name ) {                                                    \
    template <>                                                                           \
    void* LCDDConstructionFactory<name>::create( Geometry::LCDD& l, int n, char** a ) {   \
      return func( l, n, a );                                                             \
    }                                                                                     \
...
#define DECLARE_TRANSLATION( name, func )                                   \
  DD4HEP_OPEN_PLUGIN( DD4hep, name ) {                                      \
    template <>                                                             \
    Geometry::Ref_t TranslationFactory<name>::create( Geometry::LCDD& l ) { \
      return func( l );                                                     \
    }                                                                       \
...
#define DECLARE_XMLELEMENT( name, func )                                                                          \
  DD4HEP_OPEN_PLUGIN( DD4hep, xml_element_##name ) {                                                              \
    template <>                                                                                                   \
    Geometry::Ref_t XMLElementFactory<xml_element_##name>::create( Geometry::LCDD& l, xml_h e ) {                 \
      return func( l, e );                                                                                        \
    }                                                                                                             \
...
#define DECLARE_XML_DOC_READER( name, func )                                                                   \
  DD4HEP_OPEN_PLUGIN( DD4hep, xml_document_##name ) {                                                          \
    template <>                                                                                                \
    long XMLDocumentReaderFactory<xml_document_##name>::create( Geometry::LCDD& l, xml_h e ) {                 \
      return func( l, e );                                                                                     \
    }                                                                                                          \
    DD4HEP_PLUGINSVC_FACTORY( xml_document_##name, name##_XML_reader, long( Geometry::LCDD*, XML::Handle_t* ), \
                              __LINE__ )                                                                       \
  }
...
#define DECLARE_XML_PROCESSOR_BASIC( name, func, deprecated )                                              \
  DD4HEP_OPEN_PLUGIN( DD4hep, det_element_##name ) {                                                       \
    template <>                                                                                            \
    Geometry::Ref_t DetElementFactory<det_element_##name>::create( Geometry::LCDD& l, xml_h e, ref_t h ) { \
      if ( deprecated )                                                                                    \
        warning_deprecated_xml_factory( #name );                                                           \
      return func( l, e, h );                                                                              \
    }                                                                                                      \
    DD4HEP_PLUGINSVC_FACTORY( det_element_##name, name,                                                    \
                              NamedObject*(Geometry::LCDD*, XML::Handle_t*, Geometry::Ref_t*), __LINE__ )  \
  }
