...
class Callback {
 public:
  ...
  /// Default constructor
  Callback() : par( 0 ), call( 0 ) {
    func.first = func.second = 0;
  }
  /// Constructor with object initialization
  Callback( void* p ) : par( p ), call( 0 ) {
    func.first = func.second = 0;
  }
  /// Initializing constructor
  Callback( void* p, void* mf, func_t c ) : par( p ), call( c ) {
    func = *(mfunc_t*)mf;
  }
...
/// Execution overload for callbacks with 1 argument
template <typename A0>
inline void CallbackSequence::operator()( A0 a0 ) const {
  if ( !callbacks.empty() ) {
...
/// Execution overload for callbacks with 2 arguments
template <typename A0, typename A1>
inline void CallbackSequence::operator()( A0 a0, A1 a1 ) const {
  if ( !callbacks.empty() ) {
...
/// Execution overload for callbacks with 3 arguments
template <typename A0, typename A1, typename A2>
inline void CallbackSequence::operator()( A0 a0, A1 a1, A2 a2 ) const {
  if ( !callbacks.empty() ) {
