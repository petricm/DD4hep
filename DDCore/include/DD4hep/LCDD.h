...
namespace DD4hep {
  ...
  namespace Geometry {
    ...
    public:
      ...
      */
      virtual const std::vector<DetElement>& detectors(const std::string& type, bool throw_exc = false) = 0;

      /// Access a set of subdetectors according to several sensitive types.
      virtual std::vector<DetElement> detectors(const std::string& type1, const std::string& type2,
      ...
      virtual void fromXML(const std::string& fname, UriReader* entity_resolver, LCDDBuildType type = BUILD_DEFAULT) = 0;
      ...
      template <class T> T* removeExtension(bool destroy = true) { return (T*)removeUserExtension(typeid(T), destroy); }

      /// Access extension element by the type
      template <class T> T* extension(bool alert = true) const { return (T*)userExtension(typeid(T), alert); }
...
/*
    ...
    template <> inline short LCDD::constant<short>(const std::string& name) const { return (short)constantAsLong(name); }
    ...
    template <> inline int LCDD::constant<int>(const std::string& name) const { return (int)constantAsLong(name); }
    ...
    /// Typed access to constants: long values
    template <> inline long LCDD::constant<long>(const std::string& name) const { return constantAsLong(name); }
    ...
    template <> inline float LCDD::constant<float>(const std::string& name) const { return (float)constantAsDouble(name); }

    /// Typed access to constants: double values
    template <> inline double LCDD::constant<double>(const std::string& name) const { return constantAsDouble(name); }
