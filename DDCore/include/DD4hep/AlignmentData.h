...
class Delta {
 public:
  ...
  Delta( const Position& tr ) : translation( tr ), flags( HAVE_TRANSLATION ) {
  }
  /// Initializing constructor
  Delta( const RotationZYX& rot ) : translation(), rotation( rot ), flags( HAVE_ROTATION ) {
  }
  /// Initializing constructor
 ...
 */
template <typename T>
class AlignmentDecorator : public T {
 public:
  /// Forward definition of the geometry placement
...
/// Default constructor
template <typename T>
inline AlignmentDecorator<T>::AlignmentDecorator() : T() {
}

/// Copy constructor
template <typename T>
inline AlignmentDecorator<T>::AlignmentDecorator( const AlignmentDecorator& c ) : T( c ) {
}
