...
#endif
namespace {
static XmlTools::Evaluator& s__eval( DD4hep::g4Evaluator() );
}
...
class Grammar : public BasicGrammar {
...
/// PropertyGrammar overload: Access to the type information
template <typename TYPE>
const std::type_info& Grammar<TYPE>::type() const {
  return typeid( TYPE );
}

/// PropertyGrammar overload: Access to the type information
template <typename TYPE>
const std::string& Grammar<TYPE>::type_name() const {
  return m_typeName;
}

/// Access the object size (sizeof operator)
template <typename TYPE>
size_t Grammar<TYPE>::sizeOf() const {
  return sizeof( TYPE );
}

/// Evaluate string value if possible before calling boost::spirit
template <typename TYPE>
int Grammar<TYPE>::evaluate( void*, const std::string& ) const {
  return 0;
}

/// PropertyGrammar overload: Retrieve value from string
template <typename TYPE>
bool Grammar<TYPE>::fromString( void* ptr, const std::string& string_val ) const {
  int  sc = 0;
  TYPE temp;
#ifdef DD4HEP_USE_BOOST
...
template <typename TYPE>
std::string Grammar<TYPE>::str( const void* ptr ) const {
#ifdef DD4HEP_USE_BOOST
...
/// Item evaluator
template <typename T>
inline int eval_item( T* p, std::string s ) {
...
/// String evaluator
template <>
inline int eval_item<std::string>( std::string* p, std::string s ) {
  *p = s;
  return 1;
}
...
inline int eval_pair( std::pair<T, Q>* p, std::string s ) {
...
/// Object evaluator
template <typename T>
inline int eval_obj( T* p, const std::string& str ) {
...
template <typename T>
inline int eval_none( T*, const std::string& ) {
  return 1;
}
template <typename T>
inline int parse_none( T&, const std::string& ) {
  return 1;
}
...
#define DD4HEP_DEFINE_PARSER_GRAMMAR_TYPE( x )      \
  namespace DD4hep {                                \
  template <>                                       \
  const BasicGrammar& BasicGrammar::instance<x>() { \
    static Grammar<x> s;                            \
    return s;                                       \
  }                                                 \
  }
...
#define DD4HEP_DEFINE_PARSER_GRAMMAR_EVAL( x, func )                \
  namespace DD4hep {                                                \
  template <>                                                       \
  int Grammar<x>::evaluate( void* p, const std::string& v ) const { \
    return func( (x*)p, v );                                        \
  }                                                                 \
  }
