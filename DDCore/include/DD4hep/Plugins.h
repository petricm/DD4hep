...
namespace DD4hep {
  ...
  template <> inline const std::string& PluginFactoryBase::value<const std::string&>(const void* p) {
    return *(std::string*)(p);
  }
  ...
  public:
    ...
    template <typename FUNCTION> static FuncPointer<FUNCTION> function(FUNCTION func) { return FuncPointer<FUNCTION>(func); }
    ...
    template <typename R, typename A0> static R Create(const std::string& name, A0 a0);

    template <typename R, typename A0, typename A1> static R Create(const std::string& name, A0 a0, A1 a1);
...
#else
namespace DD4hep {
  template <> inline long PluginFactoryBase::make_return(const long& p) {
    static long s = p;
    return (long)&s;
  }
}
...
#define DD4HEP_PLUGIN_FACTORY_ARGS_0(R)                                                                       \
  template <typename P> class Factory<P, R()> : public DD4hep::PluginFactoryBase {                            \
  public:                                                                                                     \
    static void wrapper(void* ret, void*, const std::vector<void*>&, void*) { *(void**)ret = (void*)call(); } \
    static R                  call();                                                                         \
  };                                                                                                          \
  template <typename P> inline R Factory<P, R()>::call()

#define DD4HEP_PLUGIN_FACTORY_ARGS_1(R, A0)                                          \
  template <typename P> class Factory<P, R(A0)> : public DD4hep::PluginFactoryBase { \
  public:                                                                            \
...
#define DD4HEP_PLUGIN_FACTORY_ARGS_2(R, A0, A1)                                          \
  template <typename P> class Factory<P, R(A0, A1)> : public DD4hep::PluginFactoryBase { \
  public:                                                                                \
    static void wrapper(void* ret, void*, const std::vector<void*>& a, void*) {          \
...
#define DD4HEP_PLUGIN_FACTORY_ARGS_3(R, A0, A1, A2)                                          \
  template <typename P> class Factory<P, R(A0, A1, A2)> : public DD4hep::PluginFactoryBase { \
  public:                                                                                    \
    static void wrapper(void* ret, void*, const std::vector<void*>& a, void*) {              \
...
#define DD4HEP_PLUGIN_FACTORY_ARGS_4(R, A0, A1, A2, A3)                                               \
  template <typename P> class Factory<P, R(A0, A1, A2, A3)> : public DD4hep::PluginFactoryBase {      \
  public:                                                                                             \
    static void wrapper(void* ret, void*, const std::vector<void*>& a, void*) {                       \
...
#define DD4HEP_PLUGIN_FACTORY_ARGS_5(R, A0, A1, A2, A3, A4)                                                            \
  template <typename P> class Factory<P, R(A0, A1, A2, A3, A4)> : public DD4hep::PluginFactoryBase {                   \
  public:                                                                                                              \
    static void wrapper(void* ret, void*, const std::vector<void*>& a, void*) {                                        \
