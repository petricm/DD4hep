...
namespace DD4hep {
  ...
  namespace XML {
    ...
    struct Strng_t {
      ...
      ~Strng_t() {
        if (m_xml)
          xercesc::XMLString::release(&m_xml);
      }
...
#define INLINE inline
    template <> INLINE Attribute Handle_t::attr<Attribute>(const XMLCh* tag) const { return attr_ptr(tag); }

    typedef const XMLCh* cpXMLCh;
    template <> INLINE cpXMLCh Handle_t::attr<cpXMLCh>(const XMLCh* tag) const { return attr_value(tag); }

    template <> INLINE bool Handle_t::attr<bool>(const XMLCh* tag) const { return _toBool(attr_value_nothrow(tag)); }

    template <> INLINE int Handle_t::attr<int>(const XMLCh* tag) const { return _toInt(attr_value(tag)); }

    template <> INLINE float Handle_t::attr<float>(const XMLCh* tag) const { return _toFloat(attr_value(tag)); }

    template <> INLINE double Handle_t::attr<double>(const XMLCh* tag) const { return _toDouble(attr_value(tag)); }

    template <> INLINE std::string Handle_t::attr<std::string>(const XMLCh* tag) const {
      return _toString(attr_nothrow(tag));
    }

    template <> INLINE bool Handle_t::attr<bool>(const Attribute tag) const { return _toBool(attr_value(tag)); }

    template <> INLINE int Handle_t::attr<int>(const Attribute tag) const { return _toInt(attr_value(tag)); }

    template <> INLINE float Handle_t::attr<float>(const Attribute tag) const { return _toFloat(attr_value(tag)); }

    template <> INLINE double Handle_t::attr<double>(const Attribute tag) const { return _toDouble(attr_value(tag)); }
