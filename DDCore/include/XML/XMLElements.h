...
namespace DD4hep {
  ...
  namespace XML {
    ...
    /// Format void pointer (64 bits) to string with arbitrary format  \ingroup DD4HEP_XML
    template <typename T> std::string _toString(const T* p, const char* fmt = "%p") { return _ptrToString((void*)p, fmt); }
    ...
    public:
      ...
      /// String length in native representation
      size_t length() const { return XmlString::length(m_xml); }
...
#define INLINE inline
    ...
    template <> INLINE Attribute Handle_t::attr<Attribute>(const XmlChar* tag_value) const { return attr_ptr(tag_value); }

    template <> INLINE cpXmlChar Handle_t::attr<cpXmlChar>(const XmlChar* tag_value) const { return attr_value(tag_value); }

    template <> INLINE bool Handle_t::attr<bool>(const XmlChar* tag_value) const { return _toBool(attr_value(tag_value)); }

    template <> INLINE int Handle_t::attr<int>(const XmlChar* tag_value) const { return _toInt(attr_value(tag_value)); }

    template <> INLINE long Handle_t::attr<long>(const XmlChar* tag_value) const { return _toLong(attr_value(tag_value)); }
    ...
    public:
      ...
      Handle_t parent() const { return m_element.parent(); }
      /// Access the XmlElements parent
      ...
      template <class T> T attr(const XmlChar* tag_value) const { return m_element.attr<T>(tag_value); }
#ifndef __TIXML__
      /// Access typed attribute value by it's name
      template <class T> T attr(const char* name) const { return this->attr<T>(Strng_t(name)); }
#endif
      /// Access attribute name (throws exception if not present)
      const XmlChar* attr_name(const Attribute a) const { return m_element.attr_name(a); }
      /// Access attribute value by the attribute  (throws exception if not present)
      const XmlChar* attr_value(const Attribute a) const { return m_element.attr_value(a); }
      /// Access the number of children of this DOM element with a given tag name
      size_t numChildren(const XmlChar* tag_value, bool exc = true) const { return m_element.numChildren(tag_value, exc); }
      ...
      template <class T> Attribute setAttr(const XmlChar* nam, const T& val) const { return m_element.setAttr(nam, val); }
      /// Set element value
      ...
      Handle_t child(const Strng_t& tag_value, bool except = true) const { return m_element.child(tag_value, except); }
      /// Remove a child node identified by its handle
      Handle_t remove(Handle_t node) const { return m_element.remove(node); }
      /// Check the existence of a child with a given tag name
      bool hasChild(const XmlChar* tag_value) const { return m_element.hasChild(tag_value); }
