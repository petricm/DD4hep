...
static Ref_t create_detector( LCDD& lcdd, xml_h e, SensitiveDetector sens ) {
  ...
  DetElement sdet(
      det_name,
      x_det.id() );  // sdet is the detelement of the detector!!(actually is a Handle,already a pointer to m_element)
  ...
  PlacedVolume pv;  // struct of Handle giving the volume id(ayto pou 8a kanw volume kai 8a to steilw me
                    // setplacement),dld o detector mou
  ...
  for ( int i = 0; i < Ni;
        i++ ) {  // lets start from 100 and when getting the x dimensions of the detector, use num_pixels
    for ( int j = 0; j < Nj; j++ ) {
      ...
      // printout(INFO,det_name,"Giving to this pixel with position (%7.3f, %7.3f, %7.3f)
      // id:%d",ppxl.x(),ppxl.y(),ppxl.z(),mi_id);
    }
  }
  ...
  pv = motherVol.placeVolume(
      m_volume, Transform3D( Position( det_x, det_y,
                                       det_z ) ) );  // det_x,det_y,det_z are the dimensions of the detector in space
