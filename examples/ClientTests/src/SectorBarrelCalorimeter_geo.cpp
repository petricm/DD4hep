...
static void placeStaves( DetElement& parent, DetElement& stave, double rmin, int numsides, double total_thickness,
  ...
  for ( int module = 1; module <= numsides; ++module ) {
    ...
    PlacedVolume pv  = envelopeVolume.placeVolume(
        sectVolume, Transform3D( RotationZYX( 0, rotY, rotX ), Translation3D( -posX, -posY, 0 ) ) );
...
static Ref_t create_detector( LCDD& lcdd, xml_h e, SensitiveDetector sens ) {
  ...
  for ( xml_coll_t c( x_det, _U( layer ) ); c; ++c ) {
    ...
    for ( int j = 0; j < repeat; j++ ) {
      ...
      PlacedVolume layer_phv = staveOuterVol.placeVolume(
          layer_vol, Position( ( layer_dim_x + layer_pos_x - half_polyFace ), 0, layer_pos_z ) );
      ...
      cout << "Rmin: " << rmin << " Rmax: " << rmax << " half_polyFace: " << half_polyFace << " Layer " << layer_num
           << " layerR: " << layerR << " layer_dim_x:" << layer_dim_x << endl;
