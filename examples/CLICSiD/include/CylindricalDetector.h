...
namespace DD4hep {
  namespace Geometry {
    ...
    public:
      ...
      double getRMin() const { return getCylinder()->GetRmin(); }
      double getRMax() const { return getCylinder()->GetRmax(); }
      double getZMin() const { return -getCylinder()->GetDZ() / 2.; }
      double getZMax() const { return getCylinder()->GetDZ() / 2.; }

    protected:
      Tube getCylinder() const { return Tube(volume().solid()); }
    };
