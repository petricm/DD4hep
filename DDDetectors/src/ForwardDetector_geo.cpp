  ...
  for ( xml_coll_t c( x_det, _U( layer ) ); c; ++c ) {
    ...
    for ( int i = 0, repeat = x_layer.repeat(); i < repeat; ++i ) {
      ...
      for ( xml_coll_t l( x_layer, _U( slice ) ); l; ++l ) {
        ...
        SubtractionSolid sliceSubtraction1( sliceTube, beamInTube,
                                            Transform3D( beamInRot, Position( slicePosX, 0, 0 ) ) );
        // Second slice subtraction solid.
        SubtractionSolid sliceSubtraction2( sliceSubtraction1, beamOutTube,
                                            Transform3D( beamOutRot, Position( -slicePosX, 0, 0 ) ) );
        // Slice LV.
