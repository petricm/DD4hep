...
void Installer<UserData>::install( DetElement component, PlacedVolume pv ) {
  ...
  if ( comp_vol.isSensitive() ) {
    ...
    if ( !comp_shape.isValid() || !mod_shape.isValid() ) {
      ...
      std::cout << "dz:" << dz << " Si-pos:" << si_position << " Mod-thickness:" << half_mod_thickness
                << " Comp-thickness:" << half_comp_thickness << std::endl;
      VolPlane surf( comp_vol, Type( Type::Sensitive, Type::Measurement1D ), inner_thickness, outer_thickness, u, v, n,
                     o );
