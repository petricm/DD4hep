...
class CaloBarrelPlaneImpl : public DD4hep::DDRec::VolPlaneImpl {
 ...
 public:
  ...
  CaloBarrelPlaneImpl( DDSurfaces::SurfaceType typ, double thickness_inner, double thickness_outer,
                       DDSurfaces::Vector3D u_val, DDSurfaces::Vector3D v_val, DDSurfaces::Vector3D n_val,
                       DDSurfaces::Vector3D o_val, DD4hep::Geometry::Volume vol, int id_val )
  ...
  void setID( DD4hep::long64 id_val ) {
    _id = id_val;
  }
  ...
  bool insideBounds( const DDSurfaces::Vector3D& point, double epsilon ) const {
    ...
    return ( std::abs( distance( point ) ) < epsilon ) && std::abs( uvVec[ 0 ] ) < _width / 2. &&
           std::abs( uvVec[ 1 ] ) < _length / 2.;
  }
  ...
  virtual std::vector<std::pair<DDSurfaces::Vector3D, DDSurfaces::Vector3D>> getLines( unsigned ) {
    ...
    lines.push_back( std::make_pair( origin() + _width / 2. * u() + _length / 2. * v(),
...
void Installer<UserData>::install( DetElement component, PlacedVolume pv ) {
  ...
  printout( DD4hep::INFO, "DD4hep_CaloFaceBarrelSurfacePlugin", "install tracking surfaces for :  %s ",
            component.name() );
  ...
  for ( unsigned i = 0; i < symmetry; ++i ) {
    ...
    Vector3D u( cos( gam + M_PI / 2. ), sin( gam + M_PI / 2. ), 0. ), v( 0., 0., 1. ), n( cos( gam ), sin( gam ), 0. ),
        o( radius * cos( gam ), radius * sin( gam ), 0. );

    CaloBarrelPlane surf( comp_vol, Type( Type::Helper, Type::Sensitive ), inner_thickness, outer_thickness, u, v, n,
                          o );
