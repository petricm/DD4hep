...
static Ref_t create_detector( LCDD& lcdd, xml_h e, SensitiveDetector sens ) {
  ...
  if ( reflect ) {
    ...
    pv =
        assembly.placeVolume( endcapVol, Transform3D( RotationZYX( M_PI / numsides, 0, 0 ), Position( 0, 0, z_pos ) ) );
  ...
  pv = motherVol.placeVolume( endcapVol, Transform3D( RotationZYX( M_PI / numsides, 0, 0 ), Position( 0, 0, z_pos ) ) );
  pv.addPhysVolID( "system", det_id );
  pv.addPhysVolID( "barrel", 1 );
